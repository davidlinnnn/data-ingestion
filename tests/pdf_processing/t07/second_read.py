...
async def main():
    ...
    store=Store(s3,'t07','second-read-'+uuid.uuid4().hex)
    ...
    source=json.loads(Path('/tmp/t07-matrix/baseline.json').read_text())
    result=source['result']
    original=Store(s3,'t07','final')
    ...
    class DisappearOnSecondRead:
        ...
        def get_object(self,**kwargs):
            if kwargs['Key']==target:
                self.reads+=1
                if self.reads==2:s3.delete_object(Bucket='t07',Key=target)
