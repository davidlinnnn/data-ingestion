...
async def main():
    ...
    if not any(x['Name']=='t04' for x in s3.list_buckets()['Buckets']): s3.create_bucket(Bucket='t04')
    s3.put_bucket_versioning(Bucket='t04',VersioningConfiguration={'Status':'Enabled'})
    store=Store(s3,'t04','final');client=await Client.connect('temporal:7233')
    ...
    for path in sorted(Path(os.environ.get('T04_FIXTURES','/fixtures')).glob('*.pdf')):
        ...
        saved=s3.put_object(Bucket='t04',Key=key,Body=data)
        ...
        for repeat in range(2):
            handle=await client.start_workflow(PDFProcessing.run,{'request':req,'activity_queue':'t04-pdf'},
                       id='t04-'+uuid.uuid4().hex,task_queue='t04-workflows')
    ...
    legacy_result = await client.execute_workflow(PDFProcessing.run, {'request':legacy,'activity_queue':'t04-pdf'},
                        id='legacy-'+uuid.uuid4().hex,task_queue='t04-workflows')
