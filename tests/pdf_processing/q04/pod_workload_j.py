...
RUN_ID = "q04-yolo-pod-cgroup-20260920-j"
WORKFLOW_QUEUE = "q04-pod-cgroup-j-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-j-07"
EVIDENCE_ROOT = Path("/q04-evidence/q04-yolo-pod-cgroup-20260920-j")
PARSER_BUDGETS = {
    "startup_seconds": 120,
...
def build_init_argv(args) -> list[str]:
    return [
        args.python,
        str(args.workspace / "tests/pdf_processing/q04/pod_init_j.py"),
...
def cleanup_markers(state: Path) -> dict:
    roots = sorted(root for root in state.glob("yolo-pod-cgroup-j/worker-*") if root.is_dir())
