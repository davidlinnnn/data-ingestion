...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_preflight_be_engine", Path(__file__).with_name("pod_preflight_q.py")
)
...
TOPOLOGY_PHASE = "q04-pod-cgroup-be"
RUN_ID = "q04-process-drain-pod-cgroup-20260925-be"
sha256 = base.base.sha256


def verify_configuration_at(**kwargs):
...
def verify_workload_imports_at(*, workspace, bundle, prefix):
    modules = (
        "candidate.process_drain_window_be",
        "pod_init_p",
        "pod_workload_be",
        "pod_remote_evidence_be",
        "host_bc",
        "host_be",
        "worker_bc",
    )
    ...
    from candidate.process_drain_window_be import validate_scope
    ...
    path = workspace / "tests/pdf_processing/q04/pod-topology-v56/RUNTIME-INTEGRATION-MANIFEST.json"
    ...
    return {
        ...
        "reviewed_contract": {
            ...
            "fixtures": ["native"],
            "modes": ["drain"],
            "injection": "stop owned worker process during second group without Pod replacement",
...
base.RUN_ID = RUN_ID
base.verify_configuration_q = verify_configuration_at
base.verify_workload_imports_q = verify_workload_imports_at
main = base.main
