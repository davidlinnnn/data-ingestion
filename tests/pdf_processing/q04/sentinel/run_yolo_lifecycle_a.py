"""25-minute YOLO fixture-07 lifecycle candidate acceptance runner."""
...
OFFLINE_MANIFEST = (
    REPO
    / "tests/pdf_processing/q04/preflight/yolo-lifecycle-a/OFFLINE-MANIFEST.json"
)
REMOTE = "/tmp/q04-yolo-lifecycle-20260919-a"
PREFIX = "q04/yolo-lifecycle-20260919-a/"
...
SOURCE_LOCAL_BUNDLE = Path("/private/tmp/q04-inputs-option-a-v7")
MODEL_CACHE = "/experiment/PROTOTYPE-wipe-me/hf"
PHASE = "yolo-lifecycle-a"
OUT = Path("/private/tmp/q04-yolo-lifecycle-20260919-a")
CAPACITY = REMOTE + "/capacity-yolo-lifecycle-a.json"
RESERVATION = REMOTE + "/reservation-yolo-lifecycle-a.json"
RELEASE = REMOTE + "/release-reservation-yolo-lifecycle-a"
ADMISSION_EVIDENCE = REMOTE + "/pre-admission-yolo-lifecycle-a.jsonl"
ADMISSION_RESULT = REMOTE + "/admission-yolo-lifecycle-a.json"
RUNNER_DIR = REMOTE + "/runner-yolo-lifecycle-a"
...
EXPECTED_BUNDLE_SHA256 = (
    "37a4cf0259d659d6ecdbce5a752d0bb8c6dc1a3679b1ade92d685a449858375c"
)
...
require(len(CANDIDATES) == 32, "expected exactly 32 held Deployments")
STATE = {
    "phase": "preparing",
    "errors": [],
...
for item in s3.list_objects_v2(Bucket='t09a',Prefix=prefix+'registered/').get('Contents',[]):
 ...
 for file in manifest['files']:
  ...
  if hashlib.sha256(raw).hexdigest()!=file['sha256'] or len(raw)!=file['bytes']:
   raise ValueError('retained object digest changed')
...
if len(found)!=1:raise ValueError('expected one retained request binding')
...
def build_staging_probe_program(*, root=REMOTE):
    ...
    candidate_files = sorted(candidate_staging_files())
...
""".replace("ROOT", repr(root)).replace("CANDIDATE_FILES", repr(candidate_files))
...
def validate_staged_runtime(staged, bundle):
    ...
    if staged["bound_files"] != expected_bound:
        raise ValueError("staged candidate worker or harness changed")
...
def build_measurement_argv(
    ...
    *,
    ...
    """Return the fixed argv for the candidate acceptance driver."""
    if not expected_run_id.startswith("q04-"):
        raise ValueError("initialized run identity is required")
    return [
        python,
        RUNNER_DIR + "/yolo_candidate_window.py",
...
def build_runtime_command(
    ...
    *,
    ...
    """Render a fixed shell wrapper around one safely quoted diagnostic argv."""
    verify = (
    ...
    inner = "\n".join(
        (
            ...
            "exec " + shlex.join(build_measurement_argv(
                expected_run_id, remote=remote, capacity=capacity, python=python
            )),
        )
    )
    return "\n".join(
        (
            ...
            'export PYTHONPATH="$R/runner-yolo-lifecycle-a:$R/code/src:'
...
def offline_validation_record():
    """Validate the complete fixed staging/init/launch interface without I/O."""
    bundle = json.loads((LOCAL_BUNDLE / "inputs.json").read_text())
    staged = {
        "bundle_sha256": hashlib.sha256(
            (LOCAL_BUNDLE / "inputs.json").read_bytes()
        ).hexdigest(),
        "runtime_sha256": hashlib.sha256(
            (REPO / "tests/pdf_processing/q04/q04_runtime.py").read_bytes()
        ).hexdigest(),
        "reference_07_sha256": hashlib.sha256(
            (LOCAL_BUNDLE / "references/07.json").read_bytes()
        ).hexdigest(),
        "quality_oracle_sha256": hashlib.sha256(
            (LOCAL_BUNDLE / "oracles/quality-oracle.json").read_bytes()
        ).hexdigest(),
        "table_oracle_sha256": hashlib.sha256(
            (LOCAL_BUNDLE / "oracles/07-table-full-audit.json").read_bytes()
        ).hexdigest(),
    ...
    initialized = {
        "run_id": "q04-offline-candidate",
        "prefix": PREFIX,
        "bundle": REMOTE + "/inputs",
        "bundle_sha256": EXPECTED_BUNDLE_SHA256,
        "state_sha256": "offline-state-sha256",
        "producer": bundle["producer"],
    ...
    argv = build_measurement_argv(run_id)
    command = build_runtime_command(run_id)
    ...
    require(retained["identity"] == identity, "offline identity manifest changed")
    require(retained["measurement_argv"] == argv, "offline argv manifest changed")
    require(
        retained["candidate_staging_files"]
        == staged["bound_files"],
        "offline candidate staging manifest changed",
    )
    reviewed_source_hashes()
    return {
        "status": "PASS offline only",
        "identity": identity,
        "candidate_inputs_sha256": staged["bundle_sha256"],
        "producer_manifest_sha256": json.loads(CANDIDATE_MANIFEST.read_text())[
            "producer_manifest_sha256"
        ],
        "measurement_argv": argv,
        "runtime_command_sha256": hashlib.sha256(command.encode()).hexdigest(),
        "outer_observation_seconds": capacity["outer_observation_seconds"],
        "outer_continuous_seconds": capacity["outer_continuous_seconds"],
...
def staged_sources():
    return {
        ...
        "run_yolo_lifecycle_a.py": Path(__file__),
    }


def candidate_staging_files(manifest_path=CANDIDATE_MANIFEST):
    """Return every candidate file that must replace the frozen code tree."""
    manifest = json.loads(manifest_path.read_text())
    require(
        manifest["candidate_inputs_sha256"] == EXPECTED_BUNDLE_SHA256,
    ...
    paths.update({name: REPO / name for name in manifest["changed_test_files"]})
    expected = {
    ...
    require(observed == expected, "candidate source or worker manifest changed")
    return paths
...
def reviewed_source_hashes(sources=None, manifest_path=OFFLINE_MANIFEST):
    """Bind staging to the reviewed offline manifest, not the working tree."""
    retained = json.loads(manifest_path.read_text())
    require(retained.get("phase") == PHASE, "offline manifest phase changed")
    require(retained.get("fixture") == "07", "offline manifest fixture changed")
    require(retained.get("modes") == ["fresh", "restored", "replay"], "offline manifest modes changed")
    require(
        retained.get("runtime_authorized") is False,
        "offline manifest must not embed runtime authorization",
    )
    candidate_manifest = json.loads(CANDIDATE_MANIFEST.read_text())
    require(
        retained.get("candidate_manifest_sha256")
        == hashlib.sha256(CANDIDATE_MANIFEST.read_bytes()).hexdigest(),
        "candidate manifest digest changed",
    )
    require(
        retained.get("candidate_inputs_sha256") == EXPECTED_BUNDLE_SHA256
        and retained.get("producer_manifest_sha256")
        == candidate_manifest["producer_manifest_sha256"],
        "candidate bundle or producer identity changed",
    )
    require(
        retained.get("candidate_staging_files")
        == {
            name: hashlib.sha256(path.read_bytes()).hexdigest()
            for name, path in candidate_staging_files().items()
        },
        "candidate staging manifest changed",
    )
    observed = source_hashes(sources)
    require(
        retained.get("staged_sources") == observed,
        "staged source differs from reviewed offline manifest",
    )
    return observed


def require_reviewed_git_state(sources=None, manifest_path=OFFLINE_MANIFEST):
    """Reject local edits to every reviewed source and its manifest."""
    sources = staged_sources() if sources is None else sources
    ...
    paths.append(str(CANDIDATE_MANIFEST.resolve().relative_to(REPO)))
    paths.append(str(manifest_path.resolve().relative_to(REPO)))
...
def stage_admission_runner():
    ...
    reviewed = reviewed_source_hashes(sources)
    ...
    if manifest != reviewed or remote_manifest != reviewed:
        raise ValueError("staged admission runner digest mismatch")
    return manifest


def validate_reservation_identity(record, token):
    required = {
    ...
    for field, expected in required.items():
        require(record.get(field) == expected, "reservation " + field + " changed")
    for field in (
        "pid",
        "start_ticks",
        "lock_device_major",
        ...
        require(
            ...
            "reservation " + field + " missing",
        )
    return record


def read_reservation_identity(token):
    raw = remote(
        "from pathlib import Path\nprint(Path("
        + repr(RESERVATION)
        + ").read_text())\n"
    )
    return validate_reservation_identity(json.loads(raw), token)


def read_qualification_lock_state():
    """Read the shared qualification lock without mutating an unknown holder."""
...
if lock.exists():
 ...
 held=any(needle in line for line in Path('/proc/locks').read_text().splitlines())
print(json.dumps({'qualification_lock_held':held}))
"""
    return json.loads(remote(script, timeout=15))


def build_reservation_state_program(identity):
    """Read exact PID/start-tick and lock ownership after release/reap."""
...
alive=False
if proc.exists():
 text=proc.read_text();ticks=int(text[text.rfind(')')+1:].split()[19]);alive=ticks==record['start_ticks']
needle=f"{record['lock_device_major']:02x}:{record['lock_device_minor']:02x}:{record['lock_inode']}"
held=any(needle in line for line in Path('/proc/locks').read_text().splitlines())
print(json.dumps({'identity_alive':alive,'qualification_lock_held':held}))
""".replace("RECORD", repr(identity))


def build_reservation_reap_program(identity):
    """Terminate only the exact holder recorded by the acquired reservation."""
    return """import json,os,signal,time
from pathlib import Path
expected=RECORD
actual=json.loads(Path(RESERVATION).read_text())
for key in ('coordinator_uid','token','phase','pid','start_ticks','lock_device_major','lock_device_minor','lock_inode'):
 if actual.get(key)!=expected.get(key):raise ValueError('reservation identity changed: '+key)
lock=Path('/tmp/data-ingestion-pdf-qualification.lock').stat()
if (os.major(lock.st_dev),os.minor(lock.st_dev),lock.st_ino)!=(expected['lock_device_major'],expected['lock_device_minor'],expected['lock_inode']):
 raise ValueError('qualification lock identity changed')
def same_process():
 path=Path('/proc')/str(expected['pid'])/'stat'
 if not path.exists():return False
 text=path.read_text();return int(text[text.rfind(')')+1:].split()[19])==expected['start_ticks']
if same_process():os.kill(expected['pid'],signal.SIGTERM)
deadline=time.monotonic()+5
while same_process() and time.monotonic()<deadline:time.sleep(.05)
if same_process():os.kill(expected['pid'],signal.SIGKILL)
deadline=time.monotonic()+2
while same_process() and time.monotonic()<deadline:time.sleep(.05)
if same_process():raise RuntimeError('identity-fenced reservation holder survived reap')
print(json.dumps({'identity_reaped':True}))
""".replace("RECORD", repr(identity)).replace("RESERVATION", repr(RESERVATION))


def _stop_local_transport(holder):
...
def release_reservation(
    ...
    *,
    ...
    """Release normally, then identity-fence and reap on either failure path."""
    release_writer = release_writer or (lambda: write_remote(RELEASE, b"complete\n"))
    remote_reaper = remote_reaper or (
        lambda: json.loads(remote(build_reservation_reap_program(identity), timeout=15))
    )
    remote_verifier = remote_verifier or (
        lambda: json.loads(remote(build_reservation_state_program(identity), timeout=15))
    )
    errors = []
    try:
        release_writer()
        holder.wait(timeout=10)
        require(holder.returncode == 0, "reservation release failed")
    except BaseException as error:
        errors.append(("normal_release", error))
        try:
            remote_reaper()
        except BaseException as error:
            errors.append(("identity_reap", error))
        try:
            _stop_local_transport(holder)
        except BaseException as error:
            errors.append(("transport_stop", error))
    state = None
    try:
        state = remote_verifier()
        require(state.get("identity_alive") is False, "reservation holder still alive")
        require(
            state.get("qualification_lock_held") is False,
            "qualification lock still held",
        )
    except BaseException as error:
        errors.append(("release_verification", error))
    if errors:
        labels = ", ".join(label for label, _ in errors)
        raise RuntimeError(
            "reservation release required fallback or failed: " + labels
        ) from errors[0][1]
    return state
...
def reconcile_reservation(
    ...
    *,
    ...
    releaser=release_reservation,
    transport_stopper=_stop_local_transport,
    global_verifier=None,
):
    """Reconcile a launched holder even if acquisition acknowledgement was lost."""
    identity_reader = identity_reader or read_reservation_identity
    global_verifier = global_verifier or read_qualification_lock_state
    if identity is None:
        read_errors = []
        for _ in range(3):
            try:
                identity = identity_reader(token)
                break
            except BaseException as error:
        ...
        if identity is None:
            cleanup_errors = []
            try:
                transport_stopper(holder)
            except BaseException as error:
                cleanup_errors.append(("transport_stop", error))
            try:
                state = global_verifier()
                require(
                    state.get("qualification_lock_held") is False,
                    "qualification lock still held without a validated identity",
                )
            except BaseException as error:
                cleanup_errors.append(("global_lock_verification", error))
            labels = ["reservation_identity"] + [
                label for label, _ in cleanup_errors
            ]
            raise RuntimeError(
                "reservation reconciliation incomplete: " + ", ".join(labels)
            ) from read_errors[0]
    validate_reservation_identity(identity, token)
    releaser(holder, identity)
    return identity
...
def deployment_snapshot():
    ...
    for expected in CANDIDATES:
        ...
        require(row["uid"] == row["expected_uid"], "Deployment UID changed")
        require(
            row["replicas"] == 0 and row["ready"] == 0,
            "paused service resumed",
        )
        result.append(row)
    return result
...
def assert_candidate_pods_absent():
    ...
    require(not live, "paused service Pods remain: " + repr(live))
...
"""
    ...
    require(
        value["healthy"] and not value["running"] and value["objects"] == 200,
        "T09a services are not idle and healthy",
    )
...
def frozen_precheck():
    ...
    bundle = json.loads((SOURCE_LOCAL_BUNDLE / "inputs.json").read_text())
    ...
    require(
        inventory["python_matches"] and inventory["platform_matches"],
        "Python or platform identity changed",
    )
    require(
        not inventory["package_differences"] and not inventory["model_differences"],
        "package or model inventory changed",
    )
    require(
        not inventory["processes"] and inventory["proposed_path_exists"],
        "unexpected runtime process or missing source state",
    )
    frozen_producer = SOURCE_REMOTE + "/code/src/pdf_processing"
    require(
        not inventory["producer_differences"][frozen_producer],
        "frozen producer changed",
    )
    require(inventory["retained_profile"]["method_matches"], "profile method changed")
    require(
        inventory["retained_profile"]["path"]
        == SOURCE_REMOTE + "/inputs/inputs.json",
        "retained profile path changed",
    )
...
""".replace("ROOT", repr(SOURCE_REMOTE)).replace(
    ...
    require(
        frozen["run_id"] == SOURCE_RUN_ID and frozen["prefix"] == SOURCE_PREFIX,
        "source run identity changed",
    )
    require(frozen["bundle"] == SOURCE_REMOTE + "/inputs", "source bundle path changed")
    validate_frozen_artifacts(frozen)
    validate_remote_baseline(frozen)
    for field in remote_absence_paths():
        require(frozen[field], field + " collision")
...
def copy_log_path(destination):
    identity = hashlib.sha256(destination.encode()).hexdigest()[:12]
    return OUT / (identity + "-" + Path(destination).name + "-copy.log")
...
def copy_to_remote(source, destination):
    ...
    subprocess.run(
        ...
        check=True,
        timeout=120,
        stdout=copy_log_path(destination).open("x"),
...
def stage_runtime_tree():
    ...
    remote(
        ...
        + "if not source.is_dir() or not target.is_dir():raise ValueError('staging root missing')\n"
        + "if (target/'code').exists() or (target/'inputs').exists():raise FileExistsError('staging collision')\n"
    ...
    candidate_files = candidate_staging_files()
    for relative, source in sorted(candidate_files.items()):
        destination = REMOTE + "/code/" + relative
        remote(
            "from pathlib import Path\nPath("
            + repr(str(Path(destination).parent))
            + ").mkdir(parents=True,exist_ok=True)\n"
        )
        copy_to_remote(source, destination)
...
""".replace("COORDINATOR_UID", repr(COORDINATOR_UID)).replace(
            ...
            require(holder.stdin is not None, "reservation stdin unavailable")
            holder.stdin.write(holder_script.encode())
            holder.stdin.close()
            for _ in range(30):
                require(holder.poll() is None, "reservation holder failed")
                exists = remote(
                    "from pathlib import Path\nprint(Path("
                    + repr(RESERVATION)
                    + ").exists())\n"
            ...
            def launch():
                STATE["phase"] = "running-candidate-matrix"
                save()
                ...
                return subprocess.Popen(
                    BASE + [
                        "-n", NS, "exec", "coordinator", "--", "sh", "-c",
                        build_runtime_command(initialized["run_id"]),
                    ],
            ...
            while driver.poll() is None:
                require(
                    holder is not None and holder.poll() is None,
                    "reservation transport lost",
                )
            ...
            require(
                driver.returncode == 0,
                "YOLO lifecycle candidate driver failed; retain logs, no retry",
            )
            ...
            if claimed:
                try:
                    ...
                    if runner_manifest is not None:
                        require(
                            hashlib.sha256(cleanup_raw_source).hexdigest()
                            == runner_manifest["cleanup.py"],
                            "cleanup source hash changed",
                        )
                    cleanup_raw = remote(
                ...
                if initialized is not None:
                    try:
                        ...
                        require(
                            post["state_sha256"] == initialized["state_sha256"],
                            "initialized state changed",
                        )
                        require(
                            post["source_state_sha256"] == SOURCE_STATE_SHA256,
            ...
            if holder is not None:
                try:
                    reservation_identity = reconcile_reservation(
                        holder,
                        reservation_token,
                    ...
                    STATE["errors"].append({"reservation-reconciliation": str(error)})
                    try:
                        _stop_local_transport(holder)
                    except BaseException as transport_error:
                        STATE["errors"].append(
                            {"reservation-transport-stop": str(transport_error)}
                        )
