"""Run AU once with an owned, read-only host-cgroup PSI observer."""
...
base, run_au, OUT, RUN_IDENTITY = runner.base, runner.main, runner.OUT, runner.RUN_IDENTITY
...
PROBE_NAME = "q04-host-pressure-au-20260925"
...
def start_probe():
    if any(path.exists() for path in (PROBE_OUT, PROBE_ERR, PROBE_CLEANUP, PROBE_CID, OUT)):
        raise ValueError("AU identity or host observer output already used")
...
def main(argv=None):
    ...
    if (not args.owner or not args.approval_reference
        ...
        raise ValueError("AU execution authorization scope changed")
    process = None
    identity = None
