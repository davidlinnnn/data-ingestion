...
import pod_topology_ar as pod_topology
from pod_remote_evidence_ar import (
    FINAL_REQUIRED,
...
PHASE = "relationship-pod-cgroup-ar"
RUN_IDENTITY = "q04-relationship-pod-cgroup-20260925-ar"
PREFIX = "q04/relationship-pod-cgroup-20260925-ar/"
OUT = Path("/private/tmp/q04-relationship-pod-cgroup-20260925-ar")
...
WORKER_YAML = base.Q04 / "pod-topology-v43/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v43/RUNNER-MANIFEST.json"
...
def authorization_scope() -> dict:
    return {
        ...
        "ocr_phase_trace": {
            ...
            "stages": ["start", "crop_ready", "engine_ready", "inference_ready", "result_written"],
        },
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_ar.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_ar.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_relationship_pod_cgroup_ar_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_ar.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_ar.py",
        "pre_inference": base.Q04 / "pod_preflight_ar.py",
        "topology_builder": base.Q04 / "pod_topology_ar.py",
        "matrix_window": base.Q04 / "candidate/relationship_interruption_window_ao.py",
        "source_manifest": base.Q04 / "pod-topology-v43/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v43/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution.py"),
        "node_pressure_guard": Path(__file__).with_name("run_relationship_pod_cgroup_ar_guarded.py"),
    }
