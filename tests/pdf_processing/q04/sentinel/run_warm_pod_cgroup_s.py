...
import pod_topology_s as pod_topology
from pod_remote_evidence_s import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with S topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_s_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "warm-pod-cgroup-s"
RUN_IDENTITY = "q04-warm-pod-cgroup-20260921-s"
PREFIX = "q04/warm-pod-cgroup-20260921-s/"
OUT = Path("/private/tmp/q04-warm-pod-cgroup-20260921-s")
...
WORKER_YAML = base.Q04 / "pod-topology-v18/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v18/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_s.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_s.py",
        "--workspace", "/workspace",
...
def adapt_execute_window():
    ...
    exec(compile(source, str(base.__file__) + ":s", "exec"), namespace)
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_s.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_s.py",
        "pre_inference": base.Q04 / "pod_preflight_s.py",
        "topology_builder": base.Q04 / "pod_topology_s.py",
        "warm_window": base.Q04 / "candidate/warm_pod_window_r.py",
        "source_manifest": base.Q04 / "pod-topology-v18/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v18/RUNTIME-INTEGRATION-MANIFEST.json",
    }
