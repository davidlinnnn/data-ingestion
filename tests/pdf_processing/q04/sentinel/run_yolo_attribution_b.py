"""25-minute YOLO fixture 07 fresh-only attribution runner."""
...
OFFLINE_MANIFEST = (
    REPO
    / "tests/pdf_processing/q04/preflight/yolo-attribution-b/OFFLINE-MANIFEST.json"
)
REMOTE = "/tmp/q04-yolo-attribution-20260918-b"
PREFIX = "q04/yolo-attribution-20260918-b/"
...
OUT = Path("/private/tmp/q04-yolo-attribution-20260918-b")
CAPACITY = REMOTE + "/capacity-yolo-attribution-b.json"
RESERVATION = REMOTE + "/reservation-yolo-attribution-b.json"
RELEASE = REMOTE + "/release-reservation-yolo-attribution-b"
ADMISSION_EVIDENCE = REMOTE + "/pre-admission-yolo-attribution-b.jsonl"
ADMISSION_RESULT = REMOTE + "/admission-yolo-attribution-b.json"
RUNNER_DIR = REMOTE + "/runner-yolo-attribution-b"
...
def build_measurement_argv(
    ...
    *,
    ...
    """Return the fixed argv for the fresh-only diagnostic driver."""
    if not expected_run_id.startswith("q04-"):
        raise ValueError("initialized run identity is required")
    return [
        python,
        RUNNER_DIR + "/yolo_fresh_measure.py",
    ...
    *,
    ...
    return "\n".join(
        (
            ...
            'export PYTHONPATH="$R/runner-yolo-attribution-b:$R/code/src:'
...
def staged_sources():
    return {
        ...
        "yolo_fresh_measure.py": REPO
        / "tests/pdf_processing/q04/sentinel/yolo_fresh_measure.py",
        "run_yolo_attribution_b.py": Path(__file__),
    }
...
def reviewed_source_hashes(sources=None, manifest_path=OFFLINE_MANIFEST):
    ...
    require(retained.get("modes") == ["fresh"], "offline manifest modes changed")
    require(
...
""".replace("COORDINATOR_UID", repr(COORDINATOR_UID)).replace(
            ...
            def launch():
                STATE["phase"] = "running-attribution"
                save()
            ...
            require(
                driver.returncode == 0,
                "YOLO attribution driver failed; retain logs, no retry",
            )
            STATE["measurement_passed"] = True
        except BaseException as error:
