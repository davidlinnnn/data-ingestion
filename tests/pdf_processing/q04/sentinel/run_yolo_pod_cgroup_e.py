...
import pod_topology_e as pod_topology
...
from pod_remote_evidence_e import (
    FINAL_REQUIRED,
...
PHASE = "yolo-pod-cgroup-e"
RUN_IDENTITY = "q04-yolo-pod-cgroup-20260920-e"
PREFIX = "q04/yolo-pod-cgroup-20260920-e/"
OUT = Path("/private/tmp/q04-yolo-pod-cgroup-20260920-e")
CONTROL = "/q04-control"
...
WORKER_YAML = Q04 / "pod-topology-v4/WORKER.yaml"
OFFLINE_MANIFEST = Q04 / "pod-topology-v4/RUNNER-MANIFEST.json"
LOCAL_PYTHON = Path(
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_e.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_e.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join(
        [
            ...
            "--owner", "main-session",
            "--approval-reference",
            "Separate explicit main-session authorization for q04-yolo-pod-cgroup-20260920-e",
            "--authorization-scope-sha256", authorization_scope_sha256(),
        ]
    )
...
def build_offline_manifest() -> dict:
    paths = {
        ...
        "pod_workload": Q04 / "pod_workload_e.py",
        "pod_init": Q04 / "pod_init_e.py",
        "remote_evidence": Q04 / "pod_remote_evidence_e.py",
        "durable_evidence": Q04 / "pod_durable_evidence.py",
        "evidence_directory": Q04 / "pod_evidence_directory_d.py",
        "pre_inference": Q04 / "pod_preflight_e.py",
        "topology_builder": Q04 / "pod_topology_e.py",
        "worker_yaml": WORKER_YAML,
        "source_manifest": Q04 / "pod-topology-v4/SOURCE-MANIFEST.json",
        "run_plan": Q04 / "pod-topology-v4/RUN-PLAN.md",
        "readme": Q04 / "pod-topology-v4/README.md",
        "pre_inference_gates": Q04 / "pod-topology-v4/PRE-INFERENCE-GATES.md",
        "storage_reconciliation": Q04 / "pod-topology-v4/STORAGE-RECONCILIATION.json",
        "readonly_preflight_reconciliation": Q04 / "pod-topology-v3/READONLY-PREFLIGHT-RECONCILIATION.json",
        "model_artifact_resolution": Q04 / "pod-topology-v4/MODEL-ARTIFACT-RESOLUTION.md",
        "offline_validation": Q04 / "pod-topology-v4/OFFLINE-VALIDATION.md",
    ...
    return {
        ...
        "runtime_blocker": (
            "main review of the E scope and one separate authorization; "
            "the fresh E mount identity remains a runtime gate"
        ),
...
evidence=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e');used=sum(p.stat().st_size for p in evidence.rglob('*') if p.is_file())
...
from pathlib import Path
path=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e/supervisor-ownership.json')
...
def terminal_cleanup_program() -> str:
    return """import json
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e');owner=json.loads((root/'supervisor-ownership.json').read_text())
...
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e');owner=json.loads((root/'supervisor-ownership.json').read_text());pid=owner['pid']
...
else: scratches=[path/'scratch' for path in (root/'state/yolo-pod-cgroup-e').glob('worker-*')]
...
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e');rows=[]
...
def execute_window(args, kube=None) -> dict:
    ...
    try:
        ...
        stage_volume_identity = (
            ...
            "write_once(Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e/evidence-volume-identity.json'),json.loads(sys.stdin.read()))"
        )
        kube.run(
                ...
                try:
                    owner = json.loads(
                        kube.exec_python(
                            ...
                            "s=json.loads(Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e/supervisor-ownership.json').read_text());"
                            "w=json.loads(Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e/ownership.json').read_text());"
                    ...
                    kube.run(
                        ["exec", "-i", pod, "--", "/experiment/.venv/bin/python", "-c",
                         "from pathlib import Path;import sys;Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-e/transport-identity.json').open('x').write(sys.stdin.read())"],
