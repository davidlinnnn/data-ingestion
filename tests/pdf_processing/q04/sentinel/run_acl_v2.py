"""Versioned 25-minute ACL fixture 09 runner with outer admission v2."""
...
PHASE = "acl-window-v2-a"
OUT = Path("/private/tmp/q04-acl-window-20260917-v2-a")
CAPACITY = REMOTE + "/capacity-acl-window-v2-a.json"
RESERVATION = REMOTE + "/reservation-acl-window-v2-a.json"
RELEASE = REMOTE + "/release-reservation-acl-window-v2-a"
ADMISSION_EVIDENCE = REMOTE + "/pre-admission-acl-window-v2-a.jsonl"
ADMISSION_RESULT = REMOTE + "/admission-acl-window-v2-a.json"
RUNNER_DIR = REMOTE + "/runner-acl-window-v2-a"
...
export PDF_QUALIFICATION_LOCK="$R/acl-window-v2-a.driver.lock"
cd "$R/code"
test ! -e "$R/state/acl-window-v2-a"
...
exec /experiment/.venv/bin/python tests/pdf_processing/q04/q04_runtime.py --phase matrix --fixture 09 --bundle "$R/inputs" --state "$R/state" --capacity CAPACITY --name acl-window-v2-a --trial-seconds 180 --capacity-approved
' > "$R/logs/acl-window-v2-a.log" 2>&1
...
""".replace("COORDINATOR_UID", repr(COORDINATOR_UID)).replace(
        ...
        try:
            ...
            def admit():
                ...
                (OUT / "admission-v2.json").write_text(
