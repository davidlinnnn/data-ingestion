"""Post-Docker-restart 25-minute ACL fixture 09 runner with outer admission v3."""
...
PHASE = "acl-window-v3-a"
OUT = Path("/private/tmp/q04-acl-window-20260917-v3-a")
CAPACITY = REMOTE + "/capacity-acl-window-v3-a.json"
RESERVATION = REMOTE + "/reservation-acl-window-v3-a.json"
RELEASE = REMOTE + "/release-reservation-acl-window-v3-a"
ADMISSION_EVIDENCE = REMOTE + "/pre-admission-acl-window-v3-a.jsonl"
ADMISSION_RESULT = REMOTE + "/admission-acl-window-v3-a.json"
RUNNER_DIR = REMOTE + "/runner-acl-window-v3-a"
...
def build_runtime_command():
    return """set -euC
export R=ROOT
export PYTHONDONTWRITEBYTECODE=1 PYTHONSAFEPATH=1 HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 OMP_NUM_THREADS=4
export PYTHONPATH="$R/runner-acl-window-v3-a:$R/code/src:$R/code/tests/pdf_processing/q04:$R/code/tests/pdf_processing/q02:$R/code/tests/pdf_processing/q03"
export PDF_QUALIFICATION_LOCK="$R/acl-window-v3-a.driver.lock"
cd "$R/code"
test ! -e "$R/state/acl-window-v3-a"
timeout --signal=INT --kill-after=180s 825s sh -eu -c '
/experiment/.venv/bin/python -c "from prepare import verify_bundle;from pathlib import Path;verify_bundle(Path(\"$R/inputs\"))"
exec /experiment/.venv/bin/python tests/pdf_processing/q04/q04_runtime.py --phase matrix --fixture 09 --bundle "$R/inputs" --state "$R/state" --capacity CAPACITY --name acl-window-v3-a --trial-seconds 180 --capacity-approved
' > "$R/logs/acl-window-v3-a.log" 2>&1
