"""Run one v3-bound Q04 warm sequence in the reviewed Pod cgroup."""
...
import pod_topology_ak as pod_topology
from pod_remote_evidence_ak import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with AK topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_ak_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "profile-pod-cgroup-ak"
RUN_IDENTITY = "q04-profile-pod-cgroup-20260924-ak"
PREFIX = "q04/profile-pod-cgroup-20260924-ak/"
OUT = Path("/private/tmp/q04-profile-pod-cgroup-20260924-ak")
...
WORKER_YAML = base.Q04 / "pod-topology-v36/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v36/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_ak.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_ak.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_ak.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_ak.py",
        "pre_inference": base.Q04 / "pod_preflight_ak.py",
        "topology_builder": base.Q04 / "pod_topology_ak.py",
        "matrix_window": base.Q04 / "candidate/profile_rejection_window_ak.py",
        "source_manifest": base.Q04 / "pod-topology-v36/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v36/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete AK profile-rejection runtime gates"
    return value
