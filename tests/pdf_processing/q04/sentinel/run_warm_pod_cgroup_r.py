"""Run one authorized original Q04 warm sequence in the reviewed Pod cgroup."""
...
import pod_topology_r as pod_topology
from pod_remote_evidence_r import (
    FINAL_REQUIRED,
...
_spec = importlib.util.spec_from_file_location(
    "q04_warm_r_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
)
...
PHASE = "warm-pod-cgroup-r"
RUN_IDENTITY = "q04-warm-pod-cgroup-20260921-r"
PREFIX = "q04/warm-pod-cgroup-20260921-r/"
OUT = Path("/private/tmp/q04-warm-pod-cgroup-20260921-r")
...
WORKER_YAML = base.Q04 / "pod-topology-v17/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v17/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_r.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_r.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        "--execute",
        "--owner", "main-session",
        "--approval-reference", "User authorized Q04 warm execution on 2026-09-21",
        "--authorization-scope-sha256", authorization_scope_sha256(),
    ])
...
def adapt_execute_window():
    ...
    exec(
        compile(source.replace(P_EVIDENCE, EVIDENCE), str(base.__file__) + ":r", "exec"),
        namespace,
    )
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_r.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_r.py",
        "pre_inference": base.Q04 / "pod_preflight_r.py",
        "topology_builder": base.Q04 / "pod_topology_r.py",
        "warm_window": base.Q04 / "candidate/warm_pod_window_r.py",
        "source_manifest": base.Q04 / "pod-topology-v17/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v17/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete R warm runtime gates"
    return value
...
base.stop_owned_supervisor_program = lambda seconds: _program(_stop_owned_supervisor_program, seconds)
