"""Run AY once with owned node and object-service cgroup telemetry."""
...
from sentinel import run_process_drain_pod_cgroup_ay as runner

base, run_ay, OUT, RUN_IDENTITY = runner.base, runner.main, runner.OUT, runner.RUN_IDENTITY


PROBE = Path(__file__).with_name("node_pressure_attribution_ay.py")
...
PROBE_NAME = "q04-host-pressure-ay-20260925"
...
OBJECT_POD = "objects-8865c4db6-bqmvr"
OBJECT_POD_UID = "ee7b24cb-bdb7-408a-9ec8-093d2ab5ac00"
...
def start_probe():
    if any(path.exists() for path in (PROBE_OUT, PROBE_ERR, PROBE_CLEANUP, PROBE_CID, OUT)):
        raise ValueError("AY identity or host observer output already used")
    verify_object_pod()
        ...
        while time.monotonic() < deadline:
            ...
            if lines:
                ...
                if (identity.get("kind") != "start" or identity.get("run_id") != RUN_IDENTITY
                        ...
                        or identity["target"]["memory_max"] != 536870912):
...
def main(argv=None):
    ...
    if (not args.owner or not args.approval_reference
        ...
        raise ValueError("AY execution authorization scope changed")
    process = None
    identity = None
