...
import pod_topology_t as pod_topology
from pod_remote_evidence_t import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with T topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_t_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "warm-pod-cgroup-t"
RUN_IDENTITY = "q04-warm-pod-cgroup-20260921-t"
PREFIX = "q04/warm-pod-cgroup-20260921-t/"
OUT = Path("/private/tmp/q04-warm-pod-cgroup-20260921-t")
...
WORKER_YAML = base.Q04 / "pod-topology-v19/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v19/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_t.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_t.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        "--execute",
        "--owner", "main-session",
        "--approval-reference", "REQUIRES_NEW_EXPLICIT_AUTHORIZATION_AFTER_S_FAIL_STOP",
        "--authorization-scope-sha256", authorization_scope_sha256(),
    ])
...
def adapt_execute_window():
    ...
    exec(compile(source, str(base.__file__) + ":t", "exec"), namespace)
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_t.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_t.py",
        "pre_inference": base.Q04 / "pod_preflight_t.py",
        "topology_builder": base.Q04 / "pod_topology_t.py",
        "warm_window": base.Q04 / "candidate/warm_pod_window_r.py",
        "source_manifest": base.Q04 / "pod-topology-v19/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v19/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "new explicit authorization after S fail-stop"
    return value
