"""25-minute ACL fixture 09 Option A adoption runner with isolated live init."""
...
REMOTE = "/tmp/q04-option-a-20260918-b"
PREFIX = "q04/option-a-20260918-b/"
SOURCE_REMOTE = "/tmp/q04-keynote-18be1b3-20260916-b"
SOURCE_PREFIX = "q04/keynote-18be1b3-20260916-b/"
...
OUT = Path("/private/tmp/q04-acl-option-a-window-20260918-b")
CAPACITY = REMOTE + "/capacity-acl-option-a-window-b.json"
RESERVATION = REMOTE + "/reservation-acl-option-a-window-b.json"
RELEASE = REMOTE + "/release-reservation-acl-option-a-window-b"
ADMISSION_EVIDENCE = REMOTE + "/pre-admission-acl-option-a-window-b.jsonl"
ADMISSION_RESULT = REMOTE + "/admission-acl-option-a-window-b.json"
RUNNER_DIR = REMOTE + "/runner-acl-option-a-window-b"
...
SOURCE_STATE_SHA256 = "6f83b29e73357e6576643948f18f0be2d461ae7285e433f2c810c5f8d4aca5b7"
SOURCE_RUN_ID = "q04-7b4f958ff3ac4e2da5b54dcaa66914b9"
...
def validate_frozen_artifacts(frozen):
    """Pin the preserved original bundle/state without adopting it in place."""
    if (
    ...
    *,
    ...
    inner = "\n".join(
        (
            ...
            + " --name acl-option-a-window-b --trial-seconds 180 --capacity-approved",
        )
    )
    return "\n".join(
        (
            ...
            'export PYTHONPATH="$R/runner-acl-option-a-window-b:$R/code/src:'
...
def staged_sources():
    return {
        ...
        "run_acl_option_a.py": Path(__file__),
    }
...
print(json.dumps({'bundle_sha256':sha(root/'inputs/inputs.json'),
 ...
 'reference_09_sha256':sha(root/'inputs/references/09.json'),
 'quality_oracle_sha256':sha(root/'inputs/oracles/quality-oracle.json'),
 'content_evidence_sha256':sha(root/'inputs/content-evidence/09.json'),
...
""".replace("ROOT", repr(REMOTE))
    ...
    assert staged["bundle_sha256"] == EXPECTED_BUNDLE_SHA256
    assert staged["method"] == local_bundle["base_profile"]["method"]
    assert staged["producer"] == local_bundle["producer"]
...
print(json.dumps({'run_id':config['run_id'],'prefix':config['prefix'],
 ...
 'profile_method':config['profiles']['09']['method']}))
""".replace("ROOT", repr(REMOTE))
    initialized = json.loads(remote(inspect))
    bundle = json.loads((LOCAL_BUNDLE / "inputs.json").read_text())
    assert initialized["run_id"].startswith("q04-")
    assert initialized["run_id"] != SOURCE_RUN_ID
    assert initialized["prefix"] == PREFIX
    assert initialized["bundle"] == REMOTE + "/inputs"
    assert initialized["bundle_sha256"] == EXPECTED_BUNDLE_SHA256
    assert initialized["producer"] == bundle["producer"]
    assert initialized["profile_method"] == bundle["base_profile"]["method"]
