...
import pod_topology_ay as pod_topology
from pod_remote_evidence_ay import (
    FINAL_REQUIRED,
...
PHASE = "process-drain-pod-cgroup-ay"
RUN_IDENTITY = "q04-process-drain-pod-cgroup-20260925-ay"
PREFIX = "q04/process-drain-pod-cgroup-20260925-ay/"
OUT = Path("/private/tmp/q04-process-drain-pod-cgroup-20260925-ay")
...
WORKER_YAML = base.Q04 / "pod-topology-v50/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v50/RUNNER-MANIFEST.json"
...
def authorization_scope() -> dict:
    return {
        ...
        "object_service_observer": {
            "read_only": True,
            "pod_uid": "ee7b24cb-bdb7-408a-9ec8-093d2ab5ac00",
            "sample_interval_seconds": 0.25,
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_ay.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_ay.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_process_drain_pod_cgroup_ay_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_ay.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_ay.py",
        "pre_inference": base.Q04 / "pod_preflight_ay.py",
        "topology_builder": base.Q04 / "pod_topology_ay.py",
        "matrix_window": base.Q04 / "candidate/process_drain_window_ay.py",
        "source_manifest": base.Q04 / "pod-topology-v50/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v50/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution_ay.py"),
        "node_pressure_guard": Path(__file__).with_name("run_process_drain_pod_cgroup_ay_guarded.py"),
    }
