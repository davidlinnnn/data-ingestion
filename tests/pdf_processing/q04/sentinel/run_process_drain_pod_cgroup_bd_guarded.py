"""Run BD once with owned node and object-service cgroup telemetry."""
...
base, run_bd, OUT, RUN_IDENTITY = runner.base, runner.main, runner.OUT, runner.RUN_IDENTITY


PROBE = Path(__file__).with_name("node_pressure_attribution_bd.py")
...
PROBE_NAME = "q04-host-pressure-bd-20260925"
...
def start_probe():
    if any(path.exists() for path in (PROBE_OUT, PROBE_ERR, PROBE_CLEANUP, PROBE_CID, OUT)):
        raise ValueError("BD identity or host observer output already used")
    verify_object_pod()
...
def main(argv=None):
    ...
    if (not args.owner or not args.approval_reference
        ...
        raise ValueError("BD execution authorization scope changed")
    process = None
    identity = None
