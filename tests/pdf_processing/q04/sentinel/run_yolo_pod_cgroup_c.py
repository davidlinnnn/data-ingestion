...
import pod_topology_c as pod_topology
...
from pod_remote_evidence_c import (
    FINAL_REQUIRED,
...
RUN_IDENTITY = "q04-yolo-pod-cgroup-20260919-c"
PREFIX = "q04/yolo-pod-cgroup-20260919-c/"
OUT = Path("/private/tmp/q04-yolo-pod-cgroup-20260919-c")
CONTROL = "/q04-control"
...
WORKER_YAML = Q04 / "pod-topology-v2/WORKER.yaml"
OFFLINE_MANIFEST = Q04 / "pod-topology-v2/RUNNER-MANIFEST.json"
LOCAL_PYTHON = Path(
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_c.py",
        "--prefix", PREFIX,
...
def exact_command() -> str:
    return shlex.join(
        [
            ...
            "--owner", "main-session",
            "--approval-reference",
            "Separate explicit main-session authorization for q04-yolo-pod-cgroup-20260919-c",
            "--authorization-scope-sha256", authorization_scope_sha256(),
        ]
    )
...
def build_offline_manifest() -> dict:
    paths = {
        ...
        "pod_workload": Q04 / "pod_workload_c.py",
        "pod_init": Q04 / "pod_init_c.py",
        "remote_evidence": Q04 / "pod_remote_evidence_c.py",
        "durable_evidence": Q04 / "pod_durable_evidence.py",
        "topology_builder": Q04 / "pod_topology_c.py",
        "worker_yaml": WORKER_YAML,
        "source_manifest": Q04 / "pod-topology-v2/SOURCE-MANIFEST.json",
        "run_plan": Q04 / "pod-topology-v2/RUN-PLAN.md",
        "readme": Q04 / "pod-topology-v2/README.md",
    ...
    return {
        ...
        "runtime_blocker": "one explicit main-session authorization for the reviewed scope digest",
...
else: scratches=[path/'scratch' for path in (root/'state/yolo-pod-cgroup-c').glob('worker-*')]
...
def execute_window(args, kube=None) -> dict:
    ...
    try:
        ...
        preflight = kube.exec_python(
            ...
            "from pod_workload_c import no_inference_preflight;"
