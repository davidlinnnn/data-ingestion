...
import pod_topology_ax as pod_topology
from pod_remote_evidence_ax import (
    FINAL_REQUIRED,
...
PHASE = "process-drain-pod-cgroup-ax"
RUN_IDENTITY = "q04-process-drain-pod-cgroup-20260925-ax"
PREFIX = "q04/process-drain-pod-cgroup-20260925-ax/"
OUT = Path("/private/tmp/q04-process-drain-pod-cgroup-20260925-ax")
...
WORKER_YAML = base.Q04 / "pod-topology-v49/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v49/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_ax.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_ax.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_process_drain_pod_cgroup_ax_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_ax.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_ax.py",
        "pre_inference": base.Q04 / "pod_preflight_ax.py",
        "topology_builder": base.Q04 / "pod_topology_ax.py",
        "matrix_window": base.Q04 / "candidate/process_drain_window_ax.py",
        "source_manifest": base.Q04 / "pod-topology-v49/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v49/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution.py"),
        "node_pressure_guard": Path(__file__).with_name("run_process_drain_pod_cgroup_ax_guarded.py"),
    }
