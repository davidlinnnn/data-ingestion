...
import pod_topology_av as pod_topology
from pod_remote_evidence_av import (
    FINAL_REQUIRED,
...
PHASE = "process-drain-pod-cgroup-av"
RUN_IDENTITY = "q04-process-drain-pod-cgroup-20260925-av"
PREFIX = "q04/process-drain-pod-cgroup-20260925-av/"
OUT = Path("/private/tmp/q04-process-drain-pod-cgroup-20260925-av")
...
WORKER_YAML = base.Q04 / "pod-topology-v47/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v47/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_av.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_av.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_process_drain_pod_cgroup_av_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_av.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_av.py",
        "pre_inference": base.Q04 / "pod_preflight_av.py",
        "topology_builder": base.Q04 / "pod_topology_av.py",
        "matrix_window": base.Q04 / "candidate/process_drain_window_av.py",
        "source_manifest": base.Q04 / "pod-topology-v47/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v47/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution.py"),
        "node_pressure_guard": Path(__file__).with_name("run_process_drain_pod_cgroup_av_guarded.py"),
    }
