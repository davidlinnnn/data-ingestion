"""Run one authorized fixture-08 matrix in the reviewed worker Pod cgroup.
...
import pod_topology_p as pod_topology
...
from pod_remote_evidence_p import (
    FINAL_REQUIRED,
...
PHASE = "aima-pod-cgroup-p"
RUN_IDENTITY = "q04-aima-pod-cgroup-20260920-p"
PREFIX = "q04/aima-pod-cgroup-20260920-p/"
OUT = Path("/private/tmp/q04-aima-pod-cgroup-20260920-p")
CONTROL = "/q04-control"
...
WORKER_YAML = Q04 / "pod-topology-v15/WORKER.yaml"
OFFLINE_MANIFEST = Q04 / "pod-topology-v15/RUNNER-MANIFEST.json"
LOCAL_PYTHON = Path(
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_p.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_p.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join(
        [
            ...
            "--owner", "main-session",
            "--approval-reference",
            "User authorized primary agent to fully handle subsequent Q04 acceptance on 2026-09-20",
            "--authorization-scope-sha256", authorization_scope_sha256(),
        ]
    )
...
def build_offline_manifest() -> dict:
    paths = {
        ...
        "persistent_python": Q04 / "pod_persistent_python.py",
        "pod_workload": Q04 / "pod_workload_p.py",
        "pod_init": Q04 / "pod_init_p.py",
        "remote_evidence": Q04 / "pod_remote_evidence_p.py",
        "durable_evidence": Q04 / "pod_durable_evidence.py",
        "evidence_directory": Q04 / "pod_evidence_directory_d.py",
        "pre_inference": Q04 / "pod_preflight_p.py",
        "topology_builder": Q04 / "pod_topology_p.py",
        "worker_yaml": WORKER_YAML,
        "source_manifest": Q04 / "pod-topology-v15/SOURCE-MANIFEST.json",
        "run_plan": Q04 / "pod-topology-v15/RUN-PLAN.md",
        "readme": Q04 / "pod-topology-v15/README.md",
        "pre_inference_gates": Q04 / "pod-topology-v7/PRE-INFERENCE-GATES.md",
        "storage_reconciliation": Q04 / "pod-topology-v7/STORAGE-RECONCILIATION.json",
        "readonly_preflight_reconciliation": Q04 / "pod-topology-v3/READONLY-PREFLIGHT-RECONCILIATION.json",
        "model_artifact_resolution": Q04 / "pod-topology-v7/MODEL-ARTIFACT-RESOLUTION.md",
        "offline_validation": Q04 / "pod-topology-v15/OFFLINE-VALIDATION.md",
        "historical_integration_manifest": Q04 / "pod-topology-v1/INTEGRATION-MANIFEST.json",
        "runtime_integration_manifest": Q04 / "pod-topology-v15/RUNTIME-INTEGRATION-MANIFEST.json",
    ...
    return {
        ...
        "runtime_blocker": (
            "live admission, new J mount identity and the complete runtime "
            "matrix remain runtime gates"
        ),
...
node_pressure=Path('/proc/pressure/memory').read_text()
cgroup_pressure=Path('/sys/fs/cgroup/memory.pressure').read_text()
full=next(x for x in node_pressure.splitlines() if x.startswith('full '))
pressure=dict(x.split('=') for x in full.split()[1:])
if not Path('/q04-control').is_dir() or not os.access('/q04-evidence',os.W_OK): raise ValueError('runtime mount readiness changed')
evidence=Path('/q04-evidence/q04-aima-pod-cgroup-20260920-p');used=sum(p.stat().st_size for p in evidence.rglob('*') if p.is_file())
...
print(json.dumps({'time':time.time(),'available':fields('/proc/meminfo')['MemAvailable']*1024,
 ...
 'sample_started_at':sample_started_at,'node_memory_pressure':node_pressure,'cgroup_memory_pressure':cgroup_pressure,
 'memory_stat':fields('/sys/fs/cgroup/memory.stat'),
 'node_vmstat':{k:v for k,v in fields('/proc/vmstat').items() if k.startswith(('compact_','allocstall','thp_','pgscan_','pgsteal_'))},
...
from pathlib import Path
path=Path('/q04-evidence/q04-aima-pod-cgroup-20260920-p/supervisor-ownership.json')
...
print(json.dumps({'identity_published':True,'pid':pid,'start_ticks':ticks,'signal_sent_at':signal_sent_at,
...
def terminal_cleanup_program() -> str:
    return """import json
from pathlib import Path
root=Path('/q04-evidence/q04-aima-pod-cgroup-20260920-p');owner=json.loads((root/'supervisor-ownership.json').read_text())
...
from pathlib import Path
root=Path('/q04-evidence/q04-aima-pod-cgroup-20260920-p');owner=json.loads((root/'supervisor-ownership.json').read_text());pid=owner['pid']
...
else: scratches=[path/'scratch' for path in (root/'state/aima-pod-cgroup-p').glob('worker-*')]
...
from pathlib import Path
root=Path('/q04-evidence/q04-aima-pod-cgroup-20260920-p');rows=[]
...
def execute_window(args, kube=None) -> dict:
    ...
    try:
        ...
        stage_volume_identity = (
            ...
            "write_once(Path('/q04-evidence/q04-aima-pod-cgroup-20260920-p/evidence-volume-identity.json'),json.loads(sys.stdin.read()))"
        )
        kube.run(
        ...
        stage_source_manifest = (
            "from pathlib import Path;import sys;"
            "Path('/q04-control/source-manifest.json').open('xb').write(sys.stdin.buffer.read())"
        )
        kube.run(
            [
        ...
        stage_pod_identity = (
            "from pathlib import Path;import sys;"
            "Path('/q04-control/pod-identity.json').open('xb').write(sys.stdin.buffer.read())"
        )
        kube.run(
            [
                "exec", "-i", pod, "--", "/experiment/.venv/bin/python", "-c",
        ...
        # Both helper processes exist before attribution starts. Their memory
        # remains in the same measured cgroup; no process is exempted.
        command = kube.base + ['exec', '-i', pod, '--', '/experiment/.venv/bin/python']
        sample_channel = PersistentPython(command)
        evidence_channel = PersistentPython(command)
        identity_program = "import os,json;from pathlib import Path;r=Path('/proc/self/stat').read_text();print(json.dumps({'pid':os.getpid(),'start_ticks':int(r[r.rfind(')')+1:].split()[19])}))"
        (OUT / "persistent-channels.json").write_text(json.dumps({
            "started_before_workload": True, "time": time.time(),
            "sample": json.loads(sample_channel.run(identity_program)),
            "evidence": json.loads(evidence_channel.run(identity_program)),
        }, indent=2) + "\n")
        ...
        while workload.poll() is None:
            row = json.loads(sample_channel.run(
                sample_program(),
            ...
            if mirror is None:
                try:
                    ...
                    if any(owner.get(key) is None for key in ("pid", "start_ticks", "config_sha256")):
                        raise FileNotFoundError("ownership not yet published")
                    ...
                    sample_channel.run(
                        "from pathlib import Path;Path(" + repr(EVIDENCE + '/transport-identity.json')
                        + ").open('x').write(" + repr(json.dumps(identity.__dict__, sort_keys=True)) + ")"
                    )
            ...
            if mirror is not None and transport_future is None and time.monotonic() >= next_transport:
                transport_future = transport_pool.submit(
                    ...
                    lambda program: evidence_channel.run(program),
                    now=time.monotonic,
                )
        ...
        with (OUT / "pod-control-evidence.tar").open("xb") as output, (OUT / "evidence-tar.log").open("x") as errors:
            subprocess.run(
        ...
        if (
            ...
            try:
                ...
                terminal_stop_proven = True
                if mirror is None:
                    raise ValueError("failure export lacks a validated transport identity")
                cleanup["live_qualification_passed"] = False
                (OUT / "live-transport-ledger.json").write_text(json.dumps({
                    "records": mirror.records, "offsets": mirror.offsets,
                    "last_received_at": mirror.last_received_at,
                    "qualification": "FAILED", "primary_error": repr(primary_error),
                }, indent=2) + "\n")
                forensic = IncrementalEvidenceMirror(
                    OUT / "failure-evidence", mirror.identity,
                    maximum_transport_gap_seconds=TRANSPORT_GAP_SECONDS,
                )
                # This is a new post-stop export, never a reset of live coverage.
                while (
                    "durable-terminal-manifest.json" not in forensic.complete
                    or set(forensic.remote_sizes) != forensic.complete
                ):
                    remaining_timeout(recovery_deadline, 30, "sealed failure evidence drain")
                    pull_once(
                        forensic, EVIDENCE,
                        lambda program: kube.exec_python(
                            pod, program,
                            timeout=remaining_timeout(
                                recovery_deadline, 30, "sealed failure evidence drain"
                            ),
                        ),
                        now=time.monotonic,
                    )
                failure_result = forensic.finalize(require_success=False)
                verify_terminal_volume_identity(
                    OUT / "failure-evidence/durable-terminal-manifest.json", evidence_volume
                )
                (OUT / "forensic-transport-ledger.json").write_text(json.dumps({
                    "records": forensic.records, "result": failure_result,
                ...
                forensic.verify_archive_fingerprint(json.loads(before_archive))
                with (OUT / "pod-control-failure-evidence.tar").open("xb") as output, (OUT / "failure-evidence-tar.log").open("x") as errors:
                    subprocess.run(
                        kube.base + ["exec", pod, "--", "tar", "cf", "-", "-C", EVIDENCE, "."],
                        check=True,
                        timeout=remaining_timeout(
                ...
                cleanup["forensic_export_complete"] = True
                evidence_captured = True
