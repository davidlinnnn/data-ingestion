...
import pod_topology_ae as pod_topology
from pod_remote_evidence_ae import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with AE topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_ae_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "warm-pod-cgroup-ae"
RUN_IDENTITY = "q04-warm-pod-cgroup-20260922-ae"
PREFIX = "q04/warm-pod-cgroup-20260922-ae/"
OUT = Path("/private/tmp/q04-warm-pod-cgroup-20260922-ae")
EVIDENCE_DIRECTORY_NAME = pod_topology.EVIDENCE_DIRECTORY_NAME
EVIDENCE = "/q04-evidence/" + EVIDENCE_DIRECTORY_NAME
BUNDLE = Path("/private/tmp/q04-inputs-warm-lifecycle-ae-final")
WORKER_YAML = base.Q04 / "pod-topology-v30/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v30/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_ae.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_ae.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_ae.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_ae.py",
        "pre_inference": base.Q04 / "pod_preflight_ae.py",
        "topology_builder": base.Q04 / "pod_topology_ae.py",
        "warm_window": base.Q04 / "candidate/warm_pod_window_r.py",
        "source_manifest": base.Q04 / "pod-topology-v30/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v30/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete AE warm runtime gates"
    return value
