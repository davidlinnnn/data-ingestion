...
import pod_topology_bd as pod_topology
from pod_remote_evidence_bd import (
    FINAL_REQUIRED,
...
PHASE = "process-drain-pod-cgroup-bd"
RUN_IDENTITY = "q04-process-drain-pod-cgroup-20260925-bd"
PREFIX = "q04/process-drain-pod-cgroup-20260925-bd/"
OUT = Path("/private/tmp/q04-process-drain-pod-cgroup-20260925-bd")
...
WORKER_YAML = base.Q04 / "pod-topology-v55/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v55/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_bd.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_bd.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_process_drain_pod_cgroup_bd_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_bd.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_bd.py",
        "pre_inference": base.Q04 / "pod_preflight_bd.py",
        "topology_builder": base.Q04 / "pod_topology_bd.py",
        "matrix_window": base.Q04 / "candidate/process_drain_window_bd.py",
        "source_manifest": base.Q04 / "pod-topology-v55/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v55/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution_bd.py"),
        "node_pressure_guard": Path(__file__).with_name("run_process_drain_pod_cgroup_bd_guarded.py"),
    }
