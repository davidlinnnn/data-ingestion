...
import pod_topology_y as pod_topology
from pod_remote_evidence_y import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with Y topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_y_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "warm-pod-cgroup-y"
RUN_IDENTITY = "q04-warm-pod-cgroup-20260921-y"
PREFIX = "q04/warm-pod-cgroup-20260921-y/"
OUT = Path("/private/tmp/q04-warm-pod-cgroup-20260921-y")
...
WORKER_YAML = base.Q04 / "pod-topology-v24/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v24/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_y.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_y.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_y.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_y.py",
        "pre_inference": base.Q04 / "pod_preflight_y.py",
        "topology_builder": base.Q04 / "pod_topology_y.py",
        "warm_window": base.Q04 / "candidate/warm_pod_window_r.py",
        "source_manifest": base.Q04 / "pod-topology-v24/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v24/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete Y warm runtime gates"
    return value
