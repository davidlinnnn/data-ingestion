...
import pod_topology_az as pod_topology
from pod_remote_evidence_az import (
    FINAL_REQUIRED,
...
PHASE = "process-drain-pod-cgroup-az"
RUN_IDENTITY = "q04-process-drain-pod-cgroup-20260925-az"
PREFIX = "q04/process-drain-pod-cgroup-20260925-az/"
OUT = Path("/private/tmp/q04-process-drain-pod-cgroup-20260925-az")
...
WORKER_YAML = base.Q04 / "pod-topology-v51/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v51/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_az.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_az.py",
        "--workspace", "/workspace",
        "--mount-root", base.EVIDENCE_MOUNT,
        "--evidence-directory-name", EVIDENCE_DIRECTORY_NAME,
        "--model-cache", "/experiment/PROTOTYPE-wipe-me/hf",
        "--provenance",
        "/workspace/tests/pdf_processing/q04/candidate/yolo-reviewed-b-fail/RETAINED-REPLAZ.json",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_process_drain_pod_cgroup_az_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_az.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_az.py",
        "pre_inference": base.Q04 / "pod_preflight_az.py",
        "topology_builder": base.Q04 / "pod_topology_az.py",
        "matrix_window": base.Q04 / "candidate/process_drain_window_az.py",
        "source_manifest": base.Q04 / "pod-topology-v51/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v51/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution_az.py"),
        "node_pressure_guard": Path(__file__).with_name("run_process_drain_pod_cgroup_az_guarded.py"),
    }
