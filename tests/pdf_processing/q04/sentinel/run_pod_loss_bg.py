...
from sentinel.object_monitor_bg import ObjectMonitor
from sentinel import object_limit_trial_bg as object_trial
from sentinel.pod_loss_mailbox_controller_bg import MailboxController
from telemetry import check_sample


PHASE = 'pod-loss-pod-cgroup-bg'
RUN_ID = 'q04-pod-loss-pod-cgroup-20260925-bg'
PREFIX = 'q04/pod-loss-pod-cgroup-20260925-bg/'
OUT = Path('/private/tmp/q04-pod-loss-pod-cgroup-20260925-bg')
EVIDENCE = '/q04-evidence/' + topology.EVIDENCE_DIRECTORY_NAME
CONTROL = '/q04-control'
BUNDLE = Path('/private/tmp/q04-inputs-warm-continuation-v3-ah')
V58 = Q04 / 'pod-topology-v58'
RUNNER_MANIFEST = V58 / 'RUNNER-MANIFEST.json'
...
def build_runner_manifest() -> dict:
    paths = {
        ...
        'mailbox_controller': Path(__file__).with_name('pod_loss_mailbox_controller_bg.py'),
        'pod_transition': Path(__file__).with_name('pod_loss_transition_bg.py'),
        'object_monitor': Path(__file__).with_name('object_monitor_bg.py'),
        'object_probe': Path(__file__).with_name('object_cgroup_probe_bg.py'),
        'object_limit_trial': Path(__file__).with_name('object_limit_trial_bg.py'),
        'activity_supervisor': Q04 / 'pod_activity_supervisor_bg.py',
        'coordinator_supervisor': Q04 / 'pod_workload_bg.py',
        'coordinator_bridge': Q04 / 'pod_loss_bridge_bg.py',
        'candidate': Q04 / 'candidate/pod_loss_window_bg.py',
        'preflight': Q04 / 'pod_preflight_bg.py',
        'topology_builder': Q04 / 'pod_topology_bg.py',
        'topology': V58 / 'WORKER.yaml',
        'source_manifest': V58 / 'SOURCE-MANIFEST.json',
        'runtime_integration': V58 / 'RUNTIME-INTEGRATION-MANIFEST.json',
    }
    return {'phase': PHASE, 'run_identity': RUN_ID, 'prefix': PREFIX,
...
def offline_check() -> dict:
    topology.validate(json.loads((V58 / 'WORKER.yaml').read_text()))
    if json.loads((V58 / 'SOURCE-MANIFEST.json').read_text()) != topology.source_manifest():
        raise ValueError('BG source projection changed')
    manifest = json.loads((V58 / 'RUNTIME-INTEGRATION-MANIFEST.json').read_text())
    from candidate.pod_loss_window_bg import SCOPE
    if (manifest['authorization_scope'] != SCOPE
            ...
            or manifest['identity'] != {'phase': PHASE, 'run_id': RUN_ID,
                                        'prefix': PREFIX}):
        raise ValueError('BG runtime integration identity changed')
    if json.loads(RUNNER_MANIFEST.read_text()) != build_runner_manifest():
        raise ValueError('BG host runner or projection source changed')
    return {'status': 'PASS_OFFLINE_ONLY',
        'authorization_scope_sha256': authorization_scope_sha256(),
        'source_manifest_sha256': sha((V58 / 'SOURCE-MANIFEST.json').read_bytes()),
        'topology_sha256': sha((V58 / 'WORKER.yaml').read_bytes())}
...
def workload_argv() -> list[str]:
    return ['/experiment/.venv/bin/python',
        '/workspace/tests/pdf_processing/q04/pod_workload_bg.py',
        '--prefix', PREFIX,
        '--authorization-scope-sha256', authorization_scope_sha256(),
...
def setup(kube, capacity: dict, owned: list[dict]):
    ...
    if (worker_deployment['spec']['replicas'] != 0
        ...
        raise ValueError('new BG Deployments were not inactive')
    ...
    if preflight.get('status') != 'PASS_PRE_INFERENCE':
        raise ValueError('BG pre-inference gate set did not pass')
    if time.time() + 825 + 300 > capacity['ends_at']:
        raise ValueError('insufficient fixed lease after BG preflight')
...
class RuntimeMonitor:
            ...
            with (OUT / 'vm-controller.jsonl').open('x', buffering=1) as vm, \
                ...
                while not self.stopping.is_set():
                    ...
                    if len(self.rows) > 1 and not 0 < row['time'] - self.rows[-2]['time'] <= 1:
                        raise ValueError('BG node VM telemetry gap')
                    if self.controller.transition_started is not None:
                        ...
                        if time.time() - self.controller.transition_started > limit:
                            raise TimeoutError('BG Pod transition exceeded approved bound')
                        ...
                        if (observed.get('generation') != generation
                            ...
                            raise ValueError('BG worker resource telemetry stale')
                        ...
                        if (attribution.get('attribution_complete') is not True
                            ...
                            raise ValueError('BG Activity Pod process attribution stale/incomplete')
    ...
    def stop(self):
        ...
        if self.thread.is_alive():
            raise TimeoutError('BG runtime monitor did not stop')
        if self.error is not None:
            raise self.error
        if not self.rows:
            raise ValueError('BG node telemetry never started')
...
def export_evidence(kube, coordinator: dict, volume: dict, *, success: bool):
    ...
    if before != after:
        raise ValueError('BG evidence changed during export')
    ...
    with tarfile.open(archive) as tar:
        ...
        if success and terminal is None:
            raise ValueError('BG durable terminal evidence missing')
        if terminal is not None:
            value = json.load(terminal)
            if value['volume_identity'] != volume:
                raise ValueError('BG terminal PVC identity changed')
            if success and not (value['workload_succeeded']
                                and value['cleanup_complete']):
                raise ValueError('BG terminal workload or cleanup not complete')
...
"""
    ...
    if (value.get('prefix') != PREFIX or value.get('bucket') != 't09a'
        ...
        raise ValueError('BG independent object readback incomplete')
...
def validate_old_resource_rows(rows: list[dict]):
    ...
    if any(gate[name] for name in (
        ...
        raise ValueError('BG old Pod all-sample resource gate failed')
...
def validate_archive(archive: Path) -> dict:
    ...
    for item in inventory:
        ...
        if len(raw) != item['bytes'] or sha(raw) != item['sha256']:
            raise ValueError('BG terminal inventory member changed')
    phase = f'state/{PHASE}/'
    ...
    if (accepted['result']['status'] != 'complete'
        ...
        raise ValueError('BG native Pod-loss business or cleanup proof incomplete')
    ...
    for generation, rows in ((1, old_samples), (2, new_samples)):
        if (len(rows) < 2 or any(row['generation'] != generation for row in rows)
            ...
            raise ValueError('BG worker resource sample coverage incomplete')
        for row in rows:
            check_sample(row, rows[0], expected['window'])
    if old_samples[-1]['time'] < request['requested_at']:
        raise ValueError('BG old worker resource coverage ended before Pod deletion')
    ...
    if (len(old_rows) < 2
        ...
        raise ValueError('BG old Pod process attribution incomplete')
    ...
    if (not summary['process_attribution_complete']
            ...
            or gate['status'] != 'PASS'):
        raise ValueError('BG replacement Pod process/resource attribution incomplete')
...
def execute(args):
    ...
    if json.loads(RUNNER_MANIFEST.read_text())['runtime_authorized'] is not True:
        raise RuntimeError('BG runtime awaits complete observer/cleanup review')
    if args.authorization_scope_sha256 != authorization_scope_sha256():
        raise ValueError('BG authorization scope digest changed')
    ...
    try:
        ...
        while workload.poll() is None:
            ...
            if time.time() >= deadline:
                raise TimeoutError('BG workload deadline')
            time.sleep(.1)
        check_monitor()
        if workload.returncode != 0:
            raise RuntimeError('BG workload failed; no retry')
        if controller.handled != {('start', 1), ('drain', 2), ('stop', 0)}:
            raise ValueError('BG complete Pod transition sequence not observed')
        reviewed.verify_held_deployments(kube)
    ...
    if primary is not None:
        raise primary
    if cleanup['errors']:
        raise RuntimeError('BG terminal cleanup incomplete: ' + repr(cleanup['errors']))
