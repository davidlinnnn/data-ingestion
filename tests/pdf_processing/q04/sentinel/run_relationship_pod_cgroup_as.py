"""Run one Q04 required-relationship interruption and recovery window."""
...
import pod_topology_as as pod_topology
from pod_remote_evidence_as import (
    FINAL_REQUIRED,
...
PHASE = "relationship-pod-cgroup-as"
RUN_IDENTITY = "q04-relationship-pod-cgroup-20260925-as"
PREFIX = "q04/relationship-pod-cgroup-20260925-as/"
OUT = Path("/private/tmp/q04-relationship-pod-cgroup-20260925-as")
...
WORKER_YAML = base.Q04 / "pod-topology-v44/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v44/RUNNER-MANIFEST.json"
...
def authorization_scope() -> dict:
    return {
        ...
        "group_requests": 44,
        "max_requests": 20,
        "expected_parser_generations": 1,
        "expected_worker_generations": 2,
        "automatic_retry": False,
        "ocr_phase_trace": {
            "env": "Q04_OCR_TRACE_DIR",
            "directory": EVIDENCE + "/state/" + PHASE + "/ocr-trace",
            "stages": ["ocr_enter", "crop_ready", "engine_start", "engine_ready",
                       "inference_start", "inference_ready", "result_written"],
        },
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_as.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_as.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_relationship_pod_cgroup_as_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_as.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_as.py",
        "pre_inference": base.Q04 / "pod_preflight_as.py",
        "topology_builder": base.Q04 / "pod_topology_as.py",
        "matrix_window": base.Q04 / "candidate/relationship_interruption_window_ao.py",
        "source_manifest": base.Q04 / "pod-topology-v44/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v44/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution.py"),
        "node_pressure_guard": Path(__file__).with_name("run_relationship_pod_cgroup_as_guarded.py"),
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete AO relationship-interruption runtime gates"
    return value
