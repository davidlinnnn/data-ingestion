...
import pod_topology_l as pod_topology
...
from pod_remote_evidence_l import (
    FINAL_REQUIRED,
...
PHASE = "yolo-pod-cgroup-l"
RUN_IDENTITY = "q04-yolo-pod-cgroup-20260920-l"
PREFIX = "q04/yolo-pod-cgroup-20260920-l/"
OUT = Path("/private/tmp/q04-yolo-pod-cgroup-20260920-l")
CONTROL = "/q04-control"
...
WORKER_YAML = Q04 / "pod-topology-v11/WORKER.yaml"
OFFLINE_MANIFEST = Q04 / "pod-topology-v11/RUNNER-MANIFEST.json"
LOCAL_PYTHON = Path(
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_l.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_l.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    paths = {
        ...
        "pod_workload": Q04 / "pod_workload_l.py",
        "pod_init": Q04 / "pod_init_l.py",
        "remote_evidence": Q04 / "pod_remote_evidence_l.py",
        "durable_evidence": Q04 / "pod_durable_evidence.py",
        "evidence_directory": Q04 / "pod_evidence_directory_d.py",
        "pre_inference": Q04 / "pod_preflight_l.py",
        "topology_builder": Q04 / "pod_topology_l.py",
        "worker_yaml": WORKER_YAML,
        "source_manifest": Q04 / "pod-topology-v11/SOURCE-MANIFEST.json",
        "run_plan": Q04 / "pod-topology-v11/RUN-PLAN.md",
        "readme": Q04 / "pod-topology-v11/README.md",
        ...
        "offline_validation": Q04 / "pod-topology-v11/OFFLINE-VALIDATION.md",
        "historical_integration_manifest": Q04 / "pod-topology-v1/INTEGRATION-MANIFEST.json",
        "runtime_integration_manifest": Q04 / "pod-topology-v11/RUNTIME-INTEGRATION-MANIFEST.json",
...
evidence=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-l');used=sum(p.stat().st_size for p in evidence.rglob('*') if p.is_file())
...
from pathlib import Path
path=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-l/supervisor-ownership.json')
...
def terminal_cleanup_program() -> str:
    return """import json
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-l');owner=json.loads((root/'supervisor-ownership.json').read_text())
...
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-l');owner=json.loads((root/'supervisor-ownership.json').read_text());pid=owner['pid']
...
else: scratches=[path/'scratch' for path in (root/'state/yolo-pod-cgroup-l').glob('worker-*')]
...
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-l');rows=[]
...
def execute_window(args, kube=None) -> dict:
    ...
    try:
        ...
        stage_volume_identity = (
            ...
            "write_once(Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-l/evidence-volume-identity.json'),json.loads(sys.stdin.read()))"
        )
        kube.run(
