"""Fresh-only ACL fixture-09 resource-calibration runner; preparation only until authorized."""
...
OUT = Path("/private/tmp/q04-acl-fresh-resource-20260917-v1")
CAPACITY = REMOTE + "/capacity-acl-fresh-resource-v1.json"
RESERVATION = REMOTE + "/reservation-acl-fresh-resource-v1.json"
RELEASE = REMOTE + "/release-reservation-acl-fresh-resource-v1"
ADMISSION_EVIDENCE = REMOTE + "/pre-admission-acl-fresh-resource-v1.jsonl"
ADMISSION_RESULT = REMOTE + "/admission-acl-fresh-resource-v1.json"
RUNNER_DIR = REMOTE + "/runner-acl-fresh-resource-v1"
...
OBSERVATION_CGROUP_BYTES = 4_294_967_296
...
STATE = {
    ...
    "fixture": "09",
    "modes": ["fresh"],
    "candidate_max_cgroup_bytes": OBSERVATION_CGROUP_BYTES,
    "candidate_is_worker_policy": False,
    "candidate_is_pod_limit": False,
    "attribution_interval_seconds": ATTRIBUTION_INTERVAL_SECONDS,
    "attribution_gap_seconds": ATTRIBUTION_GAP_SECONDS,
}
...
def remote_absence_paths():
    """Return every resource-calibration identity that must be unused."""
    return {
...
def build_capacity(*, started_at, owner, approval_reference):
    ...
    capacity.update(
        ...
        max_cgroup_bytes=OBSERVATION_CGROUP_BYTES,
    )
    return capacity
    ...
    *,
    ...
    inner = "\n".join(
        (
            ...
            + shlex.quote(python)
            + " "
            + shlex.quote(remote + "/runner-acl-fresh-resource-v1/acl_fresh_measure.py")
            + ' --bundle "$R/inputs" --state "$R/state"'
            + " --capacity "
            + shlex.quote(capacity)
            + " --name acl-fresh-resource-v1"
            + " --expected-run-id "
            + shlex.quote(RUN_ID)
            + " --expected-prefix "
            + shlex.quote(PREFIX)
    ...
    return "\n".join(
        (
            ...
            'export PYTHONPATH="$R/runner-acl-fresh-resource-v1:$R/code/src:'
...
def write_artifact_manifest():
    """Hash retained local evidence after every writer has been closed."""
    artifacts = []
    ...
    with (OUT / "artifact-manifest.json").open("x") as stream:
        stream.write(
            json.dumps(
                {"root": str(OUT), "phase": PHASE, "artifacts": artifacts},
                indent=2,
            )
            + "\n"
        )
...
""".replace("COORDINATOR_UID", repr(COORDINATOR_UID)).replace(
        ...
        try:
            ...
            def admit():
                ...
                (OUT / "admission-resource-v1.json").write_text(
                    json.dumps(result, indent=2, sort_keys=True) + "\n"
                )
                return result

            def launch():
                STATE["phase"] = "running-fresh-resource"
                save()
            ...
            assert driver.returncode == 0, "fresh resource driver failed; retain logs, no retry"
            ...
            if acquired:
                ...
                STATE["measurement_passed"] = bool(
                    STATE.get("measurement_passed") and not STATE["errors"]
                )
