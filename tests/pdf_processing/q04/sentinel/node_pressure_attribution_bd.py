"""Read-only kind-node PSI and exact object-service cgroup memory samples."""
...
def target_sample(groups, pod_uid):
    marker = 'pod' + pod_uid.replace('-', '_')
    matches = [name for name in groups if marker in name and 'cri-containerd-' in name
               and (CGROUPS / name / 'memory.max').read_text().strip() == '805306368']
    if len(matches) != 1:
        raise ValueError('exact object-service container cgroup unavailable')
    name = matches[0]
    path = CGROUPS / name
    ...
    return {'pod_uid': pod_uid, 'cgroup': name, 'memory_current': int((path / 'memory.current').read_text()),
            'memory_max': 805306368, 'full_total_us': groups[name],
            'max_events': int(events['max']), 'oom': int(events['oom']),
            'oom_kill': int(events['oom_kill']),
            'anon': int(stat['anon']), 'file': int(stat['file']), 'kernel': int(stat['kernel'])}
...
def main():
    ...
    parser.add_argument("--target-pod-uid")
    parser.add_argument("--self-test", action="store_true")
    args = parser.parse_args()
    self_test()
    if args.self_test:
        return
    if args.seconds <= 0 or args.interval <= 0 or not args.target_pod_uid:
    ...
    previous = snapshot()
    target = target_sample(previous[2], args.target_pod_uid)
    print(json.dumps({"kind": "start", "time": previous[0],
                      ...
                      "cgroup_count": len(previous[2]), "target": target}), flush=True)
    while not stopped and time.monotonic() < end:
        time.sleep(min(args.interval, max(0, end - time.monotonic())))
        current = snapshot()
        row = delta(previous, current)
        print(json.dumps({"kind": "sample", **row,
                          "target": target_sample(current[2], args.target_pod_uid)}), flush=True)
        previous = current
