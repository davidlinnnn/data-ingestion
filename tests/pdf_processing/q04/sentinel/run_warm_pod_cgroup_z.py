...
import pod_topology_z as pod_topology
from pod_remote_evidence_z import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with Z topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_z_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
RUN_IDENTITY = "q04-warm-pod-cgroup-20260921-z"
PREFIX = "q04/warm-pod-cgroup-20260921-z/"
OUT = Path("/private/tmp/q04-warm-pod-cgroup-20260921-z")
...
WORKER_YAML = base.Q04 / "pod-topology-v25/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v25/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_z.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_z.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_z.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_z.py",
        "pre_inference": base.Q04 / "pod_preflight_z.py",
        "topology_builder": base.Q04 / "pod_topology_z.py",
        "warm_window": base.Q04 / "candidate/warm_pod_window_r.py",
        "source_manifest": base.Q04 / "pod-topology-v25/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v25/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete Z warm runtime gates"
    return value
