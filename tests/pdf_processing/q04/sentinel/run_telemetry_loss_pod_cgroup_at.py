"""Run one Q04 active worker-telemetry-loss abort window."""
...
import pod_topology_at as pod_topology
from pod_remote_evidence_at import (
    FINAL_REQUIRED,
...
RUN_IDENTITY = "q04-telemetry-loss-pod-cgroup-20260925-at"
PREFIX = "q04/telemetry-loss-pod-cgroup-20260925-at/"
OUT = Path("/private/tmp/q04-telemetry-loss-pod-cgroup-20260925-at")
...
WORKER_YAML = base.Q04 / "pod-topology-v45/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v45/RUNNER-MANIFEST.json"
...
def authorization_scope() -> dict:
    return {
        ...
        "fixtures": ["native"],
        "modes": ["guard"],
        "injection": "stop worker sampler after five registered pages during active group",
        "max_requests": 20,
        "expected_parser_generations": 1,
        "expected_worker_generations": 1,
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_at.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_at.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_telemetry_loss_pod_cgroup_at_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_at.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_at.py",
        "pre_inference": base.Q04 / "pod_preflight_at.py",
        "topology_builder": base.Q04 / "pod_topology_at.py",
        "matrix_window": base.Q04 / "candidate/telemetry_loss_window_at.py",
        "source_manifest": base.Q04 / "pod-topology-v45/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v45/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution.py"),
        "node_pressure_guard": Path(__file__).with_name("run_telemetry_loss_pod_cgroup_at_guarded.py"),
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and active telemetry-loss abort runtime gate"
    return value
