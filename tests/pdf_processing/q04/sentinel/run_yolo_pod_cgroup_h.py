...
import pod_topology_h as pod_topology
...
from pod_remote_evidence_h import (
    FINAL_REQUIRED,
...
PHASE = "yolo-pod-cgroup-h"
RUN_IDENTITY = "q04-yolo-pod-cgroup-20260920-h"
PREFIX = "q04/yolo-pod-cgroup-20260920-h/"
OUT = Path("/private/tmp/q04-yolo-pod-cgroup-20260920-h")
CONTROL = "/q04-control"
...
WORKER_YAML = Q04 / "pod-topology-v7/WORKER.yaml"
OFFLINE_MANIFEST = Q04 / "pod-topology-v7/RUNNER-MANIFEST.json"
LOCAL_PYTHON = Path(
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_h.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_h.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    paths = {
        ...
        "pod_workload": Q04 / "pod_workload_h.py",
        "pod_init": Q04 / "pod_init_h.py",
        "remote_evidence": Q04 / "pod_remote_evidence_h.py",
        "durable_evidence": Q04 / "pod_durable_evidence.py",
        "evidence_directory": Q04 / "pod_evidence_directory_d.py",
        "pre_inference": Q04 / "pod_preflight_h.py",
        "topology_builder": Q04 / "pod_topology_h.py",
        "worker_yaml": WORKER_YAML,
        "source_manifest": Q04 / "pod-topology-v7/SOURCE-MANIFEST.json",
        "run_plan": Q04 / "pod-topology-v7/RUN-PLAN.md",
        "readme": Q04 / "pod-topology-v7/README.md",
        ...
        "offline_validation": Q04 / "pod-topology-v7/OFFLINE-VALIDATION.md",
        "historical_integration_manifest": Q04 / "pod-topology-v1/INTEGRATION-MANIFEST.json",
        "runtime_integration_manifest": Q04 / "pod-topology-v7/RUNTIME-INTEGRATION-MANIFEST.json",
    ...
    return {
        ...
        "runtime_blocker": (
            "live admission, new H mount identity and the complete runtime "
...
evidence=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h');used=sum(p.stat().st_size for p in evidence.rglob('*') if p.is_file())
...
from pathlib import Path
path=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h/supervisor-ownership.json')
...
def terminal_cleanup_program() -> str:
    return """import json
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h');owner=json.loads((root/'supervisor-ownership.json').read_text())
...
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h');owner=json.loads((root/'supervisor-ownership.json').read_text());pid=owner['pid']
...
else: scratches=[path/'scratch' for path in (root/'state/yolo-pod-cgroup-h').glob('worker-*')]
...
from pathlib import Path
root=Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h');rows=[]
...
def execute_window(args, kube=None) -> dict:
    ...
    try:
        ...
        stage_volume_identity = (
            ...
            "write_once(Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h/evidence-volume-identity.json'),json.loads(sys.stdin.read()))"
        )
        kube.run(
                ...
                try:
                    owner = json.loads(
                        kube.exec_python(
                            ...
                            "s=json.loads(Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h/supervisor-ownership.json').read_text());"
                            "w=json.loads(Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h/ownership.json').read_text());"
                    ...
                    kube.run(
                        ["exec", "-i", pod, "--", "/experiment/.venv/bin/python", "-c",
                         "from pathlib import Path;import sys;Path('/q04-evidence/q04-yolo-pod-cgroup-20260920-h/transport-identity.json').open('x').write(sys.stdin.read())"],
