...
import pod_topology_al as pod_topology
from pod_remote_evidence_al import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with AL topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_al_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "profile-pod-cgroup-al"
RUN_IDENTITY = "q04-profile-pod-cgroup-20260924-al"
PREFIX = "q04/profile-pod-cgroup-20260924-al/"
OUT = Path("/private/tmp/q04-profile-pod-cgroup-20260924-al")
...
WORKER_YAML = base.Q04 / "pod-topology-v37/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v37/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_al.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_al.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_al.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_al.py",
        "pre_inference": base.Q04 / "pod_preflight_al.py",
        "topology_builder": base.Q04 / "pod_topology_al.py",
        "matrix_window": base.Q04 / "candidate/profile_reuse_window_al.py",
        "source_manifest": base.Q04 / "pod-topology-v37/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v37/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete AL evidence-only reuse runtime gates"
    return value
