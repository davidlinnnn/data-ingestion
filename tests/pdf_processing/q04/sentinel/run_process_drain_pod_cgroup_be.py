"""Run one Q04 native worker-process drain/recovery window."""
...
import pod_topology_be as pod_topology
from pod_remote_evidence_be import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with AM topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_am_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "process-drain-pod-cgroup-be"
RUN_IDENTITY = "q04-process-drain-pod-cgroup-20260925-be"
PREFIX = "q04/process-drain-pod-cgroup-20260925-be/"
OUT = Path("/private/tmp/q04-process-drain-pod-cgroup-20260925-be")
...
WORKER_YAML = base.Q04 / "pod-topology-v56/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v56/RUNNER-MANIFEST.json"
...
def authorization_scope() -> dict:
    return {
        ...
        "fixtures": ["native"],
        "modes": ["drain"],
        "injection": "stop owned worker process after five registered pages during active group",
        "max_requests": 20,
        "expected_parser_generations": 2,
        "expected_worker_generations": 2,
        ...
        "node_pressure_observer": {
            "read_only": True,
            "sample_interval_seconds": 0.25,
            "owned_stop": True,
            "cgroup_namespace": "host",
            "network": "none",
            "image_id": "sha256:08497ee19eace7b4b5348db5c6a1591d7752b164530a36f855cb0f2bdcbadd48",
        },
        "object_service_observer": {
            "read_only": True,
            "pod_uid": "a84384de-8292-439e-b8ce-d75a70816d3f",
            "sample_interval_seconds": 0.25,
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_be.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_be.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_process_drain_pod_cgroup_be_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def all_inventory_copied(mirror):
    if not set(FINAL_REQUIRED).issubset(mirror.complete):
        return False
    terminal = json.loads((mirror.root / "durable-terminal-manifest.json").read_text())
    inventory = terminal.get("inventory")
    if not isinstance(inventory, list):
        raise ValueError("durable terminal inventory missing")
    return all(item["path"] in mirror.complete for item in inventory)


def adapt_execute_window():
    source = inspect.getsource(_execute_window)
    drain = "while not set(FINAL_REQUIRED).issubset(mirror.complete):"
    if source.count(P_EVIDENCE) != 1 or source.count(drain) != 1:
        raise RuntimeError("P engine evidence drain contract changed")
    namespace = dict(vars(base))
    namespace["all_inventory_copied"] = all_inventory_copied
    source = source.replace(P_EVIDENCE, EVIDENCE).replace(
        drain, "while not all_inventory_copied(mirror):"
    )
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_be.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_be.py",
        "pre_inference": base.Q04 / "pod_preflight_be.py",
        "topology_builder": base.Q04 / "pod_topology_be.py",
        "matrix_window": base.Q04 / "candidate/process_drain_window_be.py",
        "source_manifest": base.Q04 / "pod-topology-v56/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v56/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution_be.py"),
        "node_pressure_guard": Path(__file__).with_name("run_process_drain_pod_cgroup_be_guarded.py"),
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and native worker-process drain/recovery gate"
    return value
