...
import pod_topology_w as pod_topology
from pod_remote_evidence_w import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with W topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_w_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "warm-pod-cgroup-w"
RUN_IDENTITY = "q04-warm-pod-cgroup-20260921-w"
PREFIX = "q04/warm-pod-cgroup-20260921-w/"
OUT = Path("/private/tmp/q04-warm-pod-cgroup-20260921-w")
...
WORKER_YAML = base.Q04 / "pod-topology-v22/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v22/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_w.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_w.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_w.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_w.py",
        "pre_inference": base.Q04 / "pod_preflight_w.py",
        "topology_builder": base.Q04 / "pod_topology_w.py",
        "warm_window": base.Q04 / "candidate/warm_pod_window_r.py",
        "source_manifest": base.Q04 / "pod-topology-v22/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v22/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete W warm runtime gates"
    return value
