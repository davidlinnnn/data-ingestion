"""Run AZ once with owned node and object-service cgroup telemetry."""
...
from sentinel import run_process_drain_pod_cgroup_az as runner

base, run_az, OUT, RUN_IDENTITY = runner.base, runner.main, runner.OUT, runner.RUN_IDENTITY


PROBE = Path(__file__).with_name("node_pressure_attribution_az.py")
...
PROBE_NAME = "q04-host-pressure-az-20260925"
...
OBJECT_POD = "objects-6756bf97c6-pkqb7"
OBJECT_POD_UID = "a84384de-8292-439e-b8ce-d75a70816d3f"


def verify_object_pod():
    pod = json.loads(subprocess.check_output([
        "kubectl", "--context", base.CONTEXT, "-n", base.NAMESPACE,
        "get", "pod", OBJECT_POD, "-o", "json"], text=True, timeout=20))
    if (pod["metadata"]["uid"] != OBJECT_POD_UID
            or pod["status"]["phase"] != "Running"
            or pod["spec"]["nodeName"] != base.NODE):
        raise ValueError("object-service Pod identity or placement changed")
...
def start_probe():
    if any(path.exists() for path in (PROBE_OUT, PROBE_ERR, PROBE_CLEANUP, PROBE_CID, OUT)):
        raise ValueError("AZ identity or host observer output already used")
    verify_object_pod()
        ...
        while time.monotonic() < deadline:
            ...
            if lines:
                ...
                if (identity.get("kind") != "start" or identity.get("run_id") != RUN_IDENTITY
                        ...
                        or identity.get("target", {}).get("pod_uid") != OBJECT_POD_UID
                        or identity["target"]["memory_max"] != 805306368):
...
def same_object_cgroup(sample, identity):
    target = sample.get("target", {})
    start = identity["target"]
    return (target.get("pod_uid") == OBJECT_POD_UID
            and target.get("cgroup") == start["cgroup"]
            and target.get("memory_max") == start["memory_max"])
...
def stop_probe(process, identity, *, require_end=True):
    ...
    def terminal():
        ...
        if require_end and (not samples or any(
                ...
                or not same_object_cgroup(sample, identity)
                or sample["target"]["memory_current"] < 0
...
def main(argv=None):
    ...
    if (not args.owner or not args.approval_reference
        ...
        raise ValueError("AZ execution authorization scope changed")
    process = None
    identity = None
