...
REMOTE = "/tmp/q04-option-a-20260918-c"
PREFIX = "q04/option-a-20260918-c/"
...
OUT = Path("/private/tmp/q04-acl-option-a-window-20260918-c")
CAPACITY = REMOTE + "/capacity-acl-option-a-window-c.json"
RESERVATION = REMOTE + "/reservation-acl-option-a-window-c.json"
RELEASE = REMOTE + "/release-reservation-acl-option-a-window-c"
ADMISSION_EVIDENCE = REMOTE + "/pre-admission-acl-option-a-window-c.jsonl"
ADMISSION_RESULT = REMOTE + "/admission-acl-option-a-window-c.json"
RUNNER_DIR = REMOTE + "/runner-acl-option-a-window-c"
...
EXPECTED_CONTENT_EVIDENCE_SHA256 = (
    "ecf8acae8037fbd2e6896bb1d966957470a84c42cdc455910e4bec5f44968e8e"
)
...
def validate_staged_runtime(staged, bundle):
    """Fail closed on every staged Option A artifact and interface field."""
    expected_keys = {
        "bundle_sha256",
        "runtime_sha256",
        "reference_09_sha256",
    ...
    expected_hashes = {
        ...
        "reference_09_sha256": EXPECTED_REFERENCE_09_SHA256,
        "quality_oracle_sha256": EXPECTED_QUALITY_ORACLE_SHA256,
        "content_evidence_sha256": EXPECTED_CONTENT_EVIDENCE_SHA256,
    }
...
print(json.dumps({'run_id':config['run_id'],'prefix':config['prefix'],
 ...
 'profile_id':config['profiles']['09']['id'],
 'profile_release':config['profiles']['09']['release'],
    ...
    *,
    ...
    inner = "\n".join(
        (
            ...
            + " --name acl-option-a-window-c --trial-seconds 180 --capacity-approved",
        )
    )
    return "\n".join(
        (
            ...
            'export PYTHONPATH="$R/runner-acl-option-a-window-c:$R/code/src:'
...
def staged_sources():
    return {
        ...
        "run_acl_option_a_c.py": Path(__file__),
    }
