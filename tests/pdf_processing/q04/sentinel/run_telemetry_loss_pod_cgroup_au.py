...
import pod_topology_au as pod_topology
from pod_remote_evidence_au import (
    FINAL_REQUIRED,
...
RUN_IDENTITY = "q04-telemetry-loss-pod-cgroup-20260925-au"
PREFIX = "q04/telemetry-loss-pod-cgroup-20260925-au/"
OUT = Path("/private/tmp/q04-telemetry-loss-pod-cgroup-20260925-au")
...
WORKER_YAML = base.Q04 / "pod-topology-v46/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v46/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_au.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_au.py",
        "--workspace", "/workspace",
...
def exact_command() -> str:
    return shlex.join([
        ...
        str(Path(__file__).with_name("run_telemetry_loss_pod_cgroup_au_guarded.py").resolve()),
        "--execute",
        "--owner", "main-session",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_au.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_au.py",
        "pre_inference": base.Q04 / "pod_preflight_au.py",
        "topology_builder": base.Q04 / "pod_topology_au.py",
        "matrix_window": base.Q04 / "candidate/telemetry_loss_window_au.py",
        "source_manifest": base.Q04 / "pod-topology-v46/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v46/RUNTIME-INTEGRATION-MANIFEST.json",
        "node_pressure_observer": Path(__file__).with_name("node_pressure_attribution.py"),
        "node_pressure_guard": Path(__file__).with_name("run_telemetry_loss_pod_cgroup_au_guarded.py"),
    }
