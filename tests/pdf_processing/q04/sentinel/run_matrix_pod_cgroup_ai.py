...
import pod_topology_ai as pod_topology
from pod_remote_evidence_ai import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with AI topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_ai_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "matrix-pod-cgroup-ai"
RUN_IDENTITY = "q04-matrix-pod-cgroup-20260924-ai"
PREFIX = "q04/matrix-pod-cgroup-20260924-ai/"
OUT = Path("/private/tmp/q04-matrix-pod-cgroup-20260924-ai")
...
WORKER_YAML = base.Q04 / "pod-topology-v34/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v34/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_ai.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_ai.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_ai.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_ai.py",
        "pre_inference": base.Q04 / "pod_preflight_ai.py",
        "topology_builder": base.Q04 / "pod_topology_ai.py",
        "matrix_window": base.Q04 / "candidate/matrix_v3_window.py",
        "source_manifest": base.Q04 / "pod-topology-v34/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v34/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete AI matrix runtime gates"
    return value
