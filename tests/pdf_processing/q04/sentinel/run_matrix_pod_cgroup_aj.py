...
import pod_topology_aj as pod_topology
from pod_remote_evidence_aj import (
    FINAL_REQUIRED,
...
def _load_engine():
    """Load P with AJ topology present before Python binds function defaults."""
    spec = importlib.util.spec_from_file_location(
        "q04_warm_aj_pod_engine", Path(__file__).with_name("run_yolo_pod_cgroup_p.py")
    )
...
PHASE = "matrix-pod-cgroup-aj"
RUN_IDENTITY = "q04-matrix-pod-cgroup-20260924-aj"
PREFIX = "q04/matrix-pod-cgroup-20260924-aj/"
OUT = Path("/private/tmp/q04-matrix-pod-cgroup-20260924-aj")
...
WORKER_YAML = base.Q04 / "pod-topology-v35/WORKER.yaml"
OFFLINE_MANIFEST = base.Q04 / "pod-topology-v35/RUNNER-MANIFEST.json"
...
def workload_argv() -> list[str]:
    return [
        ...
        "/workspace/tests/pdf_processing/q04/pod_workload_aj.py",
        "--prefix", PREFIX,
...
def preflight_argv() -> list[str]:
    return [
        "/experiment/.venv/bin/python",
        "/workspace/tests/pdf_processing/q04/pod_preflight_aj.py",
        "--workspace", "/workspace",
...
def build_offline_manifest() -> dict:
    ...
    paths = {
        ...
        "pod_workload": base.Q04 / "pod_workload_aj.py",
        "remote_evidence": base.Q04 / "pod_remote_evidence_aj.py",
        "pre_inference": base.Q04 / "pod_preflight_aj.py",
        "topology_builder": base.Q04 / "pod_topology_aj.py",
        "matrix_window": base.Q04 / "candidate/matrix_v3_window_aj.py",
        "source_manifest": base.Q04 / "pod-topology-v35/SOURCE-MANIFEST.json",
        "runtime_integration_manifest": base.Q04 / "pod-topology-v35/RUNTIME-INTEGRATION-MANIFEST.json",
    }
    value["sources"].update({
        name: base.sha256(path.read_bytes()) for name, path in paths.items()
    })
    value["runtime_blocker"] = "live admission and complete AJ matrix runtime gates"
    return value
