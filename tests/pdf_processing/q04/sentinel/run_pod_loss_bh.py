...
from sentinel.object_monitor_bh import ObjectMonitor
...
PREFIX = 'q04/pod-loss-pod-cgroup-20260926-bh/'
OUT = Path('/private/tmp/q04-pod-loss-pod-cgroup-20260926-bh')
EVIDENCE = '/q04-evidence/' + topology.EVIDENCE_DIRECTORY_NAME
CONTROL = '/q04-control'
BUNDLE = Path('/private/tmp/q04-inputs-warm-continuation-v3-ah')
V59 = Q04 / 'pod-topology-v59'
RUNNER_MANIFEST = V59 / 'RUNNER-MANIFEST.json'
...
def build_runner_manifest() -> dict:
    paths = {
        ...
        'mailbox_controller': Path(__file__).with_name('pod_loss_mailbox_controller_bh.py'),
        'pod_transition': Path(__file__).with_name('pod_loss_transition_bh.py'),
        'object_monitor': Path(__file__).with_name('object_monitor_bh.py'),
        'object_probe': Path(__file__).with_name('object_cgroup_probe_bh.py'),
        'object_limit_trial': Path(__file__).with_name('object_limit_trial_bh.py'),
        'activity_supervisor': Q04 / 'pod_activity_supervisor_bh.py',
        'coordinator_supervisor': Q04 / 'pod_workload_bh.py',
        'coordinator_bridge': Q04 / 'pod_loss_bridge_bh.py',
        'candidate': Q04 / 'candidate/pod_loss_window_bh.py',
        'preflight': Q04 / 'pod_preflight_bh.py',
        'topology_builder': Q04 / 'pod_topology_bh.py',
        'topology': V59 / 'WORKER.yaml',
        'source_manifest': V59 / 'SOURCE-MANIFEST.json',
        'runtime_integration': V59 / 'RUNTIME-INTEGRATION-MANIFEST.json',
    }
    return {'phase': PHASE, 'run_identity': RUN_ID, 'prefix': PREFIX,
...
def offline_check() -> dict:
    topology.validate(json.loads((V59 / 'WORKER.yaml').read_text()))
    if json.loads((V59 / 'SOURCE-MANIFEST.json').read_text()) != topology.source_manifest():
        raise ValueError('BH source projection changed')
    manifest = json.loads((V59 / 'RUNTIME-INTEGRATION-MANIFEST.json').read_text())
    from candidate.pod_loss_window_bh import SCOPE
    if (manifest['authorization_scope'] != SCOPE
            ...
            or manifest['identity'] != {'phase': PHASE, 'run_id': RUN_ID,
                                        'prefix': PREFIX}):
        raise ValueError('BH runtime integration identity changed')
    if json.loads(RUNNER_MANIFEST.read_text()) != build_runner_manifest():
        raise ValueError('BH host runner or projection source changed')
    return {'status': 'PASS_OFFLINE_ONLY',
        'authorization_scope_sha256': authorization_scope_sha256(),
        'source_manifest_sha256': sha((V59 / 'SOURCE-MANIFEST.json').read_bytes()),
        'topology_sha256': sha((V59 / 'WORKER.yaml').read_bytes())}
...
def workload_argv() -> list[str]:
    return ['/experiment/.venv/bin/python',
        '/workspace/tests/pdf_processing/q04/pod_workload_bh.py',
        '--prefix', PREFIX,
        '--authorization-scope-sha256', authorization_scope_sha256(),
...
def setup(kube, capacity: dict, owned: list[dict]):
    ...
    if (worker_deployment['spec']['replicas'] != 0
        ...
        raise ValueError('new BH Deployments were not inactive')
    ...
    if preflight.get('status') != 'PASS_PRE_INFERENCE':
        raise ValueError('BH pre-inference gate set did not pass')
    if time.time() + 825 + 300 > capacity['ends_at']:
        raise ValueError('insufficient fixed lease after BH preflight')
...
class RuntimeMonitor:
            ...
            with (OUT / 'vm-controller.jsonl').open('x', buffering=1) as vm, \
                ...
                while not self.stopping.is_set():
                    ...
                    if len(self.rows) > 1 and not 0 < row['time'] - self.rows[-2]['time'] <= 1:
                        raise ValueError('BH node VM telemetry gap')
                    if self.controller.transition_started is not None:
                        ...
                        if time.time() - self.controller.transition_started > limit:
                            raise TimeoutError('BH Pod transition exceeded approved bound')
                        ...
                        if (observed.get('generation') != generation
                            ...
                            raise ValueError('BH worker resource telemetry stale')
                        ...
                        if (attribution.get('attribution_complete') is not True
                            ...
                            raise ValueError('BH Activity Pod process attribution stale/incomplete')
    ...
    def stop(self):
        ...
        if self.thread.is_alive():
            raise TimeoutError('BH runtime monitor did not stop')
        if self.error is not None:
            raise self.error
        if not self.rows:
            raise ValueError('BH node telemetry never started')
...
def export_evidence(kube, coordinator: dict, volume: dict, *, success: bool):
    ...
    if before != after:
        raise ValueError('BH evidence changed during export')
    ...
    with tarfile.open(archive) as tar:
        ...
        if success and terminal is None:
            raise ValueError('BH durable terminal evidence missing')
        if terminal is not None:
            value = json.load(terminal)
            if value['volume_identity'] != volume:
                raise ValueError('BH terminal PVC identity changed')
            if success and not (value['workload_succeeded']
                                and value['cleanup_complete']):
                raise ValueError('BH terminal workload or cleanup not complete')
...
"""
    ...
    if (value.get('prefix') != PREFIX or value.get('bucket') != 't09a'
        ...
        raise ValueError('BH independent object readback incomplete')
...
def validate_old_resource_rows(rows: list[dict]):
    ...
    if any(gate[name] for name in (
        ...
        raise ValueError('BH old Pod all-sample resource gate failed')
...
def validate_archive(archive: Path) -> dict:
    ...
    for item in inventory:
        ...
        if len(raw) != item['bytes'] or sha(raw) != item['sha256']:
            raise ValueError('BH terminal inventory member changed')
    phase = f'state/{PHASE}/'
    ...
    if (accepted['result']['status'] != 'complete'
        ...
        raise ValueError('BH native Pod-loss business or cleanup proof incomplete')
    ...
    for generation, rows in ((1, old_samples), (2, new_samples)):
        if (len(rows) < 2 or any(row['generation'] != generation for row in rows)
            ...
            raise ValueError('BH worker resource sample coverage incomplete')
        for row in rows:
            check_sample(row, rows[0], expected['window'])
    if old_samples[-1]['time'] < request['requested_at']:
        raise ValueError('BH old worker resource coverage ended before Pod deletion')
    ...
    if (len(old_rows) < 2
        ...
        raise ValueError('BH old Pod process attribution incomplete')
    ...
    if (not summary['process_attribution_complete']
            ...
            or gate['status'] != 'PASS'):
        raise ValueError('BH replacement Pod process/resource attribution incomplete')
...
def execute(args):
    ...
    if json.loads(RUNNER_MANIFEST.read_text())['runtime_authorized'] is not True:
        raise RuntimeError('BH runtime awaits complete observer/cleanup review')
    if args.authorization_scope_sha256 != authorization_scope_sha256():
        raise ValueError('BH authorization scope digest changed')
    ...
    try:
        ...
        while workload.poll() is None:
            ...
            if time.time() >= deadline:
                raise TimeoutError('BH workload deadline')
            time.sleep(.1)
        check_monitor()
        if workload.returncode != 0:
            raise RuntimeError('BH workload failed; no retry')
        if controller.handled != {('start', 1), ('drain', 2), ('stop', 0)}:
            raise ValueError('BH complete Pod transition sequence not observed')
        reviewed.verify_held_deployments(kube)
    ...
    if primary is not None:
        raise primary
    if cleanup['errors']:
        raise RuntimeError('BH terminal cleanup incomplete: ' + repr(cleanup['errors']))
