...
from pod_loss_bridge_bh import Host
    ...
    def test_start_and_drain_require_exact_pod_and_sample_proof(self):
        async def exercise():
            with tempfile.TemporaryDirectory() as raw:
                ...
                root = evidence / 'state' / 'pod-loss-bh'
                root.mkdir(parents=True)
                config = {'run_id': 'q04-bh', 'drain_seconds': 1,
        ...
        async def exercise():
            with tempfile.TemporaryDirectory() as raw:
                ...
                while not (host.control / '2-drain.request.json').exists():
                    await asyncio.sleep(.01)
                reply.write_text(json.dumps({'run_id': 'q04-bh', 'kind': 'drain',
        ...
        async def exercise():
            with tempfile.TemporaryDirectory() as raw:
                ...
                (host.control / '0-stop.reply.json').write_text(json.dumps({
                    'run_id': 'q04-bh', 'kind': 'stop', 'generation': 0,
