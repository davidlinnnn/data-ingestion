...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_preflight_ak_engine", Path(__file__).with_name("pod_preflight_q.py")
)
...
TOPOLOGY_PHASE = "q04-pod-cgroup-ak"
RUN_ID = "q04-profile-pod-cgroup-20260924-ak"
sha256 = base.base.sha256


def verify_configuration_ak(**kwargs):
...
def verify_workload_imports_ak(*, workspace, bundle, prefix):
    modules = (
        "candidate.profile_rejection_window_ak",
        "pod_init_p",
        "pod_workload_ak",
        "pod_remote_evidence_ak",
    )
    for module in modules:
        importlib.import_module(module)
    from candidate.warm_pod_window_r import validate_contract
    from candidate.profile_rejection_window_ak import validate_scope
    ...
    path = workspace / "tests/pdf_processing/q04/pod-topology-v36/RUNTIME-INTEGRATION-MANIFEST.json"
...
base.RUN_ID = RUN_ID
base.verify_configuration_q = verify_configuration_ak
base.verify_workload_imports_q = verify_workload_imports_ak
main = base.main
