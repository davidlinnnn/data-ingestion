...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_workload_ao_engine", Path(__file__).with_name("pod_workload_p.py")
)
...
WORKFLOW_QUEUE = "q04-pod-cgroup-ao-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-ao-08"
EVIDENCE_ROOT = Path("/q04-evidence/q04-relationship-pod-cgroup-20260924-ao")
...
def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [
        ...
        str(args.workspace / "tests/pdf_processing/q04/pod-topology-v40/RUNTIME-INTEGRATION-MANIFEST.json"),
...
def adapt_run():
    ...
    source = source.replace(
        old, "pod-topology-v40/RUNTIME-INTEGRATION-MANIFEST.json"
    )
