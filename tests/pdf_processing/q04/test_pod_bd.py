"""Local contracts for the BD worker-process drain window."""
...
from candidate import process_drain_window_bd as matrix
import consumer
from host_bc import Host
from prepare import verify_bundle
import pod_topology_bd as topology
import pod_workload_bd as workload
from sentinel import run_process_drain_pod_cgroup_bd as runner
from sentinel import run_process_drain_pod_cgroup_bd_guarded as guarded
from sentinel import node_pressure_attribution_bd as pressure
...
class ProcessDrainTest(unittest.TestCase):
    ...
    def test_scope_identity_and_exact_command(self):
        ...
        path = topology.base.Q04 / 'pod-topology-v55/RUNTIME-INTEGRATION-MANIFEST.json'
        ...
        self.assertEqual(guarded.PROBE_NAME, 'q04-host-pressure-bd-20260925')
        ...
        self.assertIn('run_process_drain_pod_cgroup_bd_guarded.py', command[1])
    ...
    def test_final_drain_waits_for_large_inventory_member(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            document = 'state/process-drain-pod-cgroup-bd/drain-native/document.json'
    ...
    def test_bd_host_signals_only_exact_owned_worker_and_force_stops_it(self):
        import psutil
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            script_dir = root / 'q04'
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, '-c',
                'import pod_preflight_bd; from pathlib import Path; '
                'pod_preflight_bd.verify_workload_imports_at('
