...
PHASE = "yolo-pod-cgroup-c"
RUN_ID = "q04-yolo-pod-cgroup-20260919-c"
WORKFLOW_QUEUE = "q04-pod-cgroup-c-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-c-07"
PARSER_BUDGETS = {
    "startup_seconds": 120,
...
def no_inference_preflight(
    ...
    required = (
        workspace / "src/pdf_processing/__init__.py",
        workspace / "tests/pdf_processing/q04/q04_runtime.py",
        workspace / "tests/pdf_processing/q04/candidate/yolo_reviewed_window.py",
        model_cache,
    )
    unreadable = [str(path) for path in required if not os.access(path, os.R_OK)]
    if unreadable:
        raise ValueError("UID cannot read fixed runtime paths: " + repr(unreadable))
    if os.getuid() != 1000 or os.getgid() != 1000:
        raise ValueError("worker Pod must run as UID/GID 1000")
    ...
    if len(model_files) != 35 or any(not os.access(path, os.R_OK) for path in model_files):
        raise ValueError("frozen model cache file identity/readability changed")
    packages = ("boto3", "temporalio", "PIL", "psutil")
    ...
    probe = control / ".write-probe"
    probe.write_bytes(b"")
    probe.unlink()
    return {
        "status": "PASS_NO_INFERENCE",
        "uid": os.getuid(),
        "gid": os.getgid(),
        "readable": [str(path) for path in required],
        "model_file_count": len(model_files),
        "packages": list(packages),
        "memory_max": memory_max,
        "oom_counters": {name: events.get(name, 0) for name in ("oom", "oom_kill", "oom_group_kill")},
...
def build_init_argv(args) -> list[str]:
    return [
        args.python,
        str(args.workspace / "tests/pdf_processing/q04/pod_init_c.py"),
...
def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    ...
    return [
        ...
        "--integration-manifest", str(q04 / "pod-topology-v1/INTEGRATION-MANIFEST.json"),
...
def cleanup_markers(state: Path) -> dict:
    roots = sorted(state.glob("yolo-pod-cgroup-c/worker-*"))
