...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_preflight_al_engine", Path(__file__).with_name("pod_preflight_q.py")
)
...
TOPOLOGY_PHASE = "q04-pod-cgroup-al"
RUN_ID = "q04-profile-pod-cgroup-20260924-al"
sha256 = base.base.sha256


def verify_configuration_al(**kwargs):
...
def verify_workload_imports_al(*, workspace, bundle, prefix):
    modules = (
        "candidate.profile_reuse_window_al",
        "pod_init_p",
        "pod_workload_al",
        "pod_remote_evidence_al",
    )
    for module in modules:
        importlib.import_module(module)
    from candidate.warm_pod_window_r import validate_contract
    from candidate.profile_reuse_window_al import validate_scope
    ...
    path = workspace / "tests/pdf_processing/q04/pod-topology-v37/RUNTIME-INTEGRATION-MANIFEST.json"
...
base.RUN_ID = RUN_ID
base.verify_configuration_q = verify_configuration_al
base.verify_workload_imports_q = verify_workload_imports_al
main = base.main
