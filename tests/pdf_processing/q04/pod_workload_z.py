...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_workload_z_engine", Path(__file__).with_name("pod_workload_p.py")
)
...
WORKFLOW_QUEUE = "q04-pod-cgroup-z-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-z-08"
EVIDENCE_ROOT = Path("/q04-evidence/q04-warm-pod-cgroup-20260921-z")
...
def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [
        ...
        str(args.workspace / "tests/pdf_processing/q04/pod-topology-v25/RUNTIME-INTEGRATION-MANIFEST.json"),
...
def adapt_run():
    ...
    source = source.replace(
        old, "pod-topology-v25/RUNTIME-INTEGRATION-MANIFEST.json"
    )
