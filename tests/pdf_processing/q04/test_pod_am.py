...
from candidate.method_invalidation_window_am import measurement_contract, validate_invalidation, validate_scope
from candidate.warm_v3_reference import reviewed_reference_checker, verify_v3_bundle
from consumer import check_reference
import pod_preflight_am as preflight
import pod_remote_evidence_am as evidence
import pod_topology_am as topology
import pod_workload_am as workload
from sentinel import run_method_pod_cgroup_am as runner
...
class AMProfilePodAdapterTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_argv_and_inactive_topology(self):
        manifest = json.loads((topology.base.Q04 / "pod-topology-v38/RUNTIME-INTEGRATION-MANIFEST.json").read_text())
        ...
        self.assertIn("pod_workload_am.py", runner.workload_argv()[1])
        self.assertIn("pod_preflight_am.py", runner.preflight_argv()[1])
        ...
        self.assertIn("candidate.method_invalidation_window_am", measurement)
        self.assertIn("pod-topology-v38/RUNTIME-INTEGRATION-MANIFEST.json",
                      " ".join(measurement))
    ...
    def test_preflight_contract_and_stale_bundle_rejection(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(preflight.verify_configuration_am(**kwargs)[1]["status"], "PASS")
            with self.assertRaisesRegex(ValueError, "frozen bundle inputs changed"):
                preflight.verify_configuration_am(**{**kwargs, "expected_bundle_sha256": "0" * 64})
            with self.assertRaisesRegex(ValueError, "capacity configuration changed"):
                ...
                preflight.verify_configuration_am(**kwargs)
    ...
    def test_scope_and_method_invalidation_fail_closed(self):
        path = topology.base.Q04 / "pod-topology-v38/RUNTIME-INTEGRATION-MANIFEST.json"
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, "-c",
                "import pod_preflight_am; from pathlib import Path; "
                "pod_preflight_am.verify_workload_imports_am("
