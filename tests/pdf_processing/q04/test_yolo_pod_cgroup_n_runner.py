...
import pod_topology_n as topology
import pod_remote_evidence_n as remote_evidence
import pod_workload_n as workload
...
class YoloPodCgroupJRunnerTests(unittest.TestCase):
    def test_complete_launch_arguments_parse_at_every_entrypoint(self):
        ...
        preflight = pod_preflight_n.parser().parse_args(runner.preflight_argv()[2:])
        initialized = pod_init_n.parser().parse_args(workload.build_init_argv(args)[2:])
        manifest = json.loads((runner.Q04 / "pod-topology-v13/RUNTIME-INTEGRATION-MANIFEST.json").read_text())
    ...
    def test_new_identity_and_paths_do_not_reuse_c(self):
        ...
        self.assertEqual(scope["run_identity"], "q04-aima-pod-cgroup-20260920-n")
        self.assertEqual(scope["evidence_pvc"], "q04-pod-cgroup-n-evidence-20260920-n")
        self.assertEqual(
            scope["evidence_directory_name"], "q04-aima-pod-cgroup-20260920-n"
        )
        self.assertEqual(runner.EVIDENCE_MOUNT, "/q04-evidence")
        self.assertEqual(
            runner.EVIDENCE,
            "/q04-evidence/q04-aima-pod-cgroup-20260920-n",
        )
    ...
    def test_single_gate_set_precedes_workload_and_uses_staged_inputs(self):
        ...
        self.assertIn("pod_preflight_n.py", argv[1])
        self.assertEqual(
    ...
    def test_topology_is_inactive_and_contains_only_h_identity(self):
        ...
        self.assertIn(
            "tests/pdf_processing/q04/pod-topology-v13/RUNTIME-INTEGRATION-MANIFEST.json",
            manifest["harness"],
        )

    def test_runtime_manifest_keeps_original_aima_scope(self):
        from candidate.aima_pod_window import validate_scope
        from types import SimpleNamespace
        path = runner.Q04 / 'pod-topology-v13/RUNTIME-INTEGRATION-MANIFEST.json'
    ...
    def test_workload_uses_the_h_runtime_manifest_for_validation_and_scope(self):
        ...
        runtime_path = "pod-topology-v13/RUNTIME-INTEGRATION-MANIFEST.json"
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "from pathlib import Path; import pod_preflight_n as p; "
    ...
    def test_generated_manifests_and_offline_check_are_exact(self):
        ...
        self.assertEqual(
            json.loads((runner.Q04 / "pod-topology-v13/SOURCE-MANIFEST.json").read_text()),
