...
from sentinel.object_monitor_bg import ObjectMonitor, RUN_ID
...
class ObjectMonitorTest(unittest.TestCase):
    ...
    def test_failed_remote_stop_still_settles_transport_and_records_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('sentinel.object_monitor_bg.subprocess.run',
                       side_effect=[TimeoutError('remote stop timeout'), None]) as remote, \
                 patch('sentinel.object_monitor_bg.verify_remote_absence'):
    ...
    def test_exited_transport_still_stops_surviving_remote_observer(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            monitor.process = Process()
            monitor.thread = Thread()
            with patch('sentinel.object_monitor_bg.verify_remote_absence',
                       side_effect=[RuntimeError('still running'), None]) as absence, \
                 patch('sentinel.object_monitor_bg.subprocess.run') as remote, \
                 patch('sentinel.object_monitor_bg.object_identity',
