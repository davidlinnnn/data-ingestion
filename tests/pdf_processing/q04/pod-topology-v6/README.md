...
This directory contains the offline G candidate that follows the F workload
import-closure failure. F remains historical: its runner revision, raw
evidence, retained PVC and failure classification are not rewritten. G stages
the missing telemetry dependency and imports the complete workload entrypoint
and Pod support modules as a pre-inference gate.
...
management. The G contract therefore verifies the mount root as a backend-owned
...
live G Pod. No new Pod or PVC was created while preparing it. See
...
locations. The remaining G-specific gap is live execution of this topology.
