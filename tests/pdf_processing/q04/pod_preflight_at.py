...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_preflight_at_engine", Path(__file__).with_name("pod_preflight_q.py")
)
...
TOPOLOGY_PHASE = "q04-pod-cgroup-at"
RUN_ID = "q04-telemetry-loss-pod-cgroup-20260925-at"
...
def verify_workload_imports_at(*, workspace, bundle, prefix):
    modules = (
        "candidate.telemetry_loss_window_at",
        "pod_init_p",
        "pod_workload_at",
        "pod_remote_evidence_at",
        "host_an",
        "worker",
    )
    ...
    path = workspace / "tests/pdf_processing/q04/pod-topology-v45/RUNTIME-INTEGRATION-MANIFEST.json"
    ...
    return {
        ...
        "reviewed_contract": {
            ...
            "fixtures": ["native"],
            "modes": ["guard"],
            "injection": "after five registered pages during active next group",
