"""One-shot pre-inference gate set for the Q04 fixture-08 Pod window H."""
...
WORKLOAD_IMPORTS = (
    "candidate.aima_pod_window",
    "pod_init_n",
    "pod_workload_n",
    "pod_remote_evidence_n",
)
...
def verify_reviewed_contract(*, workspace: Path, bundle: Path, prefix: str) -> dict:
    from candidate.aima_pod_window import validate_contract, validate_scope
    from q04_runtime import profiles
    inputs = json.loads((bundle / 'inputs.json').read_text())
    config = {'parser_budgets': {'startup_seconds':120, 'no_progress_seconds':180, 'terminate_seconds':5, 'reap_seconds':5, 'max_requests':20},
              'profiles': profiles(inputs, {f['id']: {'key':prefix+'sources/'+f['id']+'.pdf'} for f in inputs['fixtures']})}
    validate_contract(config, inputs)
    path = workspace / 'tests/pdf_processing/q04/pod-topology-v13/RUNTIME-INTEGRATION-MANIFEST.json'
    manifest = json.loads(path.read_text())
    validate_scope(argparse.Namespace(integration_manifest=path,
        authorization_scope_sha256=manifest['authorization_scope_sha256'],
        name='aima-pod-cgroup-n', expected_run_id='q04-aima-pod-cgroup-20260920-n',
        expected_prefix=prefix))
    return {'status':'PASS', 'fixture':'08', 'parser_max_requests':20,
            'oracle':'unchanged Q04 AIMA source oracle', 'production_default_changed':False}


def verify_workload_imports(
    *, workspace: Path, bundle: Path, prefix: str,
    reviewed_contract_probe=verify_reviewed_contract,
) -> dict:
    imported = []
    for module_name in WORKLOAD_IMPORTS:
        importlib.import_module(module_name)
        imported.append(module_name)
    contract = reviewed_contract_probe(
        workspace=workspace,
        bundle=bundle,
        prefix=prefix,
    )
    return {"status": "PASS", "modules": imported, "reviewed_contract": contract}
...
def verify_configuration(
    *,
    ...
    if source_manifest.get("phase") != "q04-pod-cgroup-n":
...
def run_preflight(
    *,
    ...
    workload_import_probe=verify_workload_imports,
) -> dict:
    gates = {}
    ...
    capture(
        ...
        lambda: verify_configuration(
            ...
            expected_phase="aima-pod-cgroup-n",
        )[1],
    )
    capture(
        "source_hashes",
