...
TOPOLOGY_PHASE = 'q04-pod-cgroup-bg'
RUN_ID = 'q04-pod-loss-pod-cgroup-20260925-bg'


def verify_workload_imports(*, workspace: Path, bundle: Path, prefix: str):
    modules = ('candidate.pod_loss_window_bg', 'pod_loss_bridge_bg',
               'pod_workload_bg', 'pod_init_p', 'worker_bc')
    for module in modules:
        importlib.import_module(module)
    from candidate.pod_loss_window_bg import validate_scope
    ...
    path = workspace / 'tests/pdf_processing/q04/pod-topology-v58/RUNTIME-INTEGRATION-MANIFEST.json'
