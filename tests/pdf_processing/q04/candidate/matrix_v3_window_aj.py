"""Run YOLO/AIMA/ACL/Keynote fresh, restored and replay under the v3 oracle."""
...
def validate_scope(args):
    ...
    require(scope == {
        ...
        "measurement": "complete process attribution and AH warm equality for 07/08",
...
def measurement_contract(outcome, summary, gate):
    return {
        "workload": "YOLO/AIMA/ACL/Keynote fresh/restored/exact replay",
...
async def run_window(args):
    ...
    q04_runtime.write(root / "reviewed-window-contract.json", {
        "status": "PASS bounded 07/08/09/10 matrix and 07/08 AH warm equality",
        "fixtures": list(FIXTURES), "modes": list(MODES),
