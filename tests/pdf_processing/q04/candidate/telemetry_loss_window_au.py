...
def verify_abort(root):
    ...
    # File mtimes change during forensic transport; stopped.json records the
    # worker's own clock after the guard canceled the owned workflow.
    observed_gap = stopped['time'] - samples[-1]['time']
    ...
    completed_events = [e for e in history if
                        e['eventType'] == 'EVENT_TYPE_WORKFLOW_EXECUTION_COMPLETED']
    require(len(completed_events) <= 1, 'multiple workflow completion events')
    business = None
    if completed_events:
        payloads = completed_events[0]['workflowExecutionCompletedEventAttributes']['result']['payloads']
        require(len(payloads) == 1, 'unexpected workflow result payloads')
        business = json.loads(base64.b64decode(payloads[0]['data']))
        require(business['status'] == 'failed' and not business['processing_complete']
                and not business['canonical_accepted']
                and business['registered_pages'] < business['pages'],
                'interrupted workflow completed successfully')
    ...
    try:
        ...
        try:
            if collector.started:
                collector.observe('owned_cleanup_finished', source='telemetry_loss_window_au',
