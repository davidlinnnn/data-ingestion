...
import time
import consumer
from candidate.aima_image_supplement import compare, REFERENCE_SHA
...
try:
    from sentinel.aima_attribution_telemetry_o import StrictAttributionCollector
except ImportError:
...
def validate_scope(args):
    ...
    require(scope == {'fixture':'08', 'modes':['fresh','restored','replay'],
        'max_requests':20, 'oracle':'Q04 AIMA source oracle plus source-verified 21 image additions',
        'measurement':'confirmed exits and source-bound reap-before-spawn handoff',
...
async def run_window(args):
    ...
    reference_path = args.bundle/'references/08.json'
    reference = json.loads(reference_path.read_text())
    require(sha(reference_path.read_bytes()) == REFERENCE_SHA, 'fixed Q01 reference changed')
    def checked_image_supplement(document, provided_reference, out):
        require(provided_reference == reference, 'AIMA reference changed')
        report = compare(document, provided_reference, args.bundle/'fixtures/08.pdf')
        original_write(out/'source-image-supplement.json', report)
    ...
    consumer.check_reference = checked_image_supplement
    try:
    ...
    original_write(root/'reviewed-window-contract.json', {'status':'PASS bounded fixture-08 only', 'fixture':'08', 'max_requests':20, 'oracle':'Q04 AIMA source oracle plus source-verified 21 image additions', 'measurement':'confirmed exits and source-bound reap-before-spawn handoff', 'resource_gate':gate})
