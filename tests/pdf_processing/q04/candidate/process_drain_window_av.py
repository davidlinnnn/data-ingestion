...
def verify_drain(root, manifest):
    ...
    require(drain['scope'] == 'owned_worker_process'
            ...
            and first['pod']['metadata']['uid'] == second['pod']['metadata']['uid']
            == drain['pod_uid'] and old['parser_absent'] and old['scratch_absent'],
            'process drain changed Pod or left owned work')
    ...
    return {'pages': result['registered_pages'], 'pod_uid': drain['pod_uid'],
    ...
    try:
        ...
        try:
            if collector.started:
                collector.observe('owned_cleanup_finished', source='process_drain_window_av',
