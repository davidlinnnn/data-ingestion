"""Local contracts for the AZ worker-process drain window."""
...
from candidate import process_drain_window_az as matrix
import consumer
from host_an import Host
import pod_topology_az as topology
import pod_workload_az as workload
from sentinel import run_process_drain_pod_cgroup_az as runner
from sentinel import run_process_drain_pod_cgroup_az_guarded as guarded
from sentinel import node_pressure_attribution_az as pressure
...
class ProcessDrainTest(unittest.TestCase):
    ...
    def test_scope_identity_and_exact_command(self):
        path = topology.base.Q04 / 'pod-topology-v51/RUNTIME-INTEGRATION-MANIFEST.json'
        ...
        self.assertEqual(guarded.PROBE_NAME, 'q04-host-pressure-az-20260925')
        ...
        self.assertIn('run_process_drain_pod_cgroup_az_guarded.py', command[1])
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, '-c',
                'import pod_preflight_az; from pathlib import Path; '
                'pod_preflight_az.verify_workload_imports_at('
