...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_preflight_aj_engine", Path(__file__).with_name("pod_preflight_q.py")
)
...
TOPOLOGY_PHASE = "q04-pod-cgroup-aj"
RUN_ID = "q04-matrix-pod-cgroup-20260924-aj"
sha256 = base.base.sha256


def verify_configuration_aj(**kwargs):
...
def verify_workload_imports_aj(*, workspace, bundle, prefix):
    modules = (
        "candidate.matrix_v3_window_aj",
        "pod_init_p",
        "pod_workload_aj",
        "pod_remote_evidence_aj",
    )
    for module in modules:
        importlib.import_module(module)
    from candidate.warm_pod_window_r import validate_contract
    from candidate.matrix_v3_window_aj import validate_scope
    ...
    path = workspace / "tests/pdf_processing/q04/pod-topology-v35/RUNTIME-INTEGRATION-MANIFEST.json"
...
base.RUN_ID = RUN_ID
base.verify_configuration_q = verify_configuration_aj
base.verify_workload_imports_q = verify_workload_imports_aj
main = base.main
