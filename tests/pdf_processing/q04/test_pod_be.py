"""Local contracts for the BE worker-process drain window."""
...
from candidate import process_drain_window_be as matrix
import consumer
from host_be import Host
from prepare import verify_bundle
import pod_topology_be as topology
import pod_workload_be as workload
from sentinel import run_process_drain_pod_cgroup_be as runner
from sentinel import run_process_drain_pod_cgroup_be_guarded as guarded
from sentinel import node_pressure_attribution_be as pressure
...
class ProcessDrainTest(unittest.TestCase):
    ...
    def test_scope_identity_and_exact_command(self):
        ...
        path = topology.base.Q04 / 'pod-topology-v56/RUNTIME-INTEGRATION-MANIFEST.json'
        ...
        self.assertEqual(guarded.PROBE_NAME, 'q04-host-pressure-be-20260925')
        ...
        self.assertIn('run_process_drain_pod_cgroup_be_guarded.py', command[1])
    ...
    def test_final_drain_waits_for_large_inventory_member(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            document = 'state/process-drain-pod-cgroup-be/drain-native/document.json'
            (root / 'durable-terminal-manifest.json').write_text(json.dumps({
                'inventory': [{'path': document, 'bytes': 22539322}]}))
            mirror = type('Mirror', (), {'root': root,
                'complete': set(runner.FINAL_REQUIRED)})()
            self.assertFalse(runner.all_inventory_copied(mirror))
            mirror.complete.add(document)
            self.assertTrue(runner.all_inventory_copied(mirror))
            self.assertIn('all_inventory_copied',
                          runner.adapt_execute_window().__code__.co_names)
    ...
    def test_be_host_signals_only_exact_owned_worker_and_force_stops_it(self):
        import psutil
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            script_dir = root / 'q04'
            ...
            host.current = state
            for name, authorized in (('worker_bc.py', True), ('worker.py', False)):
                script = script_dir / name
                script.write_text('import time\ntime.sleep(30)\n')
                process = subprocess.Popen([sys.executable, str(script)])
                try:
                    deadline = time.monotonic() + 3
                    while str(script) not in psutil.Process(process.pid).cmdline():
                        self.assertLess(time.monotonic(), deadline)
                        time.sleep(.01)
                    identity = {'pid': process.pid,
                                'created': psutil.Process(process.pid).create_time()}
                    (state / 'ready.json').write_text(json.dumps(identity))
                    (state / 'ownership.json').write_text(json.dumps(identity))
                    if authorized:
            ...
            (state / 'ownership.json').write_text(json.dumps({
                'pid': 7, 'created': 123.0}))
            with patch('host_bc.subprocess.check_output', return_value='{"forced":true}') as command:
                host.force_stop()
            self.assertIn("'q04/worker_bc.py'", command.call_args.args[0][2])
            self.assertIn('p.create_time()==created', command.call_args.args[0][2])
            self.assertTrue((state / 'forced-cleanup.json').is_file())

    def test_signal_helper_is_locked_for_its_full_process_lifetime(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            lock = root / 'lifecycle.lock'
            state = root / 'worker-1'
            state.mkdir()
            (state / 'ready.json').write_text(json.dumps({'pid': 101, 'created': 1.0}))
            host = Host(root / 'config.json', root, {'python': sys.executable})
            host.current = state

            def require_locked(*_args, **_kwargs):
                with lock.open('a') as contender:
                    with self.assertRaises(BlockingIOError):
                        fcntl.flock(contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
                return subprocess.CompletedProcess([], 0)

            with patch.dict(os.environ, {'PDF_PROCESS_LIFECYCLE_LOCK': str(lock),
                    'PDF_PROCESS_LIFECYCLE_LOCK_TIMEOUT_SECONDS': '1'}), \
                    patch('host_bc.subprocess.run', side_effect=require_locked):
                host.signal(101, signal.SIGTERM)
            with lock.open('a') as contender:
                fcntl.flock(contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
                fcntl.flock(contender, fcntl.LOCK_UN)

    def test_emergency_force_stop_does_not_wait_for_sampler_lock(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            lock = root / 'lifecycle.lock'
            state = root / 'worker-1'
            state.mkdir()
            (state / 'ownership.json').write_text(json.dumps({'pid': 101, 'created': 1.0}))
            host = Host(root / 'config.json', root, {'python': sys.executable})
            host.current = state
            with lock.open('a') as blocker, \
                    patch.dict(os.environ, {'PDF_PROCESS_LIFECYCLE_LOCK': str(lock),
                        'PDF_PROCESS_LIFECYCLE_LOCK_TIMEOUT_SECONDS': '0.01'}), \
                    patch('host_bc.subprocess.check_output',
                          return_value='{"forced":true}') as command:
                fcntl.flock(blocker, fcntl.LOCK_EX)
                host.force_stop()
                command.assert_called_once()

    def test_lock_timeout_forces_exact_owned_worker_cleanup(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            lock = root / 'lifecycle.lock'
            host = Host(root / 'config.json', root, {'python': sys.executable})
            with lock.open('a') as blocker, \
                    patch.dict(os.environ, {'PDF_PROCESS_LIFECYCLE_LOCK': str(lock),
                        'PDF_PROCESS_LIFECYCLE_LOCK_TIMEOUT_SECONDS': '0.01'}), \
                    patch.object(host, 'force_stop') as force_stop:
                fcntl.flock(blocker, fcntl.LOCK_EX)
                with self.assertRaisesRegex(RuntimeError, 'lifecycle lock timed out'):
                    host.signal(101, signal.SIGTERM)
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, '-c',
                'import pod_preflight_be; from pathlib import Path; '
                'pod_preflight_be.verify_workload_imports_at('
