...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_preflight_ai_engine", Path(__file__).with_name("pod_preflight_q.py")
)
...
TOPOLOGY_PHASE = "q04-pod-cgroup-ai"
RUN_ID = "q04-matrix-pod-cgroup-20260924-ai"
sha256 = base.base.sha256


def verify_configuration_ai(**kwargs):
...
def verify_workload_imports_ai(*, workspace, bundle, prefix):
    modules = (
        "candidate.matrix_v3_window",
        "pod_init_p",
        "pod_workload_ai",
        "pod_remote_evidence_ai",
    )
    for module in modules:
        importlib.import_module(module)
    from candidate.warm_pod_window_r import validate_contract
    from candidate.matrix_v3_window import validate_scope
    ...
    path = workspace / "tests/pdf_processing/q04/pod-topology-v34/RUNTIME-INTEGRATION-MANIFEST.json"
...
base.RUN_ID = RUN_ID
base.verify_configuration_q = verify_configuration_ai
base.verify_workload_imports_q = verify_workload_imports_ai
main = base.main
