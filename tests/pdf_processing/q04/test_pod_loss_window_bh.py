...
from candidate.pod_loss_window_bh import SCOPE, verify_drain, validate_workflow_budget
from consumer import canonical, sha


class PodLossProofTest(unittest.TestCase):
    def test_workflow_budget_is_bh_specific_and_finite(self):
        self.assertEqual(validate_workflow_budget({'trial_seconds': 300}), 300)
        with self.assertRaises(ValueError):
            validate_workflow_budget({'trial_seconds': 180})

    def test_frozen_scope_matches_reviewed_manifest(self):
        manifest = json.loads((Path(__file__).with_name('pod-topology-v59') /
    ...
    def test_distinct_pods_can_reuse_worker_pid(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw) / 'evidence' / 'state' / 'pod-loss-bh'
            ...
            values = {
                ...
                root.parents[1] / 'pod-loss-control/2-drain.reply.json': {
                    'run_id': 'q04-bh', 'kind': 'drain', 'generation': 2,
