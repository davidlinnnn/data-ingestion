...
RUN_ID = "q04-yolo-pod-cgroup-20260919-d"
WORKFLOW_QUEUE = "q04-pod-cgroup-d-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-d-07"
EVIDENCE_ROOT = Path("/q04-evidence/q04-yolo-pod-cgroup-20260919-d")
PARSER_BUDGETS = {
    "startup_seconds": 120,
...
def build_init_argv(args) -> list[str]:
    return [
        args.python,
        str(args.workspace / "tests/pdf_processing/q04/pod_init_d.py"),
...
def cleanup_markers(state: Path) -> dict:
    roots = sorted(state.glob("yolo-pod-cgroup-d/worker-*"))
