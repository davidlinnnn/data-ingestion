"""Local contracts for the v3-bound evidence-only reuse Pod adapter."""
...
from candidate.profile_reuse_window_al import measurement_contract, validate_reuse, validate_scope
from candidate.warm_v3_reference import reviewed_reference_checker, verify_v3_bundle
from consumer import check_reference
import pod_preflight_al as preflight
import pod_remote_evidence_al as evidence
import pod_topology_al as topology
import pod_workload_al as workload
from sentinel import run_profile_pod_cgroup_al as runner
...
class ALProfilePodAdapterTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_argv_and_inactive_topology(self):
        manifest = json.loads((topology.base.Q04 / "pod-topology-v37/RUNTIME-INTEGRATION-MANIFEST.json").read_text())
        ...
        self.assertEqual(runner.authorization_scope()["group_requests"], 11)
        self.assertNotEqual(runner.PREFIX, "q04/warm-pod-cgroup-20260924-ah/")
        self.assertIn("pod_workload_al.py", runner.workload_argv()[1])
        self.assertIn("pod_preflight_al.py", runner.preflight_argv()[1])
        ...
        self.assertIn("candidate.profile_reuse_window_al", measurement)
        self.assertIn("pod-topology-v37/RUNTIME-INTEGRATION-MANIFEST.json",
                      " ".join(measurement))
    ...
    def test_preflight_contract_and_stale_bundle_rejection(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(preflight.verify_configuration_al(**kwargs)[1]["status"], "PASS")
            with self.assertRaisesRegex(ValueError, "frozen bundle inputs changed"):
                preflight.verify_configuration_al(**{**kwargs, "expected_bundle_sha256": "0" * 64})
            with self.assertRaisesRegex(ValueError, "capacity configuration changed"):
                ...
                preflight.verify_configuration_al(**kwargs)
    ...
    def test_scope_and_evidence_reuse_fail_closed(self):
        path = topology.base.Q04 / "pod-topology-v37/RUNTIME-INTEGRATION-MANIFEST.json"
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            original = {"id": "native-v1", "method": {"version": 3}, "content_evidence": "original"}
            changed_profile = {"id": "native-v1", "method": {"version": 3}, "content_evidence": "changed"}
            upstream = [{"stage": "group", "operation": str(i)} for i in range(11)] + [
                {"stage": "assembly", "operation": "assembly"}]
            for mode in ("fresh", "evidence", "replay"):
                target = root / f"{mode}-native"
                target.mkdir()
                steps = [{**step, "reused": mode != "fresh"} for step in upstream]
                steps += [{"stage": "component_ocr", "reused": mode == "replay"}
                          for _ in range(7)]
                (target / "accepted.json").write_text(json.dumps({
                    "request": ({**request, "request_id": "new"} if mode == "evidence" else request),
                    "producer": {"version": "v3"},
                    "profile": changed_profile if mode == "evidence" else original,
                    "verified": True,
                    "result": {"status": "complete", "processing_complete": True,
                               "registered_pages": 51, "registered_components": 7,
                               "processing_result": "new" if mode == "evidence" else "original",
                               "steps": steps},
                    "accepted": {"document_sha256": manifest["native_document_sha256"],
                                 "final": {"content_evidence": mode}},
                }))
            validate_reuse(root, manifest)
            record = root / "evidence-native/accepted.json"
            value = json.loads(record.read_text())
            value["result"]["steps"][0]["reused"] = False
            record.write_text(json.dumps(value))
            with self.assertRaisesRegex(ValueError, "checked group/assembly reuse missing"):
                validate_reuse(root, manifest)

    def test_success_contract_seals_and_finalizes_in_al_mirror(self):
        with tempfile.TemporaryDirectory() as raw:
            base = Path(raw)
            root = base / "remote"
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, "-c",
                "import pod_preflight_al; from pathlib import Path; "
                "pod_preflight_al.verify_workload_imports_al("
