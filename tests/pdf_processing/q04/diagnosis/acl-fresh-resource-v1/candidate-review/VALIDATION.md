...
- Candidate rebuild: all five generated artifact SHA-256 values matched the
  immutable private package byte-for-byte. The repository copy of the quality
  oracle, split record and manifest matches that package; the full graph and
  content evidence remain private.
- Candidate verifier: `PASS_REVIEW_PACKAGE_ONLY`; exact graph has 134 nodes and
  fixture-09 quality coverage is 40/69/21.
- Focused tests: 10 passed (`test_acl_option_a_candidate` plus retained graph
  diagnosis tests).
- Full Q04 discovery: 121 passed in 9.503 seconds. The two previously absent
...
- Pyright 1.1.405: 0 errors, 0 warnings, 0 informations for the builder,
  verifier and candidate tests.
