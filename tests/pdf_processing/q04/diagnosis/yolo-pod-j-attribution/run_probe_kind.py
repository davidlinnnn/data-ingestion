...
mode=sys.argv[1];assert mode in ('readiness','startup');p=Path(__file__).parent/mode;p.mkdir(exist_ok=False);k=r.Kubectl();r.verify_outer_identity(k)
r.t09a_health(k,p/'before-health.json',require_idle=True)
name='q04-proc-'+mode+'-20260920'
...
spec['spec']['containers'][0][mode+'Probe']={'exec':{'command':['/bin/true']},'periodSeconds':2,'failureThreshold':15}
...
try:
 ...
 channel=PersistentPython(k.base+['exec','-i',name,'--','/experiment/.venv/bin/python'])
 probe=subprocess.Popen(k.base+['exec','-i',name,'--','/experiment/.venv/bin/python','-c',(p.parent/'probe.py').read_text()],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
 time.sleep(5)
 events=[]
 for i in range(50):
  start=time.time();channel.run('pass');events.append({'start':start,'end':time.time()});time.sleep(.2)
