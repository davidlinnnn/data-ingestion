...
This directory contains the offline E candidate that follows the D control-file
staging failure. D remains historical: its runner revision, raw evidence,
retained PVC and failure classification are not rewritten. E keeps D's proven
application-owned evidence-directory contract and fixes the two binary staging
snippets before another workload attempt.
...
management. The E contract therefore verifies the mount root as a backend-owned
...
live E Pod. No new Pod or PVC was created while preparing it. See
...
locations. The remaining E-specific gap is live execution of this topology.
