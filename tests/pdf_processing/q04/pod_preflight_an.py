...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_preflight_an_engine", Path(__file__).with_name("pod_preflight_q.py")
)
...
TOPOLOGY_PHASE = "q04-pod-cgroup-an"
RUN_ID = "q04-method-pod-cgroup-20260924-an"
sha256 = base.base.sha256


def verify_configuration_an(**kwargs):
...
def verify_workload_imports_an(*, workspace, bundle, prefix):
    modules = (
        "candidate.method_invalidation_window_an",
        "pod_init_p",
        "pod_workload_an",
        "pod_remote_evidence_an",
    )
    for module in modules:
        importlib.import_module(module)
    from candidate.warm_pod_window_r import validate_contract
    from candidate.method_invalidation_window_an import validate_scope
    ...
    path = workspace / "tests/pdf_processing/q04/pod-topology-v39/RUNTIME-INTEGRATION-MANIFEST.json"
...
base.RUN_ID = RUN_ID
base.verify_configuration_q = verify_configuration_an
base.verify_workload_imports_q = verify_workload_imports_an
main = base.main
