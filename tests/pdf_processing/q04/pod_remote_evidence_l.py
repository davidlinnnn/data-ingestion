...
STREAM_FILES = {
    "state/yolo-pod-cgroup-l-measurement/resource-attribution.jsonl",
    "vm-controller.jsonl",
    "workload.log",
}
FINAL_REQUIRED = {
    "state/yolo-pod-cgroup-l/phase-complete.json",
    "state/yolo-pod-cgroup-l/reviewed-window-contract.json",
    "state/yolo-pod-cgroup-l/fresh-index.json",
    "state/yolo-pod-cgroup-l/fresh-07/accepted.json",
    "state/yolo-pod-cgroup-l/fresh-07/document.json",
    "state/yolo-pod-cgroup-l/fresh-07/source-reviewed-equivalence.json",
    "state/yolo-pod-cgroup-l/restored-07/accepted.json",
    "state/yolo-pod-cgroup-l/restored-07/document.json",
    "state/yolo-pod-cgroup-l/restored-07/source-reviewed-equivalence.json",
    "state/yolo-pod-cgroup-l/replay-07/accepted.json",
    "state/yolo-pod-cgroup-l/replay-07/document.json",
    "state/yolo-pod-cgroup-l/replay-07/source-reviewed-equivalence.json",
    "state/yolo-pod-cgroup-l-measurement/resource-attribution.jsonl",
    "state/yolo-pod-cgroup-l-measurement/resource-attribution-summary.json",
    "state/yolo-pod-cgroup-l-measurement/measurement-contract.json",
    "workload-exit.json",
...
ALLOWED_PREFIXES = (
    "state/yolo-pod-cgroup-l/",
    "state/yolo-pod-cgroup-l-measurement/",
)
...
for directory, names, leaves in os.walk(root, onerror=traversal_error):
 here=Path(directory)
 for name in list(names):
  path=here/name; parts=path.relative_to(root).parts
  if path.is_symlink(): raise ValueError('evidence symlink forbidden: '+str(path))
  if (len(parts)==4 and parts[:2]==tuple(prefixes[0].strip('/').split('/'))
      and parts[2].startswith('worker-') and parts[2][7:].isdigit() and parts[3]=='scratch'):
   names.remove(name)
 paths.extend(here/name for name in leaves)
...
class IncrementalEvidenceMirror:
    ...
    def finalize(self, *, require_success: bool) -> dict:
        required = set(FINAL_REQUIRED if require_success else {
            "workload-exit.json", "cleanup-complete.json", "durable-terminal-manifest.json"
        })
        ...
        if (
            ...
            or (require_success and terminal.get("status") != "PASS_CANDIDATE")
            or (require_success and terminal.get("workload_succeeded") is not True)
            or terminal.get("cleanup_complete") is not True
            or type(terminal.get("workload_succeeded")) is not bool
            or terminal.get("status") != (
                "PASS_CANDIDATE" if terminal.get("workload_succeeded") else "INCOMPLETE"
            )
        ...
        if (
            ...
            or inventory_paths != sorted(inventory_paths, key=PurePosixPath)
        ...
        if set(inventory_paths) | {"durable-terminal-manifest.json"} != set(self.remote_sizes):
            raise ValueError("sealed inventory differs from evidence mirror")
