...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_workload_an_engine", Path(__file__).with_name("pod_workload_p.py")
)
...
WORKFLOW_QUEUE = "q04-pod-cgroup-an-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-an-08"
EVIDENCE_ROOT = Path("/q04-evidence/q04-method-pod-cgroup-20260924-an")


def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [
        args.python,
        "-m", "candidate.method_invalidation_window_an",
        ...
        str(args.workspace / "tests/pdf_processing/q04/pod-topology-v39/RUNTIME-INTEGRATION-MANIFEST.json"),
...
def adapt_run():
    ...
    source = source.replace(
        old, "pod-topology-v39/RUNTIME-INTEGRATION-MANIFEST.json"
    )
