...
import pod_topology_o as topology
import pod_remote_evidence_o as remote_evidence
import pod_workload_o as workload
from sentinel import run_yolo_pod_cgroup_o as runner


class YoloPodCgroupJRunnerTests(unittest.TestCase):
    def test_complete_launch_arguments_parse_at_every_entrypoint(self):
        import pod_init_o
        import pod_preflight_o
        from candidate import aima_pod_window
        args = workload.parser().parse_args(runner.workload_argv()[2:])
        preflight = pod_preflight_o.parser().parse_args(runner.preflight_argv()[2:])
        initialized = pod_init_o.parser().parse_args(workload.build_init_argv(args)[2:])
        manifest = json.loads((runner.Q04 / "pod-topology-v14/RUNTIME-INTEGRATION-MANIFEST.json").read_text())
        with patch.object(aima_pod_window, "run_window", new_callable=AsyncMock) as execute:
            aima_pod_window.main(workload.build_measurement_argv(
        ...
        self.assertEqual(args.activity_queue, 'q04-pod-cgroup-o-08')
    ...
    def test_new_identity_and_paths_do_not_reuse_c(self):
        ...
        self.assertEqual(scope["run_identity"], "q04-aima-pod-cgroup-20260920-o")
        self.assertEqual(scope["evidence_pvc"], "q04-pod-cgroup-o-evidence-20260920-o")
        self.assertEqual(
            scope["evidence_directory_name"], "q04-aima-pod-cgroup-20260920-o"
        )
        self.assertEqual(runner.EVIDENCE_MOUNT, "/q04-evidence")
        self.assertEqual(
            runner.EVIDENCE,
            "/q04-evidence/q04-aima-pod-cgroup-20260920-o",
        )
    ...
    def test_single_gate_set_precedes_workload_and_uses_staged_inputs(self):
        ...
        self.assertIn("pod_preflight_o.py", argv[1])
        self.assertEqual(
    ...
    def test_control_json_staging_writes_stdin_bytes_to_binary_files(self):
        source = Path(runner.__file__).read_text()
    ...
    def test_runtime_manifest_keeps_original_aima_scope(self):
        from candidate.aima_pod_window_o import validate_scope
        from types import SimpleNamespace
        path = runner.Q04 / 'pod-topology-v14/RUNTIME-INTEGRATION-MANIFEST.json'
        value = json.loads(path.read_text())
        args = SimpleNamespace(integration_manifest=path, authorization_scope_sha256=value['authorization_scope_sha256'], name=runner.PHASE, expected_run_id=runner.RUN_IDENTITY, expected_prefix=runner.PREFIX)
        validate_scope(args)
        self.assertEqual(value['authorization_scope']['fixture'], '08')
        self.assertEqual(value['authorization_scope']['max_requests'], 20)
        args.expected_prefix = 'q04/historical/'
    ...
    def test_workload_uses_the_h_runtime_manifest_for_validation_and_scope(self):
        ...
        runtime_path = "pod-topology-v14/RUNTIME-INTEGRATION-MANIFEST.json"
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "from pathlib import Path; import pod_preflight_o as p; "
    ...
    def test_generated_manifests_and_offline_check_are_exact(self):
        ...
        self.assertEqual(
            json.loads((runner.Q04 / "pod-topology-v14/SOURCE-MANIFEST.json").read_text()),
