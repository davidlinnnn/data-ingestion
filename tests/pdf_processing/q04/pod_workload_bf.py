...
_spec = importlib.util.spec_from_file_location(
    'q04_pod_workload_bf_engine', Path(__file__).with_name('pod_workload_p.py'))
...
WORKFLOW_QUEUE = 'q04-pod-cgroup-bf-workflows'
ACTIVITY_QUEUE = 'q04-pod-cgroup-bf-native'
EVIDENCE_ROOT = Path('/q04-evidence/q04-pod-loss-pod-cgroup-20260925-bf')


def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [args.python, '-m', 'candidate.pod_loss_window_bf',
        ...
        str(args.workspace / 'tests/pdf_processing/q04/pod-topology-v57/RUNTIME-INTEGRATION-MANIFEST.json'),
...
def _patched(function, old: str, new: str):
    ...
    exec(compile(source.replace(old, new), str(base.__file__) + ':bf', 'exec'), namespace)
...
base.run = _patched(base.run,
    ...
    'pod-topology-v57/RUNTIME-INTEGRATION-MANIFEST.json')
main = base.main
