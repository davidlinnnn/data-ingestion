...
def inventory(expected):
    ...
    rapid=Path(importlib.metadata.distribution('rapidocr').locate_file('rapidocr/models'))
...
async def main(expected):
    result={'captured_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'scope':'READ_ONLY_NO_WORKFLOW_NO_INFERENCE_NO_MUTATION'}
