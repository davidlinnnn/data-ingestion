...
RUN_ID = "q04-yolo-pod-cgroup-20260920-m"
WORKFLOW_QUEUE = "q04-pod-cgroup-m-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-m-07"
EVIDENCE_ROOT = Path("/q04-evidence/q04-yolo-pod-cgroup-20260920-m")
PARSER_BUDGETS = {
    "startup_seconds": 120,
...
def build_init_argv(args) -> list[str]:
    return [
        args.python,
        str(args.workspace / "tests/pdf_processing/q04/pod_init_m.py"),
...
def cleanup_markers(state: Path) -> dict:
    roots = sorted(root for root in state.glob("yolo-pod-cgroup-m/worker-*") if root.is_dir())
