"""Offline contracts for the follow-up Q04 Pod cgroup H runner."""
...
import pod_topology_h as topology
import pod_remote_evidence_h as remote_evidence
import pod_workload_h as workload
from sentinel import run_yolo_pod_cgroup_h as runner


class YoloPodCgroupHRunnerTests(unittest.TestCase):
    ...
    def test_new_identity_and_paths_do_not_reuse_c(self):
        ...
        self.assertEqual(scope["run_identity"], "q04-yolo-pod-cgroup-20260920-h")
        self.assertEqual(scope["evidence_pvc"], "q04-pod-cgroup-h-evidence-20260920-h")
        self.assertEqual(
            scope["evidence_directory_name"], "q04-yolo-pod-cgroup-20260920-h"
        )
    ...
    def test_single_gate_set_precedes_workload_and_uses_staged_inputs(self):
        ...
        self.assertIn("pod_preflight_h.py", argv[1])
        self.assertEqual(
    ...
    def test_topology_is_inactive_and_contains_only_h_identity(self):
        ...
        self.assertIn(
            "tests/pdf_processing/q04/pod-topology-v7/RUNTIME-INTEGRATION-MANIFEST.json",
            manifest["harness"],
        )
    ...
    def test_runtime_manifest_changes_only_the_window_identity(self):
        ...
        runtime = json.loads(
            (
                runner.Q04
                / "pod-topology-v7/RUNTIME-INTEGRATION-MANIFEST.json"
            ).read_text()
        )
        ...
        self.assertEqual(runtime_identity, {
            "driver_lock": "/q04-control/yolo-pod-cgroup-h.driver.lock",
            "local_output": "/private/tmp/q04-yolo-pod-cgroup-20260920-h",
    ...
    def test_workload_uses_the_h_runtime_manifest_for_validation_and_scope(self):
        ...
        runtime_path = "pod-topology-v7/RUNTIME-INTEGRATION-MANIFEST.json"
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "from pathlib import Path; import pod_preflight_h as p; "
    ...
    def test_generated_manifests_and_offline_check_are_exact(self):
        ...
        self.assertEqual(
            json.loads((runner.Q04 / "pod-topology-v7/SOURCE-MANIFEST.json").read_text()),
