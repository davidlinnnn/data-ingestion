"""Local contracts for the AY worker-process drain window."""
...
from candidate import process_drain_window_ay as matrix
import consumer
from host_an import Host
import pod_topology_ay as topology
import pod_workload_ay as workload
from sentinel import run_process_drain_pod_cgroup_ay as runner
from sentinel import run_process_drain_pod_cgroup_ay_guarded as guarded
from sentinel import node_pressure_attribution_ay as pressure
...
class ProcessDrainTest(unittest.TestCase):
    ...
    def test_scope_identity_and_exact_command(self):
        path = topology.base.Q04 / 'pod-topology-v50/RUNTIME-INTEGRATION-MANIFEST.json'
        ...
        self.assertEqual(guarded.PROBE_NAME, 'q04-host-pressure-ay-20260925')
        ...
        self.assertIn('run_process_drain_pod_cgroup_ay_guarded.py', command[1])
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, '-c',
                'import pod_preflight_ay; from pathlib import Path; '
                'pod_preflight_ay.verify_workload_imports_at('
