...
from candidate.pod_loss_window_bg import SCOPE, verify_drain
...
class PodLossProofTest(unittest.TestCase):
    def test_frozen_scope_matches_reviewed_manifest(self):
        manifest = json.loads((Path(__file__).with_name('pod-topology-v58') /
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            values = {
                ...
                root.parents[1] / 'pod-loss-control/2-drain.reply.json': {
                    'run_id': 'q04-bg', 'kind': 'drain', 'generation': 2,
