...
class BFOfflineTest(unittest.TestCase):
    ...
    def test_wrong_scope_cannot_create_cluster_objects(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(bf, 'RUNNER_MANIFEST', manifest), \
    ...
    def test_completed_bf_identity_cannot_run_again(self):
    ...
    def test_failure_export_follows_worker_cleanup(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(bf, 'OUT', out), \
                 ...
                 patch.object(bf.reviewed, 'verify_held_deployments'), \
                 patch.object(bf.reviewed, 't09a_health'), \
                 patch.object(bf.object_trial, 'capture', return_value={
                     'old_pod': {'pod_uid': 'old'}, 'trial_pod': None}), \
                 patch.object(bf.object_trial, 'enter'), \
                 patch.object(bf.object_trial, 'restore', return_value={
                     'restored_limit': '512Mi'}), \
                 patch.object(bf, 'setup', return_value=({}, {}, {},
                     {'pod_name': 'coordinator'}, {})), \
                 patch.object(bf, 'MailboxController'), \
                 patch.object(bf, 'RuntimeMonitor') as monitor, \
                 patch.object(bf, 'ObjectMonitor', return_value=BrokenObserver()), \
                 patch.object(bf, 'cleanup_owned_deployments', side_effect=cleanup), \
                 patch.object(bf, 'export_evidence', side_effect=export):
                monitor.return_value.stop.return_value = None
                with self.assertRaisesRegex(ValueError, 'synthetic observer failure'):
                    bf.execute(args)
        self.assertEqual(events, [(bf.topology.DEPLOYMENT,), 'export',
                                  (bf.topology.COORDINATOR_DEPLOYMENT,)])
