...
class ActivityControlTest(unittest.TestCase):
    ...
    def test_activity_exec_disables_thp_before_worker_spawn(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            marker = root.parent / 'pod-loss-pod-cgroup-bg-measurement/worker-1/activity-memory-policy.json'
            ...
            with patch.object(bg, 'disable_thp', create=True,
                              return_value={'thp_disabled': 1,
                                            'inherited_by_children': True}) as policy, \
                 patch.object(bg, 'StrictAttributionCollector') as collector, \
                 patch.object(Path, 'read_text', new=read), \
                 patch.object(bg.subprocess, 'Popen', side_effect=spawn):
                with self.assertRaisesRegex(RuntimeError, 'stop before worker spawn'):
                    asyncio.run(bg.run(config, root, 1))
    ...
    def test_worker_proof_and_parser_hold_are_published_in_process(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            measurement = root.parent / 'pod-loss-pod-cgroup-bg-measurement/worker-1'
            ...
            child = SimpleNamespace(pid=456, parents=lambda: [parent],
                ...
                status=lambda: bg.psutil.STATUS_STOPPED)
            def process(pid):
                return {123: parent, 456: child}[pid]
            with patch.object(bg.psutil, 'Process', side_effect=process):
                self.assertTrue(bg.publish_worker_proof(root, measurement, 1,
            ...
            with patch.object(bg.psutil, 'Process', side_effect=process), \
                 patch.object(bg.os, 'kill') as kill:
                bg.hold_requested_parser(root, measurement, 1,
                    SimpleNamespace(pid=123))
            kill.assert_called_once_with(456, bg.signal.SIGSTOP)
