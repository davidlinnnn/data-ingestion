"""Evidence paths for the v3-bound Q04 warm sequence."""
...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_remote_evidence_ap_engine",
...
FINAL_REQUIRED = {
    ...
    *{
        ...
        for name in ("accepted.json", "document.json", "history.json")
    },
    *{
        f"state/{PHASE}/interrupted-native/{name}"
        for name in ("admission.json", "result.json", "failure.json",
                     "failure-history.json", "cleanup.json", "publication-outcome.json")
    },
    *{
        f"state/{PHASE}/worker-1/{name}"
        for name in ("interruption-result.json", "interruption/observation.json",
                     "interruption/partial-manifest.json", "stopped.json")
    },
    f"state/{MEASUREMENT}/resource-attribution.jsonl",
