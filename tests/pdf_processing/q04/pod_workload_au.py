...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_workload_au_engine", Path(__file__).with_name("pod_workload_p.py")
)
...
WORKFLOW_QUEUE = "q04-pod-cgroup-au-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-au-08"
EVIDENCE_ROOT = Path("/q04-evidence/q04-telemetry-loss-pod-cgroup-20260925-au")


def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [
        args.python,
        "-m", "candidate.telemetry_loss_window_au",
        ...
        str(args.workspace / "tests/pdf_processing/q04/pod-topology-v46/RUNTIME-INTEGRATION-MANIFEST.json"),
...
def adapt_run():
    ...
    source = source.replace(
        old, "pod-topology-v46/RUNTIME-INTEGRATION-MANIFEST.json"
    )
