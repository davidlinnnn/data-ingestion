"""One-shot pre-inference gate set for the Q04 fixture-07 Pod window H."""
...
WORKLOAD_IMPORTS = (
    "candidate.yolo_reviewed_window",
    "pod_init_l",
    "pod_workload_l",
    "pod_remote_evidence_l",
)
...
def verify_reviewed_contract(*, workspace: Path, bundle: Path, prefix: str) -> dict:
    reviewed = importlib.import_module("candidate.yolo_reviewed_window")
    inputs = json.loads((bundle / "inputs.json").read_text())
    q04 = workspace / "tests/pdf_processing/q04"
    integration_manifest = (
        q04 / "pod-topology-v11/RUNTIME-INTEGRATION-MANIFEST.json"
    )
    ...
    args = argparse.Namespace(
        bundle=bundle,
        name="yolo-pod-cgroup-l",
        fixture="07",
        modes=["fresh", "restored", "replay"],
        expected_run_id="q04-yolo-pod-cgroup-20260920-l",
        ...
        resource_analysis=(
            q04 / "diagnosis/yolo-lifecycle-a/evidence/resource-peak.json"
        ),
        historical_methods=(
            workspace
            / "tests/pdf_processing/t09a_r3/evidence/20260914-0b537d0-c/actual-methods.json"
        ),
        authorization_scope_sha256=integration["authorization_scope_sha256"],
    )
    config = {
        "parser_budgets": parser_budgets,
        "profiles": {"07": {"method": inputs["base_profile"]["method"]}},
    }
    result = reviewed.validate_reviewed_inputs(args, config, inputs)
    return {
        "status": "PASS",
        "runtime_identity": integration["identity"],
...
def verify_configuration(
    *,
    ...
    if source_manifest.get("phase") != "q04-pod-cgroup-l":
    ...
    *,
    ...
    capture(
        ...
        lambda: verify_configuration(
            ...
            expected_phase="yolo-pod-cgroup-l",
        )[1],
    )
    capture(
        "source_hashes",
