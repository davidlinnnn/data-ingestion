"""Local contracts for the v3-bound changed-profile rejection Pod adapter."""
...
from candidate.profile_rejection_window_ak import measurement_contract, validate_rejection, validate_scope
from candidate.warm_v3_reference import reviewed_reference_checker, verify_v3_bundle
from consumer import check_reference
import pod_preflight_ak as preflight
import pod_remote_evidence_ak as evidence
import pod_topology_ak as topology
import pod_workload_ak as workload
from sentinel import run_profile_pod_cgroup_ak as runner
...
class AKProfilePodAdapterTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_argv_and_inactive_topology(self):
        manifest = json.loads((topology.base.Q04 / "pod-topology-v36/RUNTIME-INTEGRATION-MANIFEST.json").read_text())
        ...
        self.assertIn("pod_workload_ak.py", runner.workload_argv()[1])
        self.assertIn("pod_preflight_ak.py", runner.preflight_argv()[1])
        ...
        self.assertIn("candidate.profile_rejection_window_ak", measurement)
        self.assertIn("pod-topology-v36/RUNTIME-INTEGRATION-MANIFEST.json",
                      " ".join(measurement))
    ...
    def test_preflight_contract_and_stale_bundle_rejection(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(preflight.verify_configuration_ak(**kwargs)[1]["status"], "PASS")
            with self.assertRaisesRegex(ValueError, "frozen bundle inputs changed"):
                preflight.verify_configuration_ak(**{**kwargs, "expected_bundle_sha256": "0" * 64})
            with self.assertRaisesRegex(ValueError, "capacity configuration changed"):
                ...
                preflight.verify_configuration_ak(**kwargs)
    ...
    def test_scope_and_old_request_rejection_fail_closed(self):
        path = topology.base.Q04 / "pod-topology-v36/RUNTIME-INTEGRATION-MANIFEST.json"
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            for mode in ("fresh", "old-request", "replay"):
                target = root / f"{mode}-native"
                target.mkdir()
                (target / "accepted.json").write_text(json.dumps({
                    "request": request,
                    "producer": {"version": "v3"},
                    "profile": changed_profile if mode == "old-request" else original,
                    "verified": True,
                    "result": ({"status": "failed", "processing_complete": False,
                                "error": {"code": "worker_method_mismatch"}}
                               if mode == "old-request" else
                               {"status": "complete", "processing_result": "original"}),
                    "accepted": {"document_sha256": manifest["native_document_sha256"]},
                }))
            validate_rejection(root, manifest)
            record = root / "old-request-native/accepted.json"
            value = json.loads(record.read_text())
            value["result"]["status"] = "complete"
            record.write_text(json.dumps(value))
            with self.assertRaisesRegex(ValueError, "changed profile accepted old plan"):
                validate_rejection(root, manifest)

    def test_success_contract_seals_and_finalizes_in_ak_mirror(self):
        with tempfile.TemporaryDirectory() as raw:
            base = Path(raw)
            root = base / "remote"
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, "-c",
                "import pod_preflight_ak; from pathlib import Path; "
                "pod_preflight_ak.verify_workload_imports_ak("
