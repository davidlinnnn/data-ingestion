...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_workload_ba_engine", Path(__file__).with_name("pod_workload_p.py")
)
...
WORKFLOW_QUEUE = "q04-pod-cgroup-ba-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-ba-08"
EVIDENCE_ROOT = Path("/q04-evidence/q04-process-drain-pod-cgroup-20260925-ba")


def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [
        args.python,
        "-m", "candidate.process_drain_window_ba",
        ...
        str(args.workspace / "tests/pdf_processing/q04/pod-topology-v52/RUNTIME-INTEGRATION-MANIFEST.json"),
...
def adapt_run():
    ...
    source = source.replace(
        old, "pod-topology-v52/RUNTIME-INTEGRATION-MANIFEST.json"
    )
