...
RUN_ID = "q04-aima-pod-cgroup-20260920-p"
WORKFLOW_QUEUE = "q04-pod-cgroup-p-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-p-08"
EVIDENCE_ROOT = Path("/q04-evidence/q04-aima-pod-cgroup-20260920-p")
PARSER_BUDGETS = {
    "startup_seconds": 120,
...
def build_init_argv(args) -> list[str]:
    return [
        args.python,
        str(args.workspace / "tests/pdf_processing/q04/pod_init_p.py"),
...
def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [args.python, '-m', 'candidate.aima_pod_window_o',
        ...
        '--integration-manifest', str(args.workspace / 'tests/pdf_processing/q04/pod-topology-v15/RUNTIME-INTEGRATION-MANIFEST.json'),
...
def cleanup_markers(state: Path) -> dict:
    roots = sorted(root for root in state.glob("aima-pod-cgroup-p/worker-*") if root.is_dir())
