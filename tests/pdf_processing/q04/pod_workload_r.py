"""Pod-local supervisor for the original Q04 warm sequence."""
...
_spec = importlib.util.spec_from_file_location("q04_pod_workload_r_engine", Path(__file__).with_name("pod_workload_p.py"))
...
RUN_ID = "q04-warm-pod-cgroup-20260921-r"
WORKFLOW_QUEUE = "q04-pod-cgroup-r-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-r-08"
EVIDENCE_ROOT = Path("/q04-evidence/q04-warm-pod-cgroup-20260921-r")


def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [
        args.python,
        "-m", "candidate.warm_pod_window_r",
        ...
        str(args.workspace / "tests/pdf_processing/q04/pod-topology-v17/RUNTIME-INTEGRATION-MANIFEST.json"),
...
def adapt_run():
    ...
    exec(compile(source.replace(old, "pod-topology-v17/RUNTIME-INTEGRATION-MANIFEST.json"), str(base.__file__) + ":r", "exec"), namespace)
