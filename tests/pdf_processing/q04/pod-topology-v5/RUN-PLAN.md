# Q04 YOLO Pod cgroup F single-window plan

This is an offline candidate for one future 1,500-second fixture 07
fresh/restored/exact-replay window. It creates a new F identity and a new PVC;
it does not reuse, write or delete the retained A, C, D or E PVCs. A later runtime
...
- Run: `q04-yolo-pod-cgroup-20260920-f`
- Object prefix: `q04/yolo-pod-cgroup-20260920-f/`
- Deployment: `q04-pod-cgroup-f-activities`, initially zero replicas
- PVC: `q04-pod-cgroup-f-evidence-20260920-f`, 1 GiB, retained
- Application evidence directory:
  `/q04-evidence/q04-yolo-pod-cgroup-20260920-f`
- Authorization scope:
...
3. Create two immutable ConfigMaps, the new F PVC and an inactive Deployment.
...
6. Check the backend-owned mount root is empty, then let UID/GID 1000 exclusively create
   the F run directory at `0700`. Reject any existing path or symlink. Verify
   ownership, durable write/fsync/atomic rename/readback and capacity.
7. Write the exact PVC/PV/directory identity once in the F directory. Copy the
...
8. Execute every row in `PRE-INFERENCE-GATES.md` once, without fail-fast hiding
   later observations. Persist the complete result locally and inside the F
...
11. Stop the local transport, scale and UID-delete the Pod/Deployment, delete
    the two exact ConfigMaps, retain only the F PVC, then recheck OOM/PSI,
...
All exits keep the F PVC, remove only UID-fenced F runtime objects, and leave the
...
prove. Historical A/C/D/E PVCs, runners and evidence remain unchanged.

The earliest live-only uncertainty is UID/GID 1000 creating the run-owned child
on the new F mount. A read-only inspection of the pinned image confirmed all 17
