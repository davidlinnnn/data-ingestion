"""Local contracts for the BA worker-process drain window."""
...
from candidate import process_drain_window_ba as matrix
import consumer
from host_an import Host
import pod_topology_ba as topology
import pod_workload_ba as workload
from sentinel import run_process_drain_pod_cgroup_ba as runner
from sentinel import run_process_drain_pod_cgroup_ba_guarded as guarded
from sentinel import node_pressure_attribution_ba as pressure
...
class ProcessDrainTest(unittest.TestCase):
    ...
    def test_scope_identity_and_exact_command(self):
        path = topology.base.Q04 / 'pod-topology-v52/RUNTIME-INTEGRATION-MANIFEST.json'
        ...
        self.assertEqual(guarded.PROBE_NAME, 'q04-host-pressure-ba-20260925')
        ...
        self.assertIn('run_process_drain_pod_cgroup_ba_guarded.py', command[1])
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, '-c',
                'import pod_preflight_ba; from pathlib import Path; '
                'pod_preflight_ba.verify_workload_imports_at('
