"""Local contracts for the BC worker-process drain window."""
...
from candidate import process_drain_window_bc as matrix
import consumer
from host_bc import Host
from prepare import verify_bundle
import pod_topology_bc as topology
import pod_workload_bc as workload
from sentinel import run_process_drain_pod_cgroup_bc as runner
from sentinel import run_process_drain_pod_cgroup_bc_guarded as guarded
from sentinel import node_pressure_attribution_bc as pressure
...
class ProcessDrainTest(unittest.TestCase):
    ...
    def test_scope_identity_and_exact_command(self):
        ...
        path = topology.base.Q04 / 'pod-topology-v54/RUNTIME-INTEGRATION-MANIFEST.json'
        ...
        self.assertEqual(guarded.PROBE_NAME, 'q04-host-pressure-bc-20260925')
        ...
        self.assertIn('run_process_drain_pod_cgroup_bc_guarded.py', command[1])
    ...
    def test_bc_host_signals_only_exact_owned_worker_and_force_stops_it(self):
        import psutil
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            script_dir = root / 'q04'
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, '-c',
                'import pod_preflight_bc; from pathlib import Path; '
                'pod_preflight_bc.verify_workload_imports_at('
