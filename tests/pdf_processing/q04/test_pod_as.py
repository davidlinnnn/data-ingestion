"""Local contracts for the AS observed relationship window."""
...
import pod_topology_as as topology
import pod_workload_as as workload
from sentinel import node_pressure_attribution as probe
from sentinel import run_relationship_pod_cgroup_as as runner
from sentinel import run_relationship_pod_cgroup_as_guarded as guarded


BUNDLE = Path("/private/tmp/q04-inputs-warm-continuation-v3-ah")


class ASObservedRelationshipTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_scope_and_owned_probe_command(self):
        ...
        path = topology.base.Q04 / "pod-topology-v44/RUNTIME-INTEGRATION-MANIFEST.json"
        ...
        self.assertIn("run_relationship_pod_cgroup_as_guarded.py", runner.exact_command())
        ...
        self.assertEqual(guarded.PROBE_NAME, "q04-host-pressure-as-20260925")
    ...
    def test_ocr_phase_trace_preserves_fixture_result(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            command = [sys.executable, '-c',
                       'import json,sys; from pdf_processing.ocr import execute; execute(json.load(sys.stdin))']
            env = os.environ.copy()
            env['PYTHONPATH'] = str(topology.base.ROOT / 'src')
            plain_run = subprocess.run(command, input=json.dumps({**request, 'out': str(root / 'plain')}),
                                       text=True, capture_output=True, env=env)
            self.assertEqual(plain_run.returncode, 0, plain_run.stderr)
            env['PYTHONPATH'] = str(topology.base.Q04 / 'pod-topology-v44') + ':' + env['PYTHONPATH']
            env['PDF_PROCESS_LIFECYCLE_FD'] = '1'
            env['Q04_OCR_TRACE_DIR'] = str(root / 'trace')
            observed_run = subprocess.run(command, input=json.dumps({**request, 'out': str(root / 'observed')}),
                                          text=True, capture_output=True, env=env)
            self.assertEqual(observed_run.returncode, 0, observed_run.stderr)
            ...
            pod_env = topology.kubernetes_list()['items'][3]['spec']['template']['spec']['containers'][0]['env']
            self.assertIn({'name': 'Q04_OCR_TRACE_DIR',
                           'value': runner.authorization_scope()['ocr_phase_trace']['directory']}, pod_env)
            self.assertTrue(next(x['value'] for x in pod_env if x['name'] == 'PYTHONPATH').startswith(
                '/workspace/tests/pdf_processing/q04/pod-topology-v44:'))
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, "-c",
                "import pod_preflight_as; from pathlib import Path; "
                "pod_preflight_as.verify_workload_imports_as("
