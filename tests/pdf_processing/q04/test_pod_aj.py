"""Local contracts for the v3-bound Q04 matrix Pod adapter."""
...
from candidate.matrix_v3_window_aj import measurement_contract, validate_ah_equality, validate_scope
from candidate.warm_v3_reference import reviewed_reference_checker, verify_v3_bundle
from consumer import check_reference
import pod_preflight_aj as preflight
import pod_remote_evidence_aj as evidence
import pod_topology_aj as topology
import pod_workload_aj as workload
from sentinel import run_matrix_pod_cgroup_aj as runner
...
class AJMatrixPodAdapterTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_argv_and_inactive_topology(self):
        manifest = json.loads((topology.base.Q04 / "pod-topology-v35/RUNTIME-INTEGRATION-MANIFEST.json").read_text())
        ...
        self.assertEqual(runner.authorization_scope()["group_requests"], 8)
        self.assertNotEqual(runner.PREFIX, "q04/warm-pod-cgroup-20260924-ah/")
        self.assertIn("pod_workload_aj.py", runner.workload_argv()[1])
        self.assertIn("pod_preflight_aj.py", runner.preflight_argv()[1])
        ...
        self.assertIn("candidate.matrix_v3_window_aj", measurement)
        self.assertIn("pod-topology-v35/RUNTIME-INTEGRATION-MANIFEST.json",
                      " ".join(measurement))
    ...
    def test_preflight_contract_and_stale_bundle_rejection(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(preflight.verify_configuration_aj(**kwargs)[1]["status"], "PASS")
            with self.assertRaisesRegex(ValueError, "frozen bundle inputs changed"):
                preflight.verify_configuration_aj(**{**kwargs, "expected_bundle_sha256": "0" * 64})
            with self.assertRaisesRegex(ValueError, "capacity configuration changed"):
                ...
                preflight.verify_configuration_aj(**kwargs)
    ...
    def test_scope_and_ah_equality_fail_closed(self):
        path = topology.base.Q04 / "pod-topology-v35/RUNTIME-INTEGRATION-MANIFEST.json"
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            mutated["ah_warm_document_sha256"]["08"] = "0" * 64
            ...
            for sid in ("07", "08", "09", "10"):
                for mode in ("fresh", "restored", "replay"):
                    ...
                    (target / "accepted.json").write_text(json.dumps({
                        "request": request,
                        "accepted": {"document_sha256": manifest["ah_warm_document_sha256"].get(sid, "1" * 64)},
                    }))
            validate_ah_equality(root, manifest)
            record = root / "replay-08/accepted.json"
            value = json.loads(record.read_text())
            value["accepted"]["document_sha256"] = "0" * 64
            record.write_text(json.dumps(value))
            with self.assertRaisesRegex(ValueError, "08 replay differs"):
                validate_ah_equality(root, manifest)

    def test_success_contract_seals_and_finalizes_in_aj_mirror(self):
        with tempfile.TemporaryDirectory() as raw:
            base = Path(raw)
            root = base / "remote"
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, "-c",
                "import pod_preflight_aj; from pathlib import Path; "
                "pod_preflight_aj.verify_workload_imports_aj("
