...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_preflight_am_engine", Path(__file__).with_name("pod_preflight_q.py")
)
...
TOPOLOGY_PHASE = "q04-pod-cgroup-am"
RUN_ID = "q04-method-pod-cgroup-20260924-am"
sha256 = base.base.sha256


def verify_configuration_am(**kwargs):
...
def verify_workload_imports_am(*, workspace, bundle, prefix):
    modules = (
        "candidate.method_invalidation_window_am",
        "pod_init_p",
        "pod_workload_am",
        "pod_remote_evidence_am",
    )
    for module in modules:
        importlib.import_module(module)
    from candidate.warm_pod_window_r import validate_contract
    from candidate.method_invalidation_window_am import validate_scope
    ...
    path = workspace / "tests/pdf_processing/q04/pod-topology-v38/RUNTIME-INTEGRATION-MANIFEST.json"
...
base.RUN_ID = RUN_ID
base.verify_configuration_q = verify_configuration_am
base.verify_workload_imports_q = verify_workload_imports_am
main = base.main
