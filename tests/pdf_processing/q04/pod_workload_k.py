...
RUN_ID = "q04-yolo-pod-cgroup-20260920-k"
WORKFLOW_QUEUE = "q04-pod-cgroup-k-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-k-07"
EVIDENCE_ROOT = Path("/q04-evidence/q04-yolo-pod-cgroup-20260920-k")
PARSER_BUDGETS = {
    "startup_seconds": 120,
...
def build_init_argv(args) -> list[str]:
    return [
        args.python,
        str(args.workspace / "tests/pdf_processing/q04/pod_init_k.py"),
...
def cleanup_markers(state: Path) -> dict:
    roots = sorted(root for root in state.glob("yolo-pod-cgroup-k/worker-*") if root.is_dir())
