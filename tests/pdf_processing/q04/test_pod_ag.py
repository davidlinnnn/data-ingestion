"""Regression checks for the AG warm Pod adapter."""
...
import pod_topology_ag as topology
import pod_preflight_ag as preflight
from sentinel import run_warm_pod_cgroup_ag as runner


class AGWarmPodAdapterTest(unittest.TestCase):
    ...
    def test_af_is_new_inactive_fail_stop_identity(self):
        self.assertEqual(runner.RUN_IDENTITY, "q04-warm-pod-cgroup-20260922-ag")
        self.assertEqual(runner.PREFIX, "q04/warm-pod-cgroup-20260922-ag/")
        ...
        self.assertIn("pod_workload_ag.py", runner.workload_argv()[1])
        self.assertIn("pod_preflight_ag.py", runner.preflight_argv()[1])
    ...
    def test_complete_preflight_configuration_gate_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _, result = preflight.verify_configuration_ag(
                capacity=capacity,
                authorization_scope_sha256=runner.authorization_scope_sha256(),
    ...
    def test_preflight_rejects_bundle_with_stale_harness_binding(self):
        source_bundle = Path("/private/tmp/q04-inputs-warm-lifecycle-ag-final")
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            bundle = root / "bundle"
            bundle.mkdir()
            ...
            with self.assertRaisesRegex(ValueError, "bundle harness drift"):
                preflight.verify_configuration_ag(
                    capacity=capacity,
                    authorization_scope_sha256=runner.authorization_scope_sha256(),
                    source_manifest_path=source,
                    bundle=bundle,
    ...
    def test_s_and_w_import_order_is_isolated(self):
        ...
        for modules in (
            "sentinel.run_warm_pod_cgroup_s,sentinel.run_warm_pod_cgroup_ag",
            "sentinel.run_warm_pod_cgroup_ag,sentinel.run_warm_pod_cgroup_s",
        ):
            subprocess.run(
                [
                    sys.executable,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "import pod_preflight_ag; "
                    "import json,tempfile; from pathlib import Path; "
                    "from candidate.warm_pod_window_r import reviewed_reference_checker; "
                    "b=Path('/private/tmp/q04-inputs-warm-lifecycle-ag-final'); "
                    "pod_preflight_ag.verify_workload_imports_ag("
