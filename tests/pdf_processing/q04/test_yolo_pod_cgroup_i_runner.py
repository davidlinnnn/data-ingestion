"""Offline contracts for the follow-up Q04 Pod cgroup I runner."""
...
import pod_topology_i as topology
import pod_remote_evidence_i as remote_evidence
import pod_workload_i as workload
from sentinel import run_yolo_pod_cgroup_i as runner


class YoloPodCgroupIRunnerTests(unittest.TestCase):
    def test_complete_launch_arguments_parse_at_every_entrypoint(self):
        import pod_init_i
        import pod_preflight_i
        from candidate import yolo_reviewed_window
        args = workload.parser().parse_args(runner.workload_argv()[2:])
        preflight = pod_preflight_i.parser().parse_args(runner.preflight_argv()[2:])
        initialized = pod_init_i.parser().parse_args(workload.build_init_argv(args)[2:])
        manifest = json.loads((runner.Q04 / "pod-topology-v8/RUNTIME-INTEGRATION-MANIFEST.json").read_text())
    ...
    def test_new_identity_and_paths_do_not_reuse_c(self):
        ...
        self.assertEqual(scope["run_identity"], "q04-yolo-pod-cgroup-20260920-i")
        self.assertEqual(scope["evidence_pvc"], "q04-pod-cgroup-i-evidence-20260920-i")
        self.assertEqual(
            scope["evidence_directory_name"], "q04-yolo-pod-cgroup-20260920-i"
        )
        self.assertEqual(runner.EVIDENCE_MOUNT, "/q04-evidence")
        self.assertEqual(
            runner.EVIDENCE,
            "/q04-evidence/q04-yolo-pod-cgroup-20260920-i",
        )
    ...
    def test_single_gate_set_precedes_workload_and_uses_staged_inputs(self):
        ...
        self.assertIn("pod_preflight_i.py", argv[1])
        self.assertEqual(
    ...
    def test_runtime_manifest_changes_only_the_window_identity(self):
        ...
        runtime = json.loads(
            (
                runner.Q04
                / "pod-topology-v8/RUNTIME-INTEGRATION-MANIFEST.json"
            ).read_text()
        )
        ...
        self.assertEqual(runtime_identity, {
            "driver_lock": "/q04-control/yolo-pod-cgroup-i.driver.lock",
            "local_output": "/private/tmp/q04-yolo-pod-cgroup-20260920-i",
    ...
    def test_workload_uses_the_h_runtime_manifest_for_validation_and_scope(self):
        ...
        runtime_path = "pod-topology-v8/RUNTIME-INTEGRATION-MANIFEST.json"
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "from pathlib import Path; import pod_preflight_i as p; "
    ...
    def test_generated_manifests_and_offline_check_are_exact(self):
        ...
        self.assertEqual(
            json.loads((runner.Q04 / "pod-topology-v8/SOURCE-MANIFEST.json").read_text()),
