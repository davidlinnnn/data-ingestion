"""Local contracts for the AX worker-process drain window."""
...
from candidate import process_drain_window_ax as matrix
import consumer
from host_an import Host
import pod_topology_ax as topology
import pod_workload_ax as workload
from sentinel import run_process_drain_pod_cgroup_ax as runner
from sentinel import run_process_drain_pod_cgroup_ax_guarded as guarded
...
class ProcessDrainTest(unittest.TestCase):
    ...
    def test_scope_identity_and_exact_command(self):
        path = topology.base.Q04 / 'pod-topology-v49/RUNTIME-INTEGRATION-MANIFEST.json'
        ...
        self.assertEqual(guarded.PROBE_NAME, 'q04-host-pressure-ax-20260925')
        command = shlex.split(runner.exact_command())
        self.assertIn('run_process_drain_pod_cgroup_ax_guarded.py', command[1])
    ...
    def test_exact_projected_workspace_imports(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            result = subprocess.run([sys.executable, '-c',
                'import pod_preflight_ax; from pathlib import Path; '
                'pod_preflight_ax.verify_workload_imports_at('
