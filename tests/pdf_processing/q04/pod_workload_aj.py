...
_spec = importlib.util.spec_from_file_location(
    "q04_pod_workload_aj_engine", Path(__file__).with_name("pod_workload_p.py")
)
...
WORKFLOW_QUEUE = "q04-pod-cgroup-aj-workflows"
ACTIVITY_QUEUE = "q04-pod-cgroup-aj-08"
EVIDENCE_ROOT = Path("/q04-evidence/q04-matrix-pod-cgroup-20260924-aj")


def build_measurement_argv(args, run_id: str, authorization_sha256: str) -> list[str]:
    return [
        args.python,
        "-m", "candidate.matrix_v3_window_aj",
        ...
        str(args.workspace / "tests/pdf_processing/q04/pod-topology-v35/RUNTIME-INTEGRATION-MANIFEST.json"),
...
def adapt_run():
    ...
    source = source.replace(
        old, "pod-topology-v35/RUNTIME-INTEGRATION-MANIFEST.json"
    )
