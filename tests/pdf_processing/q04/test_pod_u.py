"""Regression checks for the U warm Pod adapter."""
...
import pod_topology_u as topology
import pod_preflight_u as preflight
from sentinel import run_warm_pod_cgroup_u as runner


class UWarmPodAdapterTest(unittest.TestCase):
    def test_private_engine_binds_u_identity_before_function_definition(self):
        deployment = {
    ...
    def test_u_is_new_inactive_fail_stop_identity(self):
        self.assertEqual(runner.RUN_IDENTITY, "q04-warm-pod-cgroup-20260921-u")
        self.assertEqual(runner.PREFIX, "q04/warm-pod-cgroup-20260921-u/")
        ...
        self.assertIn("pod_workload_u.py", runner.workload_argv()[1])
        self.assertIn("pod_preflight_u.py", runner.preflight_argv()[1])
    ...
    def test_complete_preflight_configuration_gate_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _, result = preflight.verify_configuration_u(
                capacity=capacity,
                authorization_scope_sha256=runner.authorization_scope_sha256(),
    ...
    def test_s_and_u_import_order_is_isolated(self):
        env = os.environ.copy()
        env["PYTHONDONTWRITEBYTECODE"] = "1"
        for modules in (
            "sentinel.run_warm_pod_cgroup_s,sentinel.run_warm_pod_cgroup_u",
            "sentinel.run_warm_pod_cgroup_u,sentinel.run_warm_pod_cgroup_s",
        ):
            subprocess.run(
                [
                    sys.executable,
    ...
    def test_preflight_imports_from_exact_projected_workspace(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [sys.executable, "-c", "import pod_preflight_u"],
                check=True,
                env=env,
                cwd=root,
                capture_output=True,
