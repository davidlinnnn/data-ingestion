"""Regression checks for the AF warm Pod adapter."""
...
import pod_topology_af as topology
import pod_preflight_af as preflight
from sentinel import run_warm_pod_cgroup_af as runner


class AFWarmPodAdapterTest(unittest.TestCase):
    ...
    def test_af_is_new_inactive_fail_stop_identity(self):
        self.assertEqual(runner.RUN_IDENTITY, "q04-warm-pod-cgroup-20260922-af")
        self.assertEqual(runner.PREFIX, "q04/warm-pod-cgroup-20260922-af/")
        ...
        self.assertIn("pod_workload_af.py", runner.workload_argv()[1])
        self.assertIn("pod_preflight_af.py", runner.preflight_argv()[1])
    ...
    def test_complete_preflight_configuration_gate_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _, result = preflight.verify_configuration_af(
                capacity=capacity,
                authorization_scope_sha256=runner.authorization_scope_sha256(),
    ...
    def test_preflight_rejects_bundle_with_stale_harness_binding(self):
        source_bundle = Path("/private/tmp/q04-inputs-warm-lifecycle-af-final")
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            bundle = root / "bundle"
            bundle.mkdir()
            ...
            with self.assertRaisesRegex(ValueError, "bundle harness drift"):
                preflight.verify_configuration_af(
                    capacity=capacity,
                    authorization_scope_sha256=runner.authorization_scope_sha256(),
                    source_manifest_path=source,
                    bundle=bundle,
    ...
    def test_s_and_w_import_order_is_isolated(self):
        ...
        for modules in (
            "sentinel.run_warm_pod_cgroup_s,sentinel.run_warm_pod_cgroup_af",
            "sentinel.run_warm_pod_cgroup_af,sentinel.run_warm_pod_cgroup_s",
        ):
            subprocess.run(
                [
                    sys.executable,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "import pod_preflight_af; "
                    "import json,tempfile; from pathlib import Path; "
                    "from candidate.warm_pod_window_r import reviewed_reference_checker; "
                    "b=Path('/private/tmp/q04-inputs-warm-lifecycle-af-final'); "
                    "pod_preflight_af.verify_workload_imports_af("
