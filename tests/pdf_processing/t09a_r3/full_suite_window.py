...
def main(held_lock=False):
    with open('/private/tmp/data-ingestion-pdf-qualification.lock','a+') as lock:
        ...
        try:
            ...
            with (ROOT/'tests/pdf_processing/t09a_r3/evidence/full-suite.log').open('w') as log:
                process=subprocess.Popen([str(FIXED/'.venv/bin/python'),str(ROOT/'tests/pdf_processing/t06/run_unit_suite.py')],
                ...
                while process.poll() is None:
                    ...
                    state=run.vm()
                    with (ROOT/'tests/pdf_processing/t09a_r3/evidence/full-suite-resources.jsonl').open('a') as telemetry:
                        telemetry.write(json.dumps({key:state[key] for key in ('time','available','oom_kill')})+'\n')
                    assert state['oom_kill']==baseline['oom_kill'] and state['available']>=512*2**20,'Full suite resource guard'
                    if time.monotonic()>deadline:raise TimeoutError('Full regression suite deadline')
                    time.sleep(.2)
                final=run.vm()
                assert final['oom_kill']==baseline['oom_kill'] and final['available']>=512*2**20,'Full suite final resource guard'
