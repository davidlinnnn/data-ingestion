...
import threading
import run_window
from run_window import NS, ROOT, RUN_ID, PREFIX, OUT, PYTHON, NODE, ACTIVITIES, WORKFLOWS, k, setup, admission, vm, pod, quiesce

NAME='drain-native-resume'
...
def guard_cleanup():
    """Lose telemetry during owned parsing, then prove fail-closed remote cleanup."""
    name='guard-native'
    record={'started':time.time(),'status':'starting'}
    stopped=threading.Event()
    def inject():
        try:
            deadline=time.monotonic()+180
            while not stopped.wait(.5):
                assert time.monotonic()<deadline,'No active parser for sampler-loss guard'
                path=OUT/(name+'-samples.jsonl')
                if not path.exists():continue
                lines=path.read_text().splitlines()
                if not lines:continue
                try:sample=json.loads(lines[-1])
                except ValueError:continue
                if any(p['module']=='pdf_processing.warm_child' and p['cpu_seconds']>1 for p in sample['processes']):
                    worker=pod()['metadata']['name']
                    k('cp',str(ROOT/'tests/pdf_processing/t09a/probe.py'),worker+':/tmp/r2-guard-probe.py')
                    raw=k('exec',worker,'--',PYTHON,'/tmp/r2-guard-probe.py').strip()
                    if not raw:continue
                    stage=json.loads(raw)
                    if not any(p['pid']==stage['event']['pid'] for p in sample['processes']):continue
                    record.update(injected_at=time.time(),active_sample=sample,active_stage=stage)
                    (OUT/'guard-injection.json').write_text(json.dumps(record,indent=2)+'\n')
                    k('exec',worker,'--','touch','/tmp/t09a-stop-sampling')
                    return
        except BaseException as error:record['injection_error']=str(error)
    admission();setup()
    k('scale','deployment/'+WORKFLOWS,'--replicas=1');k('rollout','status','deployment/'+WORKFLOWS,'--timeout=60s')
    thread=threading.Thread(target=inject,daemon=True);thread.start()
    try:
        run_window.trial('native',name,True,['--require-fresh'])
        raise AssertionError('Sampler loss did not reject qualification')
    except AssertionError as error:
        assert 'Sampler ended before complete workflow' in str(error) and 'injected_at' in record,str(error)
        record['guard_error']=str(error)
    finally:
        stopped.set();thread.join(timeout=50);quiesce()
    # This test owns this new request. Close its interrupted test workflow only
    # after the remote worker/child is absent; never target an older execution.
    record['workflow']=json.loads(remote("""import asyncio,json,sys
from pathlib import Path
from temporalio.client import Client
async def main():
 r=Path(sys.argv[1]);a=json.loads((r/'active.json').read_text());assert a['trial']=='guard-native'
 c=await Client.connect('temporal:7233');h=c.get_workflow_handle(a['workflow_id'])
 await h.terminate(reason='T09a controlled sampler-loss test: owned runtime is quiescent')
 d=await h.describe();print(json.dumps({'id':a['workflow_id'],'status':d.status.name}))
asyncio.run(main())""",'/tmp/'+PREFIX+'-results'))
    assert record['workflow']['status']=='TERMINATED'
    record.update(status='passed',quiescent_at=time.time())
    (OUT/'guard-cleanup.json').write_text(json.dumps(record,indent=2)+'\n')

def main(held_lock=False,run_guard=True):
    ...
    with open('/private/tmp/data-ingestion-pdf-qualification.lock','a+') as lock:
        if not held_lock:fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
        try:
            if run_guard:guard_cleanup()
            admission();setup()
            baseline=vm()
            last_node=0
            def monitor():
...
async def main():
 r=json.loads((Path(sys.argv[1])/'drain-native-resume.json').read_text());s=Store(boto3.client('s3',endpoint_url='http://objects:9000'),'t09a','final')
...
asyncio.run(main())''','/tmp/'+PREFIX+'-results'))
            ...
            if not held_lock:fcntl.flock(lock,fcntl.LOCK_UN)
