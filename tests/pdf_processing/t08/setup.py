...
apply({'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'t08-package','namespace':ns},'data':{p.name:p.read_text() for p in (root/'src/pdf_processing').glob('*.py')}})
...
old['verify.py']=(root/'tests/pdf_processing/t08/verify.py').read_text()
apply({'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'t08-driver','namespace':ns},'data':old})
import base64
fixture={'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'t08-fixtures','namespace':ns},'binaryData':{p.name:base64.b64encode(p.read_bytes()).decode() for p in pathlib.Path('/private/tmp/pdf-integration-fixtures').glob('*.pdf') if p.stat().st_size < 100000}}
...
for role in ('workflows','activities','coordinator'):
 ...
 else:obj={'apiVersion':'apps/v1','kind':'Deployment','metadata':metadata,'spec':{'replicas':1,'selector':{'matchLabels':{'app':'t08-'+role}},'template':{'metadata':{'labels':{'app':'t08-'+role}},'spec':spec}}}
 apply(obj)
