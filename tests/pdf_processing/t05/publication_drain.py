...
run=subprocess.Popen(['kubectl','-n',ns,'exec','coordinator','--','/experiment/.venv/bin/python','/tmp/case.py','pubdrain-1'])
started=time.monotonic()
while time.monotonic()-started<60:
    if 'T05_PUBLICATION_PAUSED' in k('logs',pod):
        print(json.dumps({'case':'pubdrain-1','pod':pod,'publication_pause_observed':True}),flush=True)
