...
apply({'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'t06-package','namespace':ns},'data':{p.name:p.read_text() for p in (root/'src/pdf_processing').glob('*.py')}})
profile_path=pathlib.Path('/private/tmp/t06-profile.json')
...
old['verify.py']=(root/'tests/pdf_processing/t06/verify.py').read_text()
apply({'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'t06-driver','namespace':ns},'data':old})
import base64
fixture={'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'t06-fixtures','namespace':ns},'binaryData':{p.name:base64.b64encode(p.read_bytes()).decode() for p in pathlib.Path('/private/tmp/pdf-integration-fixtures').glob('*.pdf') if p.stat().st_size < 100000}}
...
for role in ('workflows','activities','coordinator'):
 ...
 else:obj={'apiVersion':'apps/v1','kind':'Deployment','metadata':metadata,'spec':{'replicas':1,'selector':{'matchLabels':{'app':'t06-'+role}},'template':{'metadata':{'labels':{'app':'t06-'+role}},'spec':spec}}}
 apply(obj)
