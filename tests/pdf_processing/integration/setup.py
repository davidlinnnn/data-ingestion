...
apply({'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'integration-package','namespace':ns},'data':{p.name:p.read_text() for p in (root/'src/pdf_processing').glob('*.py')}})
...
old['verify.py']=(root/'tests/pdf_processing/t04/verify.py').read_text().replace('t04','integration')
apply({'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'integration-driver','namespace':ns},'data':old})
import base64
fixture={'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'integration-fixtures','namespace':ns},'binaryData':{p.name:base64.b64encode(p.read_bytes()).decode() for p in pathlib.Path('/private/tmp/pdf-integration-fixtures').glob('*.pdf')}}
...
for role in ('workflows','activities','coordinator'):
 ...
 for v in spec['volumes']:
  if 'configMap' in v:v['configMap']['name']=v['configMap']['name'].replace('t04','integration')
 spec['terminationGracePeriodSeconds']=60
 for c in spec['containers']:
  c['volumeMounts']=[v for v in c['volumeMounts'] if not v['name'].startswith('kube-api')]
  for env in c.get('env',[]):
   if 'value' in env:env['value']=env['value'].replace('t04','integration')
 ...
 else:obj={'apiVersion':'apps/v1','kind':'Deployment','metadata':metadata,'spec':{'replicas':1,'selector':{'matchLabels':{'app':'integration-'+role}},'template':{'metadata':{'labels':{'app':'integration-'+role}},'spec':spec}}}
 apply(obj)
