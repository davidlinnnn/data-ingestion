"""THROWAWAY. Explicit page checkpoint experiment, not a canonical schema."""
import argparse
import collections
import hashlib
...
def event(stage, pages, elapsed, **extra):
    ...
    with LOCK:
        EVENTS.append(value)
        with (ARGS.out / "events.jsonl").open("a") as f:
...
class CapturePipeline(StandardPdfPipeline):
    def _release_page_resources(self, item):
        ...
        if page is not None and page.assembled is not None:
            t = time.perf_counter()
            image_path = ARGS.out / "checkpoints" / f"page-{page.page_no:04}.png"
            image_path.parent.mkdir(parents=True, exist_ok=True)
            page.image.save(image_path)
            with image_path.open("rb") as f:
                os.fsync(f.fileno())
            data = {
                "format": "PROTOTYPE-page-v1", "source_sha256": sha(ARGS.pdf),
                "method_sha256": sha(ARGS.out / "method.json"),
            ...
            if ARGS.crash_after_page == page.page_no:
                event("injected_exit_during_group", [page.page_no], 0)
                os._exit(75)
    ...
    def _assemble_document(self, conv_res):
        ...
        checkpoints = sorted((ARGS.out / "checkpoints").glob("page-*.json"))
        assert len(checkpoints) == len(conv_res.pages)
        write(ARGS.out / "complete.json", {
            "format": "PROTOTYPE-document-v1", "source_sha256": sha(ARGS.pdf),
            "method_sha256": sha(ARGS.out / "method.json"),
        ...
        if ARGS.checkpoint_only:
            return conv_res
        if ARGS.crash_before_assembly:
            event("injected_exit_before_assembly", [], 0)
            os._exit(73)
        t = time.perf_counter()
...
class RestorePipeline(StandardPdfPipeline):
    ...
    def _build_document(self, conv_res):
        ...
        assert manifest["source_sha256"] == sha(ARGS.pdf)
    ...
    def _assemble_document(self, conv_res):
        ...
        event("document_assembly", [], time.perf_counter() - t)
        if ARGS.crash_before_assembly:
            event("injected_exit_after_assembly_before_output", [], 0)
            os._exit(74)
        return result
...
def method():
    ...
    models = {str(p.relative_to(SCRATCH / "hf")): sha(p) for p in sorted((SCRATCH / "hf").glob("hub/models--*/snapshots/**/*")) if p.is_file()}
    import rapidocr
    ...
    return {"format": "PROTOTYPE-page-v1", "python": platform.python_version(),
            ...
            "options": options(ARGS.scan).model_dump(mode="json", serialize_as_any=True),
            "option_types": {k: type(getattr(options(ARGS.scan), k)).__name__ for k in ("ocr_options", "layout_options", "table_structure_options")},
...
def main():
    ...
    parser.add_argument("mode", choices=["baseline", "capture", "restore", "warmup"])
    parser.add_argument("pdf", type=Path)
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--checkpoint", type=Path)
    parser.add_argument("--scan", action="store_true")
    parser.add_argument("--start", type=int, default=1)
    parser.add_argument("--end", type=int, default=9223372036854775807)
    parser.add_argument("--checkpoint-only", action="store_true")
    parser.add_argument("--crash-after-page", type=int)
    parser.add_argument("--crash-before-assembly", action="store_true")
    ARGS = parser.parse_args()
    ARGS.out.mkdir(parents=True, exist_ok=True)
    instrument(ARGS.mode == "restore")
    pipeline = {"baseline": StandardPdfPipeline, "warmup": StandardPdfPipeline,
                "capture": CapturePipeline, "restore": RestorePipeline}[ARGS.mode]
    converter = DocumentConverter(format_options={bm.InputFormat.PDF: PdfFormatOption(
        pipeline_cls=pipeline, pipeline_options=options(ARGS.scan))})
    t = time.perf_counter()
    converter.initialize_pipeline(bm.InputFormat.PDF)
    event("model_initialization", [], time.perf_counter() - t)
    current_method = method()
    pinned = json.loads((ROOT / "evidence/native-method.json").read_text())
    assert current_method["model_artifacts"] == pinned["model_artifacts"], "Model artifacts differ from captured pins"
    write(ARGS.out / "method.json", current_method)
    if ARGS.mode == "warmup":
        return
    t = time.perf_counter()
    ...
    result.document.save_as_json(ARGS.out / "document.json")
    (ARGS.out / "document.md").write_text(result.document.export_to_markdown())
    counts = collections.Counter()
    seconds = collections.Counter()
    for e in EVENTS:
        counts[e["stage"]] += len(e["pages"])
        seconds[e["stage"]] += e["seconds"]
    report = {"pid": os.getpid(), "mode": ARGS.mode, "wall_seconds": time.perf_counter()-START,
              "convert_seconds": elapsed, "peak_rss_bytes": resource.getrusage(resource.RUSAGE_SELF).ru_maxrss,
              "rss_units": "bytes on macOS", "page_stage_inputs": dict(counts),
              "stage_seconds_sum_not_wall": dict(seconds), "source_sha256": sha(ARGS.pdf),
              "document_sha256": sha(ARGS.out / "document.json"),
              "labels": dict(collections.Counter(str(item.label) for item, _ in result.document.iterate_items())),
              "errors": [e.model_dump(mode="json") for e in result.errors]}
    write(ARGS.out / "metrics.json", report)
