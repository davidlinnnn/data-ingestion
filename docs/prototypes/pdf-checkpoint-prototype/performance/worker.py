...
sys.path.insert(0,str(Path(__file__).resolve().parent.parent/"k8s"))
from object_store import Store, digest

ROOT = Path(__file__).resolve().parent.parent
PYTHON = ROOT/'.venv/bin/python'
PDF = ROOT/'fixtures'/os.environ.get('PDF_NAME','llm-survey-2303.18223v1.pdf')
PAGE_COUNT=int(os.environ.get('PAGE_COUNT','51'))
SCAN=os.environ.get('PDF_SCAN')=='1'
POLL=os.environ.get('POLL_MODE','legacy')
...
async def child(args, out, fault, identity):
    ...
    with (out/'process.log').open('wb') as log:
        p = await asyncio.create_subprocess_exec(str(PYTHON), str(ROOT/'performance/entry.py'), *map(str,args), *(['--scan'] if SCAN else []),
...
async def produce(spec):
    ...
    producer = {p: digest((ROOT/p).read_bytes()) for p in ['experiment.py', 'k8s/ocr_component.py', 'performance/worker.py', 'performance/entry.py', 'k8s/object_store.py']}
    ...
    if kind == 'group':
        ...
        verify_group(merged,1,PAGE_COUNT)
        await child(['restore',PDF,'--checkpoint',merged,'--out',out,'--end',PAGE_COUNT],out,fault,identity)
    ...
    while not publish_task.done():
        ...
        if POLL=='legacy':
            ...
            await asyncio.wait({publish_task},timeout=1)
    await publish_task
